...
## Plugin sandbox and control plane

The optional adjacent `junban-plugin-host` is the only crate linked to Wasmtime. The parent `PluginRuntimeSupervisor` discovers only the exact sibling binary, authenticates the private protocol/product version, owns one bounded child for at most 16 selected plugins, and kill/reaps it on compile/load deadline, transport failure, restore, or shutdown. Every load, invocation, callback, effect, render, and action is fenced by exact package generation, activation epoch, host session, invocation/callback identity, kind, and body hash. The plugin event worker starts lazily only for a non-dormant graph; an empty/default profile starts no child, worker, Engine, or Wasmtime compilation pool.

Before compilation, the child revalidates actual imports against the exact runtime profile and canonical grants and builds a deny-by-default linker. TypeScript receives no WASI. Rust receives only `wasi:io/error`, `wasi:io/streams`, `wasi:cli/environment`, `wasi:cli/exit`, and `wasi:cli/stderr` at exact version 0.2.6, linked individually with empty environment/arguments, closed input, no stdout, and bounded sink stderr. Ambient WASI network, filesystem/preopens, random, clocks, processes, inherited stdio/environment, HTTP, and broad linker helpers are absent. Wasmtime 36.0.13 compilation is parallel only through one lazily constructed Rayon pool bounded to two workers with 1-MiB stacks.

Each plugin has a limited retained Store/instance. Calls reset finite profile-specific fuel and an epoch deadline. Watchdogs own the frozen 1-second command/lifecycle/UI deadline or 250-millisecond event/render/validation/resync deadline; the parent owns the exact 30-second synchronous compile/load deadline and kills/reaps the whole child on expiry. Trap, fuel, interruption, limit failure, invalid/oversized output, cancellation, and abort destroy the failed Store before terminal acknowledgement and permit only a clean re-instantiation. Memory, tables/instances, stack, host resources, IPC/callback copies, output, stderr, and guest logs are bounded. Stable errors never return Wasmtime trap strings, guest stderr, callback bodies, package bytes, credentials, or private paths.

Server callback adapters, not the guest or child, are the product capability authority. They recheck current lifecycle fences and grants before bounded task/project/tag queries, validated AppService effects, isolated KV/settings, logs, pinned public-IPv4 HTTPS, clock, and service calls. Plugins never receive SQLite, profile paths, bearer tokens, signing material, or arbitrary application mutation bytes. The server exposes only operator-authorized package/trust/policy/grant/setting/lifecycle/registry/contribution routes; automation bearers are rejected even with every scope. The bundled registry and references are verified from public keys and exact content-addressed bytes. The browser renders bounded declarative trees and never executes plugin React, HTML, or JavaScript.

Restore drains and reaps the runtime, disables restored plugins, advances activation fences, requires package re-verification and event resync, and never replays pre-restore hooks. Uninstall commits authority deletion before removing only unreferenced package/cache objects. Complete backups exclude package/cache files and signing material. Operational and author guidance: [`plugins.md`](plugins.md).

Signing proves artifact integrity, not safety. `junban-plugin-artifact` accepts private seed bytes only from an owner-private file, rejects Git checkouts, `target`, evidence output, and common package-cache paths, and rejects symlinks, multiple hard links, or permissive Unix metadata. Ceremony policy additionally keeps keys outside profiles, all build roots, logs, CI, and release output. Signing intentionally fails closed on Windows because the tool has no reviewed owner-only DACL verifier. Public verification, component/reference builds, and the isolated Windows product runtime remain supported; the signing-host restriction does not reduce cross-platform runtime support.

## TypeScript plugin authoring audit disposition

The exact TypeScript reference build graph includes `@bytecodealliance/componentize-js@0.22.0 → @bytecodealliance/weval@0.4.1 → decompress@4.2.1`. Its package-local `npm audit` reports the decompression advisory and **does not pass**; this is not called fixed or accepted as a clean audit.

The exposure is bounded to an author/build host: all versions and the package-local npm lock are exact, the authoritative invocation disables AOT (`--disable all` and `enableAot:false`), and the build consumes no untrusted archive. Node and this dependency graph are absent from Junban inspection, installation, server, host, and plugin runtime. Reassess and remove or replace the disposition before enabling AOT, accepting any untrusted archive/build input, changing Node/npm/JCO/ComponentizeJS/weval/decompress versions or invocation paths, or when the advisory or an applicable patched dependency changes. The repository's mandatory root `pnpm audit --audit-level high` gate is unchanged and must not be suppressed, filtered, or weakened by this disposition.
