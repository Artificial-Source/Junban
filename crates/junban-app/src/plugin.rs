...
/// Nonserializable proof that one staged package exact-matches an entry from a
/// registry accepted by the SDK's signed JRI1 authority.
#[derive(Debug)]
pub struct VerifiedBundledPluginAdmission {
    admission: PluginPackageAdmission,
    registry_index_sha256: Sha256Digest,
}

impl VerifiedBundledPluginAdmission {
    pub fn new(
        admission: PluginPackageAdmission,
        registry: &junban_plugin_sdk::VerifiedRegistry,
        plugin_id: &PluginId,
        version: &str,
    ) -> Result<Self, SdkError> {
        ...
        let entry = registry
            ...
            .ok_or(SdkError::Registry { field: "entry" })?;
        let manifest = package.manifest();
        let requested_capabilities: Vec<_> = manifest
            .permissions
            .iter()
            .map(|permission| permission.capability)
            .collect();
        if entry.plugin_id != package.plugin_id().as_str()
            || entry.version != package.manifest().version
            || entry.package_sha256 != package.package_sha256().as_str()
            || entry.package_size != package.package_size()
            || entry.publisher_key_id != package.publisher_key_id().as_str()
            ...
            || entry.requested_capabilities != requested_capabilities
            || entry.filename
                != junban_plugin_sdk::registry_package_path(package.package_sha256().as_str())?
        {
        ...
        Ok(Self {
            admission,
            registry_index_sha256: Sha256Digest::parse(registry.index_sha256().to_owned())?,
        })
    }

    #[must_use]
    pub const fn package(&self) -> &PluginPackageAuthority {
        self.admission.package()
    }

    #[must_use]
    pub const fn registry_index_sha256(&self) -> &Sha256Digest {
        &self.registry_index_sha256
    }

    #[must_use]
    pub fn into_parts(self) -> (PluginPackageAdmission, Sha256Digest) {
        (self.admission, self.registry_index_sha256)
    }
}
...
/// Stable caller-known identity used only to replay plugin grant/setting
/// mutations before consulting current manifest or runtime authority.
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum PluginMutationRequestIdentity {
    ReplaceGrants {
...
impl PluginMutationRequestIdentity {
    #[must_use]
    pub const fn operation_id(&self) -> OperationId {
        match self {
            Self::ReplaceGrants { operation_id, .. }
            | Self::RevokeGrants { operation_id, .. }
            | Self::SetSetting { operation_id, .. }
            | Self::DeleteSetting { operation_id, .. } => *operation_id,
        }
    }
}
...
pub struct CompletePluginInvocationRequest {
    ...
    pub package_generation: u64,
    pub activation_epoch: u64,
    pub outcome: PluginInvocationPublicOutcome,
    pub delivery: crate::PluginInvocationDelivery,
}
...
fn plan_plugin_invocation_commit(
    ...
    if domain_effect.is_some() != child_operation_id.is_some()
        ...
        || (outcome != PluginInvocationPublicOutcome::Completed
            && (domain_effect.is_some()
                || kv_patch.is_some()
                || resync_kv.is_some()
                || cursor.is_some()
                || resync_session.is_some()))
    {
...
/// Closed public classification durably bound to an operator invocation receipt.
/// Authority and session failures never enter receipt material.
#[derive(Clone, Copy, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum PluginInvocationPublicOutcome {
    Completed,
    Cancelled,
    GuestTrap,
    Timeout,
...
pub trait PluginRepository: Send + Sync + 'static {
    ...
    /// Admit a package only through nonserializable signed-registry authority.
    fn install_bundled_plugin_admission(
        &self,
        _operation_id: OperationId,
        _admission: VerifiedBundledPluginAdmission,
    ...
    fn replay_plugin_mutation(
        &self,
        _identity: PluginMutationRequestIdentity,
        _now: Timestamp,
    ) -> RepositoryFuture<'_, Option<CommittedMutation>> {
        plugin_unavailable()
    }
