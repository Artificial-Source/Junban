...
Retained Rust fixtures prove both an 8-KiB maximum-valid search and a 4,190,208-byte near-callback-bound canonical transfer reach the generated adapter/callback and return normally, while a 558,081-element empty-ID list (4,464,648 canonical flat bytes) traps before callback publication, returns only normalized `resource-limit`, destroys the Store, and succeeds from a clean replacement in the same child process. The retained TypeScript all-nine-export path returns a `kv-patch` with one 4,464,641-byte delete-key string that exhausts hostcall fuel before generated post-return processing or protocol serialization, then proves clean same-process replacement. The replacement [`phase-7-hostcall-transfer-linux.json`](phase-7-hostcall-transfer-linux.json) records five optimized direct exact-test TypeScript runs for the new fixture. GNU time maximum-resident-set results were 505,368 / 531,560 / 530,608 / 546,312 / 533,672 KiB (546,312 KiB maximum); the report identifies that its direct-process metric supersedes the retired aggregate sampler. The TypeScript figure includes its ordinary compile/instantiate peak and is not relabeled as transfer-only allocation.
