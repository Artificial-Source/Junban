...
Date: 2026-08-04
Status: Wave 0 and Wave 1 accepted; Wave 2 child-runtime security recheck accepted; `P7-PLAN-2C-001`–`003` closed by focused recheck; `P7-PLAN-2C-004` fixed in writing and pending focused recheck; Slice 2C unauthorized
...
- persisted monotonic `activation_epoch` identifies a runtime admission or graph-fence generation, not every health-state change. Enable, explicit manual retry and a due automatic retry each increment it exactly once while entering `starting`; enable/manual retry begins a fresh failure series, while automatic retry preserves the prior `failure_count`. Disable, grant/revoke, package, restore and other authority changes that invalidate admitted work also advance it once. `loaded`, same-attempt `starting`→`active`, and ordinary plugin-local trap, invocation-timeout or resource-failure transitions retain the current attempt epoch. A child/session-fatal failure instead advances every matching affected selected plugin epoch exactly once in one atomic graph fence before replacement.
...
A child/session-fatal failure uses one atomic graph-fence AppService mutation before replacement. For every affected selected plugin whose expected epoch and session still match—whether `starting` or `active`, including any plugin whose compile/load failed, loaded siblings and not-yet-loaded or skipped dependents—the mutation increments its epoch exactly once and records its bounded failure progression at that new epoch. It records no material failure at the old epoch and cannot increment an affected plugin twice. The failing plugin receives the stable `child-fatal` or `compile-load` cause, loaded/active siblings receive `child/session-lost`, and a not-yet-loaded or skipped dependent whose prerequisite cannot activate receives `dependency_failed`. The existing `degraded`→`failed`→`suspended` progression applies at the new graph-fence epoch, suspension clears desired enablement, and exact bounded receipts and material events are committed in the same transaction. Any epoch/session CAS mismatch aborts fail closed without a partial graph transition. Late old-session or old-epoch frames cannot commit. A later manual or due retry is a separate attempt, increments once again into `starting`, and retains that retry epoch through `Loaded`→resync/catch-up→`active`. An interrupted `starting` attempt may therefore move E→E+1 when the graph-fatal fence invalidates the lost child session and later E+1→E+2 for its retry; those distinct fence and retry generations are intentional, not double-counting one attempt.
...
Child/session-fatal failure first closes admission and kills/reaps as required, then uses the one atomic graph-fence AppService mutation defined above. The expected epoch/session CAS covers every affected selected `starting` or `active` plugin, including the failing compile/load plugin, loaded siblings and not-yet-loaded or skipped dependents. It advances each matching epoch exactly once, records every bounded cause-specific health transition, receipt and material event only at the new epoch, and aborts the entire graph mutation fail closed on any mismatch. The graph-fence transaction completes before replacement. A later manual or due retry enters a separate fresh `starting` epoch, and successful `Loaded`→resync/catch-up→`active` retains that retry epoch.
...
- Host crash never changes task data. Runtime failure counters/cursors/KV remain plugin-local and cannot create a recursive global event stream. Admission closes and the child is killed/reaped before one atomic graph-fence AppService mutation advances every matching affected selected plugin exactly once and records bounded cause-specific graph-health/dependency transitions, receipts and material events only at the new epoch. Any CAS mismatch aborts without partial transition. Replacement starts only afterward, re-verifies exact package-generation/grants/graph, and discards old-session/old-epoch replies; a later retry increments once again, while repeated failure follows `degraded`→`failed`→`suspended` without guessing.
...
| `P7-PLAN-2C-004` | High     | fixed  | Ordinary plugin-local trap/timeout/resource failure retains the current epoch and replaces only its Store/instance. Child/session-fatal loss atomically advances every matching affected selected plugin exactly once at a new graph-fence epoch before replacement; a later retry advances once again. Fixed in writing; one focused recheck remains.                                                                                                 |

The original focused planning recheck approved `P7-PLAN-001`–`003`, after which Wave 0 retained the on-demand child and Wave 1 was accepted. The focused Slice 2C recheck closed `P7-PLAN-2C-001`–`003`. `P7-PLAN-2C-004` is fixed in writing but remains pending the one focused recheck, so Slice 2C is unauthorized until that verdict. Separately, the approved hostcall-fuel security recheck fixes `P7-PLAN-RUNTIME-001` and `P7-RUNTIME-SEC-001` and authorizes Slice 2C from that security gate. That approval does not waive the pending `P7-PLAN-2C-004` recheck, the open `P7-DEP-001`, or the requirement that the current macOS containment CI failure pass before Wave 2 acceptance.
