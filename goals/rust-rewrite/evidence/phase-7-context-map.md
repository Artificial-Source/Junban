...
Date: 2026-08-09
Status: Wave 0 and Wave 1 accepted; Wave 2 and Slice 2C in progress; Packet B lifecycle authority commit `ae9cd24` integrated with its focused database recheck approved at exact `e573cfe`, `P7-2C-B-DB-001` and `P7-2C-B-DB-002` fixed/closed, schema v7 unchanged, and Packet C authorized to consume those APIs; Packet A protocol-v2/multi-runtime child commit `651cf75` integrated but not accepted, with hostile-runtime/security review pending; `P7-DEP-001` open
...
- `junban-plugin-sdk` owns bounded package/WIT/trust/component/protocol authority without Wasmtime, storage or runtime construction. Packet A commit `651cf75` integrates private protocol v2/product-version fencing and upgrades the isolated `junban-plugin-host` from the Slice 2B one-load checkpoint to one Engine with a bounded map of at most 16 activation-fenced runtimes and independent per-plugin/four-total admission. The child retains selective linking, typed callbacks, all nine guest exports, watchdog interruption, active drain, failed-Store destruction and clean replacement. Packet A is not accepted: its hostile-runtime/security review is pending. Parent supervision and capability/effect execution remain Packet C and Slice 2D.
- SQLite schema head remains v7; accepted Wave 1 persistence owns normalized plugin package, trust, grants, settings, KV, dependencies, cursors, invocations and health authority. Packet B commit `ae9cd24` integrates the typed due-retry, activation-completion, attempt-failure and atomic graph-fence lifecycle APIs. The focused database recheck approved exact `e573cfe` with no material issues, fixed/closed `P7-2C-B-DB-001` and `P7-2C-B-DB-002`, confirmed schema v7 is unchanged, and authorized Packet C to consume those APIs.
...
Packet A commit `651cf75` bumps the private host protocol from v1 to **v2** before parent composition. The exact protocol name/version are `junban-plugin-host-v2` and `2`. Parent `Hello` and child `Hello` reply both carry the exact compiled `env!("CARGO_PKG_VERSION")` Junban product-version string as `junban_version` in addition to the protocol identity and fresh host session. The parent sends no component bytes until it has exact-matched the complete reply; protocol-name, protocol-version, product-version or session mismatch is child/session-fatal, with no v1 fallback or compatibility negotiation. Canonical frame goldens cover both Hello directions and every mismatch. This implementation remains pending hostile-runtime/security review and is not accepted.
