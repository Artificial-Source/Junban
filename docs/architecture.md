...
The accepted runtime placement remains an on-demand `junban-plugin-host` child, but that product host is a later wave and does not yet exist. Default `junban-server` touches one zero-allocation static SDK authority containing stable names plus typed pointers to every production parser/validator, so thin LTO retains the real SDK boundary; `--no-default-features` is the matched feature-off baseline. Neither build links Wasmtime. The completed Wave 0 probe crate, harness, and Wasmtime advisory exception were removed after the placement ADR was accepted.
