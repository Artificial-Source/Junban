...
**Status:** approved by the user on 2026-07-28 and expanded with an evidence-driven Phase 10 on 2026-07-29. Phases 0 through 6 are complete. Phase 7 capability-limited portable plugins are in progress: Waves 1 and child-runtime Slice 2B are implemented; exact cross-platform calibration rejected the macOS `RLIMIT_AS` remedy, and the approved narrower Wasmtime hostcall-fuel correction is implemented with focused recheck still blocking Slice 2C. Phases 8–10 remain.
...
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Contract remains `evidence/phase-7-context-map.md`. Planning, schema, package, WIT and host-placement gates are approved.
  ...
  - [ ] Wave 2 child-runtime security correction: `P7-RUNTIME-SEC-001` found canonical ABI lifting can amplify 64/128-MiB guest strings/lists into native allocations before post-lift callback/output limits. The approved minimal correction is implemented; focused recheck remains required before continuing Slice 2C.
    - [x] Calibration-only preflight and exact run `31008905408`: 128-MiB Wasmtime reservation with zero guard/growth reservation, retained valid Rust/TypeScript working-set callback barriers, and exact-child release sampling passed one warm-up plus five runs per profile on Linux/macOS/Windows. Linux and Windows had candidate separation; macOS's roughly 415-GiB valid VSIZE made its mechanical 519-GiB `RLIMIT_AS` minimum ineffective and rejected that remedy.
    - [x] Narrower guest-to-host correction: every initial/replacement Store sets and reads back 4,464,640 bytes of Wasmtime 36.0.13 hostcall fuel before instantiation, separate from wasm execution fuel and without metering host-to-guest values. The 4-MiB callback body + 139,264-byte maximum valid ABI structure + 128-KiB margin derivation, all 11 imports/9 exports, maximum-valid, near-bound, and oversized Rust lifts, high-cardinality TypeScript post-return, normalized failure, Store destruction, same-process replacement, empty diagnostics, and five-run optimized Linux exact-child peaks are covered. Public WIT/protocol/guest/runtime limits and dependencies are unchanged. `P7-PLAN-RUNTIME-001`, `P7-RUNTIME-SEC-001`, and `P7-DEP-001` remain open pending focused recheck; no Slice 2C authorization is claimed.
  - [ ] Wave 2 Slices 2C–2E: parent supervisor/verified source and compile-time kill/reap, capability/effect/dependency/event authority, integrated security review and optimized replacement measurements remain; Wave 2 and `P7-DEP-001` are still open.
