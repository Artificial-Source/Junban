...
**Status:** approved by the user on 2026-07-28 and expanded with an evidence-driven Phase 10 on 2026-07-29. Phases 0 through 6 are complete. Phase 7 capability-limited portable plugins are in progress: Waves 1 and child-runtime Slice 2B are implemented; the approved hostcall-fuel focused recheck fixes `P7-PLAN-RUNTIME-001`/`P7-RUNTIME-SEC-001` and authorizes Slice 2C from that security gate. The focused Slice 2C planning recheck closed `P7-PLAN-2C-001`–`003`; `P7-PLAN-2C-004` and `P7-PLAN-2C-004-EVENT-ATOMICITY` are fixed in writing but not planning-approved until one focused recheck, so Slice 2C remains unauthorized. `P7-DEP-001` and the current macOS containment CI failure remain open. Phases 8–10 remain.
...
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Contract remains `evidence/phase-7-context-map.md`. Wave 0/1 planning, schema, package, WIT and host-placement gates are approved; Slice 2C remains unauthorized pending the `P7-PLAN-2C-004` and `P7-PLAN-2C-004-EVENT-ATOMICITY` focused recheck.
...
- Phase 7 Slice 2C planning review — **partially closed; implementation unauthorized**. The focused recheck closed `P7-PLAN-2C-001`–`003`. `P7-PLAN-2C-004` freezes same-attempt plugin-local failure versus child/session-fatal graph fencing and the separate retry increment; `P7-PLAN-2C-004-EVENT-ATOMICITY` freezes the one-operation/transaction/revision/`plugin.health_changed` event/receipt envelope, bounded canonical per-plugin list, sorted affected IDs, atomic whole-graph CAS, replay and conflict rule. Both are high/fixed in writing but not planning-approved until one focused recheck.
- Phase 7 hostcall-fuel security recheck — **approved after fixes**. The recheck closes `P7-PLAN-RUNTIME-001` and `P7-RUNTIME-SEC-001` and authorizes Slice 2C from that security gate. `P7-DEP-001`, current macOS containment CI run `31036645108`, the `P7-PLAN-2C-004`/`P7-PLAN-2C-004-EVENT-ATOMICITY` focused recheck, parent compile/load reap, and integrated evidence remain open.
