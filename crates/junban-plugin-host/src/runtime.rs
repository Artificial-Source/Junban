...
struct RuntimeStatus {
    ...
    completed_terminal: Option<AuthorityFence>,
...
impl SharedRuntimeStatus {
    ...
    pub fn start(&self, fence: AuthorityFence, timeout: Duration) -> Result<(), StartError> {
        ...
        // A newly admitted invocation advances terminal authority. From this
        // point, a cancel for the previous invocation must be stale rather
        // than accepted by its retained completion fence.
        status.completed_terminal = None;
    ...
    fn cancel_and_wait_after_linearization(
        ...
        let Some(active) = status.active.as_mut() else {
            return if status
                .completed_terminal
                .as_ref()
                .is_some_and(|completed| completed.exact_matches(fence))
            {
                CancelResult::Lost
            } else {
                CancelResult::Stale
    ...
    fn complete(
        ...
        let result = match (
            ...
            (None, Ok(message)) => {
                if outbound.try_send(OutboundMessage::typed(message)).is_ok() {
                    self.admission.release(fence);
                    status.completed_terminal = Some(fence.clone());
                    status.active = None;
                    let pending = status.pending.take();
                    self.changed.notify_all();
                    drop(status);
        ...
        let pending = {
            ...
            let terminal_enqueued = if let Some(message) = message {
...
mod tests {
    ...
    #[test]
    fn completed_terminal_accepts_only_its_exact_late_cancel() {
        ...
        status.mark_loaded();
        let first = fence();
        status.start(first.clone(), Duration::ZERO).unwrap();
        let (outbound, receiver) = mpsc::sync_channel(2);
        let discarded = Cell::new(false);

        status.complete(&first, Err(HostFailureCode::GuestError), &outbound, || {
            discarded.set(true)
        });
        ...
        assert!(matches!(
            receiver.recv().unwrap().frame,
            ChildFrame::Failed {
                fence,
                code: HostFailureCode::Timeout,
            } if fence == first
        ));
        assert_eq!(status.cancel_and_wait(&first), CancelResult::Lost);

        let mut changed_invocation = first.clone();
        changed_invocation.invocation_id = "00000000-0000-4000-8000-000000000003".into();
        assert_eq!(
            status.cancel_and_wait(&changed_invocation),
            CancelResult::Stale
        );
        let mut changed_activation = first.clone();
        changed_activation.activation_epoch += 1;
        assert_eq!(
            status.cancel_and_wait(&changed_activation),
            CancelResult::Stale
        );

        status
            .start(changed_invocation.clone(), Duration::from_secs(1))
            .unwrap();
        assert_eq!(status.cancel_and_wait(&first), CancelResult::Stale);
        let outcome = InvocationOutcome::Activate(neutral::WitResult::Ok(()))
            .into_child_message(changed_invocation.clone())
            .unwrap();
        status.complete(&changed_invocation, Ok(outcome), &outbound, || {
            panic!("successful Store must be retained")
        });
        assert!(matches!(
            receiver.recv().unwrap().frame,
            ChildFrame::Outcome { fence, .. } if fence == changed_invocation
        ));
        assert_eq!(status.cancel_and_wait(&first), CancelResult::Stale);
        assert_eq!(
            status.cancel_and_wait(&changed_invocation),
