...
**Status:** approved by the user on 2026-07-28 and expanded with an evidence-driven Phase 10 on 2026-07-29. Phases 0 through 7 are complete. Phase 7 delivers signed capability-limited portable plugins, the isolated on-demand host, operator contracts, preserved Extensions UI, three offline references, and accepted cross-platform/dogfood/performance evidence. All named Phase 7 findings are closed or explicitly disposed; the single-squash exact-head CI publication gate passed. Phases 8–10 remain open.
...
- exactly one on-demand sibling host whose one Engine owns at most 16 activation-fenced serialized plugin runtimes, with independent parent/child one-plugin/four-total admission and counted bounded dependency invokes;
- strict `current_exe()` sibling discovery, cleared environment and exact private protocol-v2/Junban-product-version handshake before component bytes, with no product executable override;
- SQLite/AppService-owned desired state, epochs, health, retry/backoff, dependency propagation, events and receipts; `Loaded` precedes required resync/catch-up and same-epoch activation CAS; ordinary plugin-local trap/timeout/resource failure retains its attempt epoch and replaces only its Store/instance; child/session-fatal loss covers the complete selected max-16 graph with either zero or exactly one `Failing`, never multiple, using one operation identity and exactly one AppService mutation/SQLite transaction/global revision/bounded `plugin.health_changed` event/durable receipt to fence every selected plugin once at a new epoch before replacement. A triggered fence has the exact one plugin-correlated `Failing` + `CompileLoad`/`ChildFatal`, exact transitive selected `SkippedDependent` + `DependencyFailed` entries, and all other `LoadedSibling` + `SessionLost` entries; a Hello/idle/uncorrelated session loss without authorized plugin identity is triggerless with zero failing/skipped entries and all selections `LoadedSibling` + `SessionLost`. The session ID is generated before connect and remains canonical receipt authority; empty selection stays `Dormant` with no session/fence; attributable source/package failure retains plugin-local/package or compile/load authority; request/results/affected IDs remain sorted, unique and complete, and a later retry increments once again and retains its retry epoch through activation;
...
- ordinary plugin-local trap/timeout/resource failure is same-attempt, retains that plugin's current epoch and replaces only its Store/instance while siblings remain active; protocol/transport/EOF/exit/worker/session loss and compile/load timeout or failure requiring child reap use one fail-closed atomic triggered or triggerless graph fence before replacement;
- one PID loads the dependency graph; 16/17 runtime bounds and four/fifth/same-plugin/nested saturation pass; exact triggered compile/load failure/timeout and plugin-correlated fatal protocol/runtime fixtures prove exactly one `Failing`, exact transitive selected `SkippedDependent` + `DependencyFailed`, and every other selected entry `LoadedSibling` + `SessionLost`; exact triggerless connect/spawn failure, Hello failure, idle exit and uncorrelated EOF/transport/worker/process/control fixtures prove zero `Failing`, zero `SkippedDependent`, every selected entry `LoadedSibling` + `SessionLost`, pre-connect canonical receipt identity and one epoch advance even before component bytes; empty selection stays `Dormant` without session/fence and attributable source/package failures retain plugin-local/package or compile/load authority; both variants mix durable `starting`/`active` selections and prove exact before/after epochs, per-plugin failure progression, no old-epoch material transition, and exactly one operation/AppService mutation/SQLite transaction/global revision/bounded `plugin.health_changed` event/durable receipt for the complete graph fence—not N envelopes; request/results/`AffectedIds.plugin_ids` are sorted, unique, bounded to 16 and complete; focused Packet B request/receipt/open/backup/restore/replay tests accept zero or one `Failing`, reject multiple and reject wrong role/cause shapes; whole-graph CAS, transitions, affected IDs, event and receipt roll back together on mismatch; exact replay returns the one receipt, a changed request conflicts, and no new event/schema/OpenAPI shape or migration is introduced; retry-epoch activation plus stale-frame rejection, compile/load reap, discovery rejection, sibling survival and Linux/macOS/Windows no-orphan behavior pass;
...
- each supported installed package verifies that the server and `junban-plugin-host` sibling directory is writable only by the same OS principal that owns the installation or by administrator/elevated authority, using target-native owner/mode/ACL evidence; a less-privileged writer is an unsupported insecure installation with fail/stop guidance, not a supported lower-privilege boundary;
...
- release-packaging evidence re-verifies that the server and `junban-plugin-host` sibling directory is writable only by the installation owner or administrator/elevated authority on every supported target and retains fail/stop guidance for an unsupported less-privileged-writable installation;
...
- [x] Phase 7 Wave 0 host-placement gate: temporary `tools/phase7-host-placement` probes compared protocol-only / lazy in-process / SDK-parent-plus-child with real Rust + pure TypeScript components, actual import inspection, fair warm/peak/cold selection, trap/CPU/memory/in-flight-child-crash/recovery/cleanup containment, exact byte/hash binding and cgroup evidence. The clean idle five-sample campaign and focused architecture recheck closed `P7-ARCH-001`–`004` and accepted `on_demand_child_host`. Historical projected active gates were later disproved and formally amended by corrected Slice 2E zero-swap evidence to Rust 91,678,720/131,328,000 bytes and TypeScript 692,418,560/806,824,960 bytes current/peak; ordinary no-plugin 24/32 MiB remains unchanged. See `evidence/phase-7-host-placement-adr.md` and `evidence/phase-7-host-placement.json`.
- [x] Phase 7 portable-plugin implementation (Waves 1–5). All waves are accepted; named findings are closed or explicitly disposed; the clean-candidate dogfood, performance, cross-platform, final-review, single-squash, and exact-head CI gates pass. Full closure: [`evidence/phase-7-outcome.md`](evidence/phase-7-outcome.md).
  ...
  - [x] Wave 1 persistence Slice A: schema v7 and all ten normalized plugin tables, direct SDK authority reuse, verified pre-v7 snapshots, transaction-last commit semantics, strict normal-open/backup/restore validation, and restore disable/reverify/epoch/cursor sanitization. Focused database review closed `P7-DB-001`–`005` and authorized Slice B.
  - [x] Wave 1 persistence Slice B and database gate: normalized lifecycle/trust/grant/settings/KV/dependency/cursor/invocation/health authority, AppService-owned atomic effects, exact terminal receipts, one-invocation fencing and private staged streaming package publication are integrated. Focused recheck closed `P7-DB-006`–`011` and approved Wave 1 persistence; see `evidence/phase-7-wave-1-slice-b.md`.
  - [x] Wave 2 plan and dependency-security checkpoint: `RUSTSEC-2026-0222` blocks the historical Wasmtime 45.0.3 spike line. Production keeps Rust 1.93 and selects exact patched 36.0.13 LTS with defaults off and no advisory exception. Replacement hostile/performance evidence culminated in passing exact amended-head Slice 2E run `31634972732`; focused final recheck reported no material finding and closes `P7-DEP-001` for the dependency/runtime evidence boundary. See `evidence/phase-7-wave-2-plan.md`.
  - [x] Wave 2 Slice 2A isolated child/protocol foundation: exact Wasmtime 36.0.13 is confined to `junban-plugin-host`; the SDK-owned strict protocol codec, one-component hash-bound compile authority, state/correlation fencing, bounded failures, protocol-only stdout, clean EOF/shutdown, and scrubbed absolute sibling-process harness pass. Later Slices 2B–2E supplied selective linking, guest execution, hostile containment, parent cleanup, and replacement optimized evidence, culminating in closure of `P7-DEP-001`; this historical Slice 2A checkpoint did not itself close the gate. See `evidence/phase-7-wave-2-slice-2a.md`.
  - [x] Wave 2 Slice 2A.1 private body implementation: checked-in WIT-parser-generated serde values/function adapters and neutral↔Wasmtime binding adapters, strict canonical JSON, exhaustive typed request/outcome/host-call mappings, kind/branch-derived frame/body hashes, fence-plus-entry context construction, standalone outcome kinds, protocol validation, exhaustive goldens/adapter round trips, and a permanent regeneration/WIT-SHA drift check are integrated. Load remains raw; cancellation is the only empty callback reply. No linker or guest execution was added. See `evidence/phase-7-wave-2-slice-2a1.md`.
  - [x] Wave 2 Slice 2A.1 focused API-contract recheck: `P7-API-001` is fixed; generated authority, strict canonical rejection, all 9 invocation and 11 host-call mappings, typed hashes/context, error/cancel semantics, adapters and WIT drift checks are approved. Slice 2B guest execution is authorized.
  - [x] Wave 2 Slice 2B.1 bounded selective sandbox: actual imports are revalidated against profile/grants; only exact imported Junban interfaces—with grants required for capability-bearing ones—and Rust's five individually linked frozen WASI interfaces exist; one limited serial Store/instance executes all nine generated-adapter exports with retained state and exact callback rendezvous/rejection. Retained zero-WASI TypeScript and Rust callback/grant tests pass with busy/active-control fail-closed and idle cleanup. See `evidence/phase-7-wave-2-slice-2b1.md`.
  - [x] Wave 2 Slice 2B.2 child containment: one watchdog owns exact 1,000/250-ms wall deadlines and epoch/cancel/drain ordering; failed/stopped Stores are destroyed before terminal/control acknowledgement and cleanly re-instantiated from retained compiled authority. Retained hostile Rust/TypeScript coverage proves fuel/CPU, memory/bulk-memory, core-table/host-resource, stack, output/log/stderr, forbidden imports, callback EOF/kill/races, replacement and cleanup; CI runs Rust containment plus the TypeScript failure/replacement path on Linux/macOS/Windows. The exact 30-second compile/load deadline remains protocol authority but is correctly owned by the later parent supervisor rather than falsely claimed child-local. See `evidence/phase-7-wave-2-slice-2b2.md`.
  - [x] Wave 2 child-runtime hostcall-fuel security correction: `P7-RUNTIME-SEC-001` found canonical ABI lifting could amplify 64/128-MiB guest strings/lists into native allocations before post-lift callback/output limits. Cross-platform calibration run `31008905408` rejected macOS `RLIMIT_AS`; every initial/replacement Store instead sets and reads back 4,464,640 bytes of Wasmtime 36.0.13 guest-to-host hostcall fuel before instantiation. All 11 imports/9 exports, maximum-valid/near-bound/oversized Rust lifts, the retained TypeScript bulk typed-array oversized-import argument, normalized failure, Store destruction, same-process replacement, empty diagnostics and optimized Linux evidence pass. Focused security recheck fixes `P7-PLAN-RUNTIME-001` and `P7-RUNTIME-SEC-001` and authorizes Slice 2C from that gate without changing WIT/WIT SHA, generated bodies, JBP1/JRI1, hashes, OpenAPI, dependencies or schema shape.
  - [x] Wave 2 Slice 2C planning corrections `P7-PLAN-2C-001`–`003`: the focused recheck closed the one-child/16-runtime and dual-admission topology, strict sibling discovery plus private protocol-v2/product-version handshake, and bounded real-owner Slice 2E harness findings.
  - [x] Wave 2 Slice 2C lifecycle planning recheck: the prior verdict at exact reviewed HEAD `442ebdd61e71a19a3e906b33b54bf7857ad555f8` remains **APPROVE** with no blockers for its reviewed authority; `P7-PLAN-2C-004` and `P7-PLAN-2C-004-EVENT-ATOMICITY` remain fixed and closed. Ordinary plugin-local trap/timeout/resource failure retains the current attempt epoch and replaces only that plugin's Store/instance. Child/session-fatal loss uses one operation identity and exactly one graph-fence AppService mutation/SQLite transaction/global revision/bounded `plugin.health_changed` event/durable receipt—not N envelopes—to advance every matching selected `starting` or `active` plugin exactly once and record only new-epoch `child-fatal`/`compile-load`, `child/session-lost`, or `dependency_failed` progression. Its one sorted unique max-16 canonical request/result list records exact epochs, health, cause, desired-enable outcome and dependency disposition; sorted `AffectedIds.plugin_ids` names every changed plugin. Whole-graph CAS, transitions, affected IDs, event and receipt commit or roll back together before replacement; exact replay returns the one receipt and a changed request conflicts. A later retry advances once again and retains its retry epoch through activation, so `starting` E→E+1 fencing followed by E+1→E+2 retry is intentional rather than double-counting.
  - [x] Wave 2 Slice 2C triggerless graph-fence plan correction: stable high finding `P7-PLAN-2C-005` is fixed by the written authority. A complete selected max-16 fence now has zero or exactly one `Failing`, never multiple. Triggered fences use exactly one correlated compile/load or child-fatal trigger, exact transitive selected skipped dependents and session-lost siblings; Hello/idle/uncorrelated session failures are triggerless and assign all selections session loss without false plugin blame. Empty selection remains `Dormant`, pre-byte nonempty failure still fences once, pre-connect session identity remains receipt authority, attributable source/package failures retain their authority, and all existing one-envelope/CAS/replay/progression rules remain. Focused planning recheck returned **APPROVE** at exact `f6183fd9ffdbe5879600cf7c071157e78e615ebb`; `P7-PLAN-2C-005` is fixed and closed.
  - [x] Wave 2 Slice 2C `P7-PLAN-2C-005` Packet B follow-up: commit `656e8431956dd3aebf7dc86029e5130beff930cd` makes request/receipt validators accept zero or exactly one `Failing` entry and reject multiple. Two focused tests and full storage validation passed. The focused database review returned **APPROVE** with no material issues after checking triggerless backup/restore/reopen, exact replay and fail-closed corruption handling. The supervisor may consume the API; no schema, DTO, event, OpenAPI or migration authority changed.
  - [x] Wave 2 Slice 2C Packet B lifecycle authority/database gate: typed due retry, activation completion, same-attempt failure and one-envelope graph fencing are integrated at `ae9cd24`. The focused database recheck approved exact `e573cfe` with no material issues; high findings `P7-2C-B-DB-001` and `P7-2C-B-DB-002` are fixed/closed. The accepted `656e843` follow-up adds the approved zero/exact-one triggerless authority without changing schema v7 or DTO/event/OpenAPI/migration contracts, and Packet C may consume these APIs.
  - [x] Wave 2 Slice 2C Packet A child and hostile-runtime/security review: private protocol v2/product/session framing, one Engine with at most 16 activation-fenced runtimes, independent per-plugin/four-total child admission, per-plugin Store/watchdog replacement, sibling survival, fatal cleanup and transfer/fuel/redaction authority are integrated at exact commit `651cf7530c951302be0e135143360706a42a9eac`. The completed single review inspected those boundaries and focused SDK/host/process tests and accepted that exact Packet A commit with no material security issues; no stable finding ID was created. Shared SDK framing codec commit `aa3e591` and parent-supervisor code are outside that exact review scope.
  - [x] Integrated Packet A/B exact-head remote gate: head `0f2f9b31633470dc75973fb15aef99ade354eafb` passed full CI run `31300532652` (Frontend/repository `93212480064`, release E2E `93212480099`, Rust `93212480101`, Rust supply chain `93212480103`, containment macOS `93212480105`, Windows `93212480137`, Ubuntu `93212480144`) and process-memory calibration run `31300530297` (Windows `93212474022`, Ubuntu `93212474036`, macOS `93212474038`). This closes only the former integrated Packet A/B CI pending statement; it does not establish a remote gate for the later Packet C1 head.
  - [x] Wave 2 Slice 2C Packet C shared-codec foundation: shared symmetric SDK framing codec extraction commit `aa3e591` is integrated and locally validated. It is not part of Packet A's exact review scope; the completed process-transport security review found no material codec issue, and exact pre-supervisor head `c9474bc` supplies a remote integration gate without expanding Packet A's focused review boundary.
  - [x] Wave 2 Slice 2C verified component-source port: commit `48ac139b0ad6109a1784d0ad2043916b80a16e00` is integrated and its focused security/integrity review verdict is **APPROVE** with no material issues. The parent supervisor may consume `AppService::open_plugin_component_sources`; static Linux/macOS/Windows path review and 13 focused source/package/normal-open/restore tests passed. The public hidden `OpenedPluginComponentSource::from_verified_package_file` constructor is trusted in-repository storage authority only and must not be called by the supervisor. No stable finding ID was created because the review found no issue.
  - [x] Wave 2 Slice 2C process-transport security review: initial transport at `cc23507` had no material codec issue. `P7-PC-SEC-001` remains rejected as a blocker because same-user Junban binary/filesystem control is outside the frozen threat model; supported Phase 8/9 packaging must verify that the server/host sibling directory is writable only by the installation owner or administrator/elevated authority, while a less-privileged writer is an unsupported insecure installation with fail/stop guidance. No descriptor-bound execution is claimed. Process corrections at exact `ddacfa2d65c4b4a373f8165f20033bad1ffbfc2f` passed focused security recheck with **APPROVE** and no material issues after all 18 focused tests passed; `P7-PC-SEC-002` and `P7-PC-SEC-003` are fixed and closed.
  - [x] Wave 2 Slice 2C process-owner correction: strict pre/post deadline checks and bounded 10-ms polling place send, acknowledgement, process exit and reader EOF under one 1,000-ms deadline; every failure kills, waits/reaps and joins, while success requires timely zero exit plus clean EOF. Trailing bytes, partial bodies, body-bearing terminal frames and malformed output are fatal. The focused recheck accepts exact `ddacfa2` for serialized supervisor consumption only.
  - [x] Wave 2 Slice 2C wakeable runtime driver and security correction: commit `6d0e94dc51cc9dae4ca835a5a3c2bbba71d4f200` adds bounded nonblocking runtime-frame/supervisor-command multiplexing. Initial medium finding `P7-WAKE-SEC-001` found forced or `Drop` closure could emit graceful. First fix `2ac281350aab551b1c1ab025bdea07d10baa4f08` remained open after a shutdown-overlap race; final correction `c9474bce9258dfe6072f2bc702f658d497be1be2` introduces monotonic `Open`→`GracefulInProgress`→`GracefulCommitted` CAS authority with forced CAS winners, a nonblocking admission critical section and RAII body-byte accounting. Focused security recheck returned **APPROVE** and fixed/closed the finding. All 35 focused tests passed in three full debug and one full optimized runs; paused-overlap passed 20 debug and 10 optimized repetitions. The driver is authorized for supervisor consumption. `P7-PC-SEC-002`/`003` and `P7-PLAN-2C-005` remain closed; `P7-PC-SEC-001` remains rejected under the explicit same-user/install-directory threat model.
  - [x] Wave 2 Slice 2C exact pre-supervisor driver remote gate: head `c9474bce9258dfe6072f2bc702f658d497be1be2` passed GitHub CI run `31310661503` with conclusion **SUCCESS**: Rust job `93237883593` **SUCCESS**; Release binary E2E job `93237883622` **SUCCESS**; Plugin host containment (windows-latest) job `93237883626` **SUCCESS**; Plugin host containment (ubuntu-latest) job `93237883627` **SUCCESS**; Frontend and repository job `93237883639` **SUCCESS**; Plugin host containment (macos-latest) job `93237883653` **SUCCESS**; and Rust supply chain job `93237883670` **SUCCESS**. Phase 7 process-memory calibration run `31310658785` also concluded **SUCCESS**: Calibrate plugin host (ubuntu-latest) job `93237877430` **SUCCESS**; Calibrate plugin host (macos-latest) job `93237877473` **SUCCESS**; and Calibrate plugin host (windows-latest) job `93237877480` **SUCCESS**. This proves integrated codec, verified source, process, storage and wakeable-driver behavior at the pre-supervisor boundary. The Rust job completed successfully beyond the former 20-minute interruption under the bounded 30-minute timeout, validating the `4fbd100` CI correction. This gate excludes the in-progress final supervisor, does not accept Slice 2C or Wave 2, does not close `P7-DEP-001`, and does not broaden any focused review verdict.
  - [x] Wave 2 Slice 2D planning gate: the initial exact-`08c42c5` review returned **REVISE** and closed only `P7-PLAN-2D-003`; the corrected authority then passed narrow focused API/planning recheck at exact `073f00d98dac4b9110ec028da01d0fb71eaa3ae3` with **APPROVED**, closing `P7-PLAN-2D-001`, `002`, and `004` and authorizing Slice 2D coding at that reviewed boundary.
  - [x] Wave 2 Slice 2D delivery persistence focused database recheck: exact `d641d69ac89436eb39624ca3eeca7dd6b90d795d` returned **APPROVED**. `P7-2D-DB-001` and `P7-2D-DB-003` are fixed/closed. The persistence part of `P7-2D-DB-002` is accepted, but the finding remains an open **REMOVAL BLOCKER** until server composition migrates and every legacy unwrapped API is removed.
  - [x] Wave 2 Slice 2D event converter focused API recheck: exact `ff76e01abec9fb7377210833a48e409d694b6b1b` returned **APPROVED**, closing `P7-API-2D-001`–`004` without accepting final callback composition.
  - [x] Wave 2 Slice 2D HTTPS focused security recheck: exact `71aad742cf33ab256a70142858b720146f7dc439` returned **APPROVED**, closing `P7-2D-SEC-001`. IPv6 answers are never usable; a request proceeds only through pinned validated public IPv4.
  - [x] `P7-PLAN-2D-005`/`P7-PLAN-2D-005-UUID` focused planning recheck **APPROVED** at exact `dbf63a58f38df245b0b62337b29c644b8054984c`, closing both and authorizing app/storage coding. Ordinary plugin query implementation at exact clean HEAD `dbb3700fa5459369633eb42b53723427244a6683` (implementation equivalent `aa31d9878937e4310752b0999aa222bec3a174f5`) then received focused database **APPROVE** with no material issues; callback composition may consume its APIs only with the required pre/post authority and grant checks and remains unaccepted.
  - [x] Wave 2 retention-loss correction: exact implementation `2b9172319a083ba0d8110f834d1fd7966b85a06f` adds only the receipt-backed nonserializable transition, passed focused database recheck with no material issue, and closes storage-side `P7-RLC-001`–`003`; exact current-session matching, graph-fence refresh, `P7-2D-DB-002`, callbacks, and Wave 2 acceptance remain composition work.
  - [x] Wave 2 callback/resync adapter security correction: exact `d58d33119ca3df456cdbd2c6ca15d07c4f326d21` passed focused security recheck and fixed/closed `P7-2D-ADAPTER-001`–`004`; callback tests 19/19 and resync tests 6/6 passed. This does not accept composition or Wave 2.
  - [x] Wave 2 transient-invocation planning correction: Activate/Deactivate/RenderSurface/ValidateSettings/CallService use one server-private nonserializable authority rather than relabelled durable deliveries. Exact frame/body, generation/session, grants, entry and nested ancestry are actor-owned; transient work creates no row/receipt/recovery and cannot use HTTP/effect/cursor/durable-KV paths. Focused API/planning recheck approved and fixed/closed `P7-PLAN-2D-006`/`P7-PLAN-2D-006-ACTOR` for planning only; implementation remains open.
  - [x] Wave 2 Slice 2C Packet C through Slice 2E and integrated acceptance: final supervisor security recheck, focused resync/database/callback/composition reviews, corrected zero-swap active-runtime evidence, and all named finding closures remain intact. Exact current-head CI run `31640217977` passed all ten jobs at `f6973c58d2309df7864a93f1f099d0f5cfbf5cdc`; the single final integrated security gate returned **APPROVE** with no material issue and accepts the supervisor, resync, Slices 2C/2D/2E, and Wave 2.
  - [x] Wave 3 server lifecycle/operator contract: product supervisor, signed registry, typed operator-only install/lifecycle/trust/grant/settings/contribution/command/action/event routes, exact idempotency/replay, and schema-v7 conformance with catalog 87 are implemented. Focused API-contract recheck accepts `P7-W3-API-001`–`004` as fixed. Final exact-head Phase 7 CI remains later.
  - [x] Wave 4 preserved Extensions/contribution UI: exact authority fences, scoped permission review, all plugin event invalidation, accessible uninstall confirmation, browser enable/disable, safe declarative contributions, plugin/frontend tests, TypeScript checking, and 13 immutable visuals pass. Focused frontend/accessibility recheck accepts `P7-W4-REV-001`–`005` as fixed; baseline PNGs are unchanged. Final exact-head Phase 7 CI remains later.
  - [x] Wave 5 protocol/recheck: [`evidence/phase-7-wave-5-protocol.md`](evidence/phase-7-wave-5-protocol.md) records SDK-owned typed source-manifest and permanent pack-sign-verify/public-checker tooling before ceremony, disposable-key preflight and public checks before one-time-key destruction, exact separate five-sample default/Rust/TypeScript corpora and byte budgets, and clean-candidate invalidation/single-commit/final-CI rules. `P7-W5-PLAN-001`–`003` are fixed and closed after the focused recheck approved the corrected protocol.
  - [x] Wave 5 implementation and Phase 7 closure: finalized signed references/registry, public-only checks, Linux/macOS/Windows reference and composition matrices, authoritative product dogfood, separate clean-candidate default/Rust/TypeScript reports, final security approval, docs/outcome, one squashed Phase commit, and exact-head final CI.
...
- Phase 7 Slice 2C planning review — prior **APPROVE; no blockers** at exact reviewed HEAD `442ebdd61e71a19a3e906b33b54bf7857ad555f8`. `P7-PLAN-2C-001`–`003` remain closed. `P7-PLAN-2C-004` closes same-attempt plugin-local failure versus child/session-fatal graph fencing and the separate retry increment; `P7-PLAN-2C-004-EVENT-ATOMICITY` closes the one-operation/transaction/revision/`plugin.health_changed` event/receipt envelope, bounded canonical per-plugin list, sorted affected IDs, atomic whole-graph CAS, replay and conflict rule. That approval and its evidence remain intact; the later focused recheck at exact `f6183fd9ffdbe5879600cf7c071157e78e615ebb` approved and closed `P7-PLAN-2C-005` as recorded next.
- `P7-PLAN-2C-005` — **high/fixed and closed** at exact `f6183fd9ffdbe5879600cf7c071157e78e615ebb`. The complete selected max-16 graph fence has exactly two shapes: one correlated trigger plus exact skipped transitive dependents and session-lost siblings, or zero-trigger all-session-lost fencing when Hello/idle/uncorrelated session failure authorizes no plugin identity. Multiple failing entries and false blame are forbidden; empty selection stays `Dormant`; pre-byte failure still advances a nonempty selected graph once; pre-connect session receipt identity, attributable package/source authority and the existing one-envelope/CAS/replay/progression rules remain. Packet B follow-up `656e8431956dd3aebf7dc86029e5130beff930cd` implements request/receipt zero-or-exactly-one validation and triggerless backup/restore/reopen, exact replay and corruption rejection; two focused tests and full storage validation passed, and focused database review returned **APPROVE** with no material issues. The supervisor may consume the API; no schema, DTO, event, OpenAPI or migration authority changed.
- `P7-PLAN-2D-001`–`004` — **high/fixed and closed**. The initial exact-`08c42c5` review closed HTTP finding `003` and required delivery-hash/recovery, transcript/KV, and resync-tail corrections for `001`, `002`, and `004`. Narrow Slice 2D planning recheck at exact `073f00d98dac4b9110ec028da01d0fb71eaa3ae3` returned **APPROVED**, closed all four findings, and authorized coding at that reviewed boundary.
- `P7-PLAN-2D-005` — **high/fixed and closed** by focused planning recheck **APPROVED** at exact `dbf63a58f38df245b0b62337b29c644b8054984c`; it authorizes app/storage coding only and does not accept callback composition.
- `P7-PLAN-2D-005-UUID` — **high/fixed and closed** by focused planning recheck **APPROVED** at exact `dbf63a58f38df245b0b62337b29c644b8054984c`. Exact UUID octets, nil/non-RFC/version semantics, fixed 122-byte layout, MAC, and error mapping are preserved.
- Slice 2D persistence recheck — **APPROVED** at exact `d641d69ac89436eb39624ca3eeca7dd6b90d795d`. `P7-2D-DB-001` and `003` are fixed/closed. `P7-2D-DB-002` accepts persistence but remains an open **REMOVAL BLOCKER** for server migration and deletion of every legacy unwrapped API.
- Slice 2D converter recheck — **APPROVED** at exact `ff76e01abec9fb7377210833a48e409d694b6b1b`; `P7-API-2D-001`–`004` are fixed/closed.
- Slice 2D HTTPS recheck — **APPROVED** at exact `71aad742cf33ab256a70142858b720146f7dc439`; `P7-2D-SEC-001` is fixed/closed, IPv6 is never usable, and only validated public IPv4 is pinned.
- Supervisor recheck — final supervisor security recheck **APPROVED** exact `768671ed07cc55df902d525d757d8fb0f1eb0308`, closing `P7-2C-SEC-004`/`P7-2C-TEST-001`. EOF correction `71856e5` resolved the second prior Rust race. Exact integrated CI run `31640217977` later passed at `f6973c58d2309df7864a93f1f099d0f5cfbf5cdc`; the final integrated security gate accepts the supervisor, Slice 2C, and Wave 2.
- `P7-W5-PLAN-001` — **high/fixed and closed** after focused recheck approval. The Wave 5 protocol requires SDK-owned typed source-manifest/runtime-manifest derivation and permanent package/index pack-sign-verify/public checking before ceremony, proves it first with disposable external keys, and requires public-only ceremony checks before one-time-key destruction. Rust owns runtime-manifest canonicalization; JRI1 has no second canonicalizer.
- `P7-W5-PLAN-002` — **high/fixed and closed** after focused recheck approval. The protocol freezes five matched samples and separate corpora: default Phase 1 workload; at-most-one-runtime Rust Pomodoro 100-round then cleanup and automation 100-event/effect/replay at 91,678,720/131,328,000 bytes; TypeScript bounded bulk command/action only, no events, at 692,418,560/806,824,960 bytes. Linux cgroup-v2 zero-swap/file-only-reclaim/no-runtime-Node provenance is mandatory; scale 4/16 is functional/informational and no latency ceiling is invented.
- `P7-W5-PLAN-003` — **high/fixed and closed** after focused recheck approval. The protocol requires all executable source/locks/generated contracts/signed artifacts/include table/protocol/checkers in a clean candidate before authoritative evidence, defines applicable reruns and bounded result-only append exceptions, then requires docs/review, one squashed Phase 7 commit, and exact-head final CI.
- Phase 7 hostcall-fuel security recheck — **approved after fixes**. The recheck closes `P7-PLAN-RUNTIME-001` and `P7-RUNTIME-SEC-001` at that historical Slice 2C boundary. The replacement-CI/macOS gate is closed at `ce80dea`, the Packet B database gate is approved at exact `e573cfe` and `656e843`, integrated Packet A/B full CI and process-memory calibration passed at exact `0f2f9b31633470dc75973fb15aef99ade354eafb`, and amended-head Slice 2E closes `P7-DEP-001`. Later exact-head CI and the integrated security gate accept the process owner, wakeable driver, final callback/server composition, Slices 2C/2D/2E, and Wave 2.
- Phase 7 Packet A hostile-runtime/security review — **accepted** at exact commit `651cf7530c951302be0e135143360706a42a9eac`. The completed single review found no material security issues after inspecting protocol-v2/product/session framing, the one-Engine bounded-16 coordinator, child one-per-plugin/four-total admission, per-plugin Store/watchdog replacement, sibling survival, fatal cleanup, transfer/fuel/redaction authority, and focused SDK/host/process tests. No stable finding ID was created. This verdict does not cover shared SDK framing codec commit `aa3e591` or parent-supervisor code; the codec is integrated and locally validated as a Packet C foundation, the later process-transport review found no material codec issue, and exact pre-supervisor head `c9474bc` passed a remote integration gate without expanding the focused review. Packet C foundations are underway, not complete.
- Phase 7 verified component-source security/integrity review — **APPROVE; no material issues** at exact commit `48ac139b0ad6109a1784d0ad2043916b80a16e00`. Static Linux/macOS/Windows path review and 13 focused source/package/normal-open/restore tests passed, so the parent supervisor may consume `AppService::open_plugin_component_sources`. The public hidden constructor remains trusted in-repository only and must not be called by the supervisor. No stable finding ID was created because no issue was found. That source-port verdict did not itself accept the strict process owner, supervisor, Slice 2C or Wave 2; the independent process-owner gate later accepted exact `ddacfa2` for serialized supervisor consumption only.
- Phase 7 process-transport security review — initial transport `cc23507` had no material codec issue. `P7-PC-SEC-001` remains **high/rejected**, not fixed: same-user control of the Junban binary/filesystem account is outside the explicit threat model, so supported packaging instead must verify the server/host sibling directory is writable only by the installation owner or administrator/elevated authority; a less-privileged writer is an unsupported insecure installation with fail/stop guidance, not an in-scope attacker, and descriptor-bound execution is not claimed. Corrections at exact `ddacfa2d65c4b4a373f8165f20033bad1ffbfc2f` passed focused security recheck with **APPROVE**, no material issues and all 18 tests. Strict pre/post deadline checks and bounded 10-ms polling place send/acknowledgement/exit/EOF under one 1,000-ms deadline; every failure kills, waits/reaps and joins; success requires timely zero exit plus clean EOF; and trailing, partial-body, body-bearing and malformed output are fatal. `P7-PC-SEC-002` and `P7-PC-SEC-003` remain **medium/fixed and closed**, and the process owner is accepted for serialized supervisor consumption.
- `P7-WAKE-SEC-001` — **medium/fixed and closed** at exact `c9474bce9258dfe6072f2bc702f658d497be1be2`. Initial wakeable driver `6d0e94d` allowed forced or `Drop` closure to emit graceful. First fix `2ac2813` remained open after a shutdown-overlap race. Final monotonic `Open`→`GracefulInProgress`→`GracefulCommitted` CAS authority lets forced states win from open/in-progress without rewriting committed terminals, keeps admission in a nonblocking critical section, and uses RAII body-byte accounting. Focused security recheck returned **APPROVE** after all 35 focused tests passed in three full debug and one full optimized runs plus 20 debug and 10 optimized paused-overlap repetitions. The wakeable driver is authorized for supervisor consumption; exact driver head `c9474bc` passed its pre-supervisor remote gate, while final supervisor-packet implementation remains in progress but not accepted or composed and final-supervisor current-head CI is not claimed.
...
- 2026-08-05: Exact clean-commit run `31008905408` passed one warm-up plus five optimized runs per Rust/TypeScript profile on Linux, macOS and Windows. Linux virtual-address and Windows private-commit ranges separate valid workloads from a candidate cap, but macOS reports roughly 415 GiB valid VSIZE, making its mechanical 519-GiB `RLIMIT_AS` minimum ineffective and rejecting that remedy. No cap was frozen at that checkpoint; the process-memory planning blocker awaited the narrower correction.
- 2026-08-05: The approved narrower correction uses Wasmtime 36.0.13 guest-to-host hostcall fuel rather than a hard process cap. The smallest audited authority is 4,464,640 bytes: 4-MiB private callback body + 139,264-byte maximum valid nested ABI structure + 128-KiB margin. Every initial/replacement Store sets and reads it back before instantiation; all 11 imports/9 exports, valid/near-bound/oversized Rust lifts, the retained TypeScript bulk typed-array oversized-import argument with no capability request published, normalized failure, Store destruction/replacement, diagnostics and five-run optimized Linux exact-child peaks are covered. The focused security recheck fixed `P7-PLAN-RUNTIME-001` and `P7-RUNTIME-SEC-001` and authorized Slice 2C from that gate; `P7-DEP-001` remained open at that checkpoint and closed after Slice 2E.
- 2026-08-09: Slice 2C planning retains exactly one product child and rejects per-plugin processes. One Engine owns at most 16 serialized activation-fenced runtimes; only the exact strict `current_exe()` sibling may execute; private protocol v2 binds exact product version before bytes; and SQLite/AppService is the sole durable lifecycle authority. One non-shipped Slice 2E real-owner harness may replace runtime evidence but cannot claim Wave 3 ordinary composition or Wave 5 integration. The focused planning verdict at exact reviewed HEAD `442ebdd61e71a19a3e906b33b54bf7857ad555f8` is **APPROVE** with no blockers. `P7-PLAN-2C-001`–`003` remain closed; `P7-PLAN-2C-004` and `P7-PLAN-2C-004-EVENT-ATOMICITY` are fixed and closed. Their rule separates same-attempt plugin-local failure from one child/session-fatal graph fence that uses one operation, transaction, revision, bounded `plugin.health_changed` event and receipt—not N envelopes—and from a later independently incremented retry. Focused planning recheck at exact `f6183fd9ffdbe5879600cf7c071157e78e615ebb` subsequently approved, fixed and closed `P7-PLAN-2C-005`; its Packet B follow-up is approved at exact `656e8431956dd3aebf7dc86029e5130beff930cd`. These gates do not accept Wave 2 or close `P7-DEP-001`.
- 2026-08-09: Replacement CI run `31296663305` at exact head `ce80dead79c5ec2dadd95bddb8202ee7dbd2bf46` passed every Rust, Rust supply-chain, frontend/repository, release E2E, Ubuntu containment, Windows containment and macOS containment job; process-memory calibration run `31296660062` at the same exact head passed. The macOS containment job passed original execution `93202756069` and strict exact-head reruns `93205509663` and `93207040552`. The old macOS blocker and replacement-CI gate are closed at `ce80dea`, but the runs predate integrated Packet B `ae9cd24`, Packet A `651cf75` and database corrections `e573cfe`; at that checkpoint, integrated-head CI green and Wave 2 acceptance were not claimed.
- 2026-08-09: Slice 2C Packet B lifecycle authority is integrated at `ae9cd24`; its focused database recheck approved exact `e573cfe` with no material issues, fixed/closed high findings `P7-2C-B-DB-001` and `P7-2C-B-DB-002`, left schema v7 unchanged, and authorized Packet C to consume those APIs. The completed single hostile-runtime/security review accepted Packet A protocol-v2/multi-runtime child authority at exact commit `651cf7530c951302be0e135143360706a42a9eac` with no material security issues and without a stable finding ID. The verdict is exact-commit bounded and does not cover later codec/supervisor code. Packet C remained unaccepted at that checkpoint; `P7-DEP-001` closed only after Slice 2E.
- 2026-08-09: Integrated Packet A/B head `0f2f9b31633470dc75973fb15aef99ade354eafb` passed full CI run `31300532652`: Frontend/repository `93212480064`, release E2E `93212480099`, Rust `93212480101`, Rust supply chain `93212480103`, containment macOS `93212480105`, Windows `93212480137`, and Ubuntu `93212480144`. Process-memory calibration run `31300530297` passed Windows `93212474022`, Ubuntu `93212474036`, and macOS `93212474038`. This closes the former integrated Packet A/B CI pending statement at exact `0f2` only, not later Packet C1 current-head CI.
- 2026-08-09: Shared symmetric SDK framing codec extraction commit `aa3e591` is integrated and locally validated as a Packet C foundation and remains outside Packet A's exact review scope. The later process-transport review found no material codec issue, and exact pre-supervisor head `c9474bc` supplies a remote integration gate without expanding Packet A's focused review.
- 2026-08-09: Verified component-source port commit `48ac139b0ad6109a1784d0ad2043916b80a16e00` is integrated. Its focused security/integrity review verdict is **APPROVE** with no material issues; static Linux/macOS/Windows path review and 13 focused source/package/normal-open/restore tests passed, and the parent supervisor may consume `AppService::open_plugin_component_sources`. The public hidden constructor is trusted in-repository only and must not be called by the supervisor. No stable finding ID was created.
- 2026-08-09: `P7-PLAN-2C-005` freezes triggerless graph fencing: complete selected max-16 fences have zero or exactly one `Failing`; only exact plugin correlation authorizes a trigger, while Hello/idle/uncorrelated loss assigns every selection `LoadedSibling` + `SessionLost`. Focused planning recheck returned **APPROVE** at exact `f6183fd9ffdbe5879600cf7c071157e78e615ebb`, fixing and closing the finding. Packet B follow-up `656e8431956dd3aebf7dc86029e5130beff930cd` implements zero-or-exactly-one validation and triggerless backup/restore/reopen, exact replay and corruption rejection; two focused tests and full storage validation passed, focused database review returned **APPROVE** with no material issues, and the supervisor may consume the API. Schema, DTO, event, OpenAPI and migration authority are unchanged.
- 2026-08-09: Initial process transport commit `cc23507` completed focused security review with no material codec issue. `P7-PC-SEC-001` remains rejected under the explicit same-user/install-directory threat-model exclusion; supported Phase 8/9 packaging must verify owner/mode/ACL on the server/host sibling directory, and a less-privileged-writable installation is unsupported/insecure with fail/stop guidance. No descriptor-bound execution is claimed. Corrections at exact `ddacfa2d65c4b4a373f8165f20033bad1ffbfc2f` passed focused security recheck with **APPROVE**, no material issues and 18 tests; strict pre/post checks, bounded 10-ms polling and one 1,000-ms send/ack/exit/EOF deadline now require timely zero exit plus clean EOF, make all trailing/partial/body/malformed output fatal, and kill/wait/reap/join every failure. `P7-PC-SEC-002`/`003` remain fixed and closed, and the process owner is accepted for serialized supervisor consumption.
- 2026-08-09: Wakeable driver `6d0e94d` received initial medium `P7-WAKE-SEC-001` because forced or `Drop` closure could emit graceful. First fix `2ac2813` remained open after a shutdown-overlap race. Final correction `c9474bc` adds monotonic `Open`→`GracefulInProgress`→`GracefulCommitted` CAS authority with forced CAS winners, a nonblocking admission critical section and RAII body-byte accounting. Focused security recheck returned **APPROVE** and closed the finding after all 35 focused tests passed in three full debug and one full optimized runs, plus 20 debug and 10 optimized paused-overlap repetitions. The driver is authorized for supervisor consumption. Final supervisor-packet implementation is in progress but not accepted or composed; Slice 2C and Wave 2 remain open.
- 2026-08-09: CI run `31303724227` at exact `2c8affb028af27372437be0f8827f0f375029e16` passed every non-Rust job. The Rust job completed workspace tests and release-binary compilation before GitHub cancelled it at the workflow's 20-minute job limit prior to step completion; this was a CI time-budget interruption, not a product or test failure. Commit `4fbd1001a3496d8012f8e45586d478b033d38a68` raises only the Rust job timeout to a bounded 30 minutes. Accepted CI at `0f2f9b31633470dc75973fb15aef99ade354eafb` remains intact.
- 2026-08-09: Exact pre-supervisor driver head `c9474bce9258dfe6072f2bc702f658d497be1be2` passed CI run `31310661503` and Phase 7 process-memory calibration run `31310658785`. This proves codec/source/process/storage/driver integration and the 30-minute Rust timeout correction at that exact boundary only. It excludes the final supervisor, does not accept Slice 2C or Wave 2, and did not by itself close `P7-DEP-001` or broaden focused review boundaries. Exact-current-head integrated CI remains pending.
- 2026-08-12: Slice 2E corrected its active-memory authority after review found proactive reclaim had swapped anonymous pages. Exact `e196313b5a463681254a2401ebc9787f99e97e13` sets zero swap, uses file-only `swappiness=0` reclaim, completes representative warm work, and binds failed evidence to run `31629776892` plus immutable release asset SHA-256 `75b5336589f67fb9106cbda4a7d720517434ab75224b660015dfe646623500e5`. `P7-2E-PERF-001`/`P7-2E-EVID-001` are closed. The corrected normalized scale-1 maxima disproved Wave 0 projections, so the original headroom rule formally amends active current/peak gates to Rust 91,678,720/131,328,000 bytes and TypeScript 692,418,560/806,824,960 bytes. Default 24/32 MiB remains unchanged. Exact amended-head run `31634972732` then passed at `67a7f89e1b72676b7ab0d36b050358f16d401da4`; immutable release asset SHA-256 is `9fb264569f47c10b3338728707aa94a9c47d11bd1d70acf6b7cffd4174aa6982`. Focused final recheck reported no material finding, closing `P7-2E-PERF-002` and `P7-DEP-001` and accepting Slice 2E active-runtime evidence.
- 2026-08-12: Focused final database/composition recheck **APPROVED** exact `e56674f9879ea72412e567369dc72c7a63e44461` with no material finding, closes `P7-2D-DB-002`, and accepts Slice 2D callback composition from the database boundary. Exact current-head CI run `31640217977` passed all ten jobs at `f6973c58d2309df7864a93f1f099d0f5cfbf5cdc`. The single final integrated security gate returned **APPROVE** with no material issue and accepts the supervisor, resync, Slices 2C/2D/2E, and Wave 2. Wave 3+ and final Phase 7 remain outside this gate.
- 2026-08-12: Wave 3 focused API-contract recheck accepts `P7-W3-API-001`–`004` as fixed across operator auth/body ordering, concrete typed contracts, exact mutation idempotency/replay, signed registry/lifecycle/contribution/events, and unchanged catalog 87. Wave 4 focused frontend/accessibility recheck accepts `P7-W4-REV-001`–`005` as fixed across command generation/epoch/session fences, exact scoped permissions, every plugin event, accessible uninstall confirmation, and browser enable/disable. Plugin/frontend tests, TypeScript checking, and all 13 immutable visuals pass without baseline PNG changes; final exact-head Phase 7 CI remains later.
- 2026-08-12: Wave 5 planning review returned **REVISE** for `P7-W5-PLAN-001`–`003`. The corrected implementation-ready protocol freezes SDK-owned source-manifest/package/index tooling and permanent public checking before ceremony, disposable external-key preflight plus public checks before one-time-key destruction, package-specific five-sample zero-swap corpora/budgets, and clean-candidate invalidation/result-only append/single-squash/final-CI sequencing. This records the initial verdict; the focused approval below supersedes its pending status.
- 2026-08-12: Focused Wave 5 planning recheck returned **APPROVE** with no blockers, fixing and closing `P7-W5-PLAN-001`–`003`. Wave 5 implementation/evidence and Phase 7 closure remained open at that checkpoint.
- 2026-08-14: Wave 5 finalized three signed offline references plus the JRI1 registry, SDK authoring examples/tooling, permanent public verification, optimized Linux/macOS/Windows reference matrices, product dogfood, and separate five-sample default/Rust/TypeScript reports. Clean candidate `7ac35e3588a0521671de19ac16bfa4ffc4ddd9f7` passed CI run `31770558122` and performance run `31770557554`; all memory gates, zero-swap/no-Node/cleanup assertions, and immutable visuals passed.
- 2026-08-14: Final integrated security review found `P7-FINAL-SEC-001`/`002`; exact correction `9fc7143c9486f56d6bf7893b84ae0293ac6d1b40` serialized terminal publication against cancellation and replaced a private-body decode panic with typed stale authority. Focused recheck returned **APPROVED**; API-contract and docs closure reviews found no additional material issue. The final `7ac35e3588a0521671de19ac16bfa4ffc4ddd9f7` candidate replaced a timeout-sensitive late-cancel probe with a deterministic unload ordering barrier and removed a duplicate manual workflow entry without changing shipped runtime behavior. Result-only evidence/docs then completed the one-squash exact-head publication gate and Phase 7 acceptance.
- 2026-08-09: Initial Slice 2D authority-only correction at exact `08c42c5b44411fdb4b81545c993a60a80960682d` received focused API/planning **REVISE**. The review accepted and fixed/closed HTTP `P7-PLAN-2D-003`, but kept `P7-PLAN-2D-001`, `002`, and `004` open for complete request hashing/process-loss, transcript/KV serialization, and concurrent resync-tail coverage.
- 2026-08-09: Narrow Slice 2D planning recheck at exact `073f00d98dac4b9110ec028da01d0fb71eaa3ae3` returned **APPROVED**, closing `P7-PLAN-2D-001`–`004` and authorizing coding. Subsequent focused rechecks approved delivery persistence at `d641d69` (`P7-2D-DB-001`/`003` closed; `002` remains a server migration/removal blocker), retained-event conversion at `ff76e01` (`P7-API-2D-001`–`004` closed), and public-IPv4-only HTTPS at `71aad74` (`P7-2D-SEC-001` closed).
- 2026-08-09: Final supervisor security recheck **APPROVED** exact `768671ed07cc55df902d525d757d8fb0f1eb0308`, closing `P7-2C-SEC-004`/`P7-2C-TEST-001` for coding security only. EOF correction `71856e5` resolves the second prior Rust race; no exact-head CI rerun exists, and no acceptance boundary expands.
- 2026-08-09: Focused query planning recheck **APPROVED** exact `dbf63a58f38df245b0b62337b29c644b8054984c`, closing `P7-PLAN-2D-005`/`P7-PLAN-2D-005-UUID` and authorizing app/storage coding only. It preserves exact UUID octet/nil/non-RFC/version semantics, fixed 122-byte layout, MAC, and error mapping; callback composition remains unaccepted.
...
- 2026-08-10: Slice 2D composition mapping found that represented-event terminal and verified-skip authorities could not express actual retained-tail loss after the legacy unwrapped cursor mutation is removed. The approved correction adds one receipt-backed internal retention-loss transition with SQLite-proven gap, exact operation/session digest replay, scoped ambiguity carry-forward, and refreshed whole-graph fatal-fence authority. Focused planning recheck fixed/closed `P7-RLC-001`–`003`; implementation and `P7-2D-DB-002` remain open.
- 2026-08-10: Focused transient-invocation review found five valid SDK exports had no correct authority and that the decoded-only callback seam dropped actor-owned frame/body/callback state. The approved server-private transient authority and actor-only bridge fix/close `P7-PLAN-2D-006`/`P7-PLAN-2D-006-ACTOR` for planning without changing durable delivery, schema, WIT, app/storage or public contracts; implementation remains open.
- 2026-08-10: Retention-loss implementation `2b91723` passed focused database recheck with no material issue; callback/resync adapter correction `d58d331` passed focused security delta recheck and fixed/closed `P7-2D-ADAPTER-001`–`004`. Integrated full server 381/381, process lifecycle 6/6, focused app plugin 42/42, storage plugin 100/100, callback 19/19, resync 6/6, workspace Clippy, format, and diff checks passed. Callback composition and `P7-2D-DB-002` remain open.
...
- **Outcome:** signed JBP1 Component Model plugins run in one adjacent on-demand Rust child per active profile, with schema-v7 authority, bounded typed capabilities, generation/epoch/session fencing, operator-only lifecycle APIs, declarative host-rendered contributions, the preserved Extensions UI, and three offline signed reference plugins. Ordinary startup links no Wasmtime and launches no host or Node process.
- **Evidence:** `goals/rust-rewrite/evidence/phase-7-outcome.md`; `phase-7-dogfood.json`; separate `phase-7-default-benchmark.json`, `phase-7-rust-benchmark.json`, and `phase-7-typescript-benchmark.json`; Linux/macOS/Windows reference-matrix and Slice 2E result JSON; `phase-7-review-ledger.md`; thirteen immutable legacy visual authorities.
- **Performance:** five-sample authoritative default evidence passed at 5,484,544-byte median / 5,541,888-byte maximum warm and 8,220,672-byte maximum peak; Rust passed at 29,319,168/32,632,832 bytes maximum current/peak against 91,678,720/131,328,000; TypeScript passed at 611,205,120/715,870,208 bytes against 692,418,560/806,824,960. Every sample had zero swap, file-only reclaim, exact process composition, no runtime Node, and complete cleanup.
- **Validation:** workspace format, denied-warning Clippy, all-feature and no-default-feature Rust tests, frontend checks/build, artifact/registry/OpenAPI/catalog checks, author SDK checks, root supply-chain checks, all 13 immutable visuals, optimized product dogfood, Linux cgroup evidence, and Linux/macOS/Windows containment/composition/reference jobs passed.
- **Review:** all package, persistence, runtime, callback, API, UI, evidence, and final security findings are fixed/closed or explicitly disposed. `P7-FINAL-SEC-001`/`002` close terminal/cancel publication and private-body decode-panic races; focused recheck approved the final delta.
- **Follow-up:** Phase 8/9 packaging must enforce the adjacent-host install-directory owner/mode/ACL expectation. Phase 8 adds desktop packaging over the accepted shared core.
