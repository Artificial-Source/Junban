...
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Contract remains `evidence/phase-7-context-map.md`. Wave 0/1 planning, schema, package, WIT and host-placement gates are approved. All Slice 2C planning findings through `P7-PLAN-2C-005` are approved/fixed/closed, and the Packet B zero-or-exactly-one follow-up is approved for supervisor consumption. The process owner at exact `ddacfa2` and wakeable runtime driver at exact `c9474bc` are accepted for supervisor consumption. Exact pre-supervisor head `c9474bc` has passed CI and process-memory calibration. Final supervisor-packet implementation, final-supervisor current-head CI, Slice 2C/Wave 2 acceptance and `P7-DEP-001` remain open. Focused Slice 2D review at exact `08c42c5b44411fdb4b81545c993a60a80960682d` fixed/closed `P7-PLAN-2D-003` and returned **REVISE** for the now-corrected `P7-PLAN-2D-001`, `002`, and `004`; those three await a narrow recheck, and no Slice 2D coding is authorized.
  ...
  - [x] Wave 2 Slice 2D initial authority-only planning correction and review: focused API/planning review at exact `08c42c5b44411fdb4b81545c993a60a80960682d` returned **REVISE** overall. It accepted exact HTTPS metadata/cardinality/ambiguity and fixed/closed `P7-PLAN-2D-003`; it left `P7-PLAN-2D-001`, `002`, and `004` open for narrow correction.
  - [x] Wave 2 Slice 2D REVISE correction: `P7-PLAN-2D-001` now freezes existing delivery digest plus exact command/action, retained-event, and resync payload digests; v2 hook-tag/persisted-entry-ID/authority final request hash; received-body canonical equality; and process-loss abandonment of every Starting/Active non-HTTP row with only HTTP dispatch ambiguity surviving. `P7-PLAN-2D-002` freezes exact transcript initial and step domains/`u32be` index/tag/lengths/request/outcome bytes, SET-only globally sorted/unique empty-based replacement maps, omission deletion, leave/replace/zero-replace, and one final transaction. `P7-PLAN-2D-004` freezes subscription+affected-ID `Represented`/`Irrelevant`/`Invalidating` tail coverage, never-skip restart on invalidation, and catch-up-to-head before activation. Existing service depth remains. No WIT/WIT SHA/generated body, JBP1/JRI1/package hash, OpenAPI/DTO, schema SQL/version/table shape, or dependency shape changes.
  - [ ] Wave 2 Slice 2D narrow focused API/planning recheck gate: one focused reviewer must approve corrected `P7-PLAN-2D-001`, `002`, and `004` and their exact named hash/recovery, transcript/KV, and concurrent resync/catch-up tests. Those three remain fixed-in-plan/narrow-recheck-pending; `P7-PLAN-2D-003` remains fixed/closed. Slice 2D coding remains blocked. The final Slice 2C supervisor packet remains independently in progress, unaccepted and uncomposed.
  - [ ] Wave 2 Slice 2C Packet C through Slice 2E: Packet C, Slice 2C and Wave 2 remain unaccepted and incomplete. Final supervisor-packet implementation is now in progress but is not accepted or composed. Parent supervision and compile/load graph-fence wiring, the pending narrow Slice 2D recheck for `P7-PLAN-2D-001`, `002`, and `004` plus later capability/effect/dependency/HTTP/event implementation, final-supervisor current-head CI, integrated security review and optimized replacement measurements remain. The exact 1,000-ms parent control deadline remains frozen at docs commit `18fbb87`. Slice 2E may use one non-shipped real-supervisor/storage/AppService/sibling-host harness for separate Rust/TypeScript and one/four/sixteen-runtime replacement evidence only; Wave 3 ordinary `ServerState` composition and Wave 5 product-integrated evidence remain separate. Replacement CI run `31296663305` and process-memory calibration run `31296660062` passed at exact head `ce80dead79c5ec2dadd95bddb8202ee7dbd2bf46`; all three strict exact-head macOS containment executions (`93202756069`, `93205509663`, `93207040552`) passed. Integrated Packet A/B CI and process-memory calibration passed later at accepted exact `0f2f9b31633470dc75973fb15aef99ade354eafb`. CI run `31303724227` at exact `2c8affb028af27372437be0f8827f0f375029e16` passed every non-Rust job; its Rust job completed workspace tests and release-binary compilation before GitHub cancelled it at the former 20-minute job limit prior to step completion. This is a CI time-budget interruption, not a product or test failure. Commit `4fbd1001a3496d8012f8e45586d478b033d38a68` raises only the Rust job timeout to a bounded 30 minutes. Exact pre-supervisor head `c9474bc` later passed its remote gate; no final-supervisor current-head CI, Slice 2C acceptance or Wave 2 acceptance is claimed. `P7-DEP-001` remains open.
...
- `P7-PLAN-2D-001` — **high/fixed in plan; narrow recheck pending after REVISE**. Review at exact `08c42c5b44411fdb4b81545c993a60a80960682d` found incomplete payload/final request hashing and Active process-loss recovery. The revision keeps the exact delivery digest, defines all hook payload digests and canonical-body equality, freezes the exact v2 hook-tag/persisted-entry-ID/authority final hash, and abandons every Starting/Active non-HTTP row on process/session loss while only HTTP dispatch becomes/stays ambiguous. Exact goldens/one-field/body-mismatch/recovery tests and narrow approval remain required.
- `P7-PLAN-2D-002` — **high/fixed in plan; narrow recheck pending after REVISE**. The revision freezes exact transcript initialization and each step's domain/`u32be` index/tag/length/request/outcome bytes after canonical equality; keeps compact counters and mandatory Task→Project→Tag/flush/finalize rules; rejects resync delete; and hashes the SET-only globally sorted/unique empty-based resulting map with exact omission/leave/replace/zero-replace behavior in one final transaction. Exact serialization/KV/crash tests and narrow approval remain required.
- `P7-PLAN-2D-003` — **high/fixed and closed** by focused review at exact `08c42c5b44411fdb4b81545c993a60a80960682d`. HTTPS exact request/response allowlists and bounds, forbidden/transport-owned headers, invalid-versus-omitted metadata, consume-once durable pre-send callback, identical same-memory ambiguity resend, and unresolved process loss were accepted. Required tests remain retained.
- `P7-PLAN-2D-004` — **high/fixed in plan; narrow recheck pending after REVISE**. The revision retains exhaustive direct conversion, malformed fail-closed, no live synthesis, atomic terminal+cursor, and service-depth rules while adding exact subscription+affected-ID `Represented`/`Irrelevant`/`Invalidating` classification. Invalidating resync tails/catch-up are never skipped and force fresh-head restart, covering Task revision and Project/Tag anti-join omissions. Exact named concurrent mutation/restart-then-complete tests and narrow approval remain required.
...
- 2026-08-09: Slice 2D REVISE correction now freezes those exact missing authorities and named regressions in the same seven Phase 7 documents without code/schema/WIT/JBP/OpenAPI/dependency changes. `P7-PLAN-2D-001`, `002`, and `004` are fixed in plan pending one narrow focused recheck; `P7-PLAN-2D-003` remains fixed/closed. Slice 2D coding remains blocked; final Slice 2C supervisor work remains independently in progress and unaccepted.
