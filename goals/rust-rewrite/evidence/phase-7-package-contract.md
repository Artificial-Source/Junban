...
An HTTP origin is canonical `https://host[:nondefault-port]` only: no userinfo, path other than `/`, query, fragment, wildcard, IP literal, mixed-script/Unicode host, or default port spelling. Loopback/private/link-local origins are rejected for community packages; deterministic hostile tests use an explicit test-only authority that cannot ship. DNS resolution and every connection hop are checked against the approved public origin/address policy; redirects are disabled.

Guest request headers are limited to exactly `accept`, `accept-language`, `content-type`, `if-match`, and `if-none-match`. Guest-visible response headers are limited to exactly `cache-control`, `content-language`, `content-type`, `etag`, `expires`, `last-modified`, `location`, and `retry-after`; every other valid response header is omitted. A name is an exact lowercase ASCII HTTP token of 1–64 bytes and lists are sorted and unique—case, order, duplicates, whitespace, or other spelling errors are rejected rather than normalized. A value is at most 8 KiB of visible ASCII plus internal HTAB and has no edge SP/HTAB, CR, LF, NUL, or other control. Each request or exposed response has at most 32 entries and at most 64 KiB aggregate name-plus-value bytes.

`authorization`, proxy authorization, cookies and set-cookie, `host`, `content-length`, forwarding/proxy headers, hop-by-hop headers, and every `x-junban-*` header are always forbidden. The dedicated transport owns Host/content length, injects exactly one `x-junban-plugin-delivery-id`, forces `accept-encoding: identity`, and disables environment/system proxies, redirects, automatic credentials/cookie storage or sending, and transport retries. Duplicate/invalid allowlisted response names or allowlisted/raw response metadata over any name/value/count/aggregate bound returns typed `invalid-response`; nonallowlisted valid response headers are omitted.

The dedicated client resolves every request origin immediately before connect, rejects the request if **any** answer is not globally routable, and pins the connection to one validated answer while preserving the canonical hostname for URL authority, TLS certificate verification/SNI, and transport-owned Host. Request bodies over 1 MiB fail before send. Responses return at most the first 1 MiB and set the typed WIT `truncated` flag when another byte exists; body oversize is never alternatively reported as an error. Post-send uncertainty uses the typed `may-have-been-sent` delivery state.

Each top-level command, subscribed event, or surface action has a consume-once parent permit and can issue zero or one logical HTTP callback. Durable `DispatchingHttp` precedes every send, and a second callback never reaches transport. A same-process ambiguous resend uses only the identical in-memory method, URL, sorted headers, body, and delivery ID without rerunning the guest; it performs fresh all-answer DNS validation and a new pinned connection. Process loss leaves the dispatch unresolved: no request is reconstructed, no guest is rerun, and success is never guessed. This runtime rule changes no manifest permission or package bytes.
