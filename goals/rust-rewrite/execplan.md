...
- [x] Phase 7 Wave 0 host-placement spike (preliminary): temporary `tools/phase7-host-placement` probes for protocol-only / lazy in-process / on-demand child, Rust + pure TypeScript components, trap/CPU/memory/cleanup containment, cgroup self-check + quick evidence, and preliminary ADR preferring on-demand child by fault-containment tiebreak. Exact Wasmtime 45.0.3; ordinary `junban-server` does not link Wasmtime. Evidence is **not** authoritative (quick/contended). See `evidence/phase-7-host-placement-adr.md` and `evidence/phase-7-host-placement-quick.json`. Wave 1 remains blocked on five-sample idle-host evidence + architecture gate.
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Contract remains `evidence/phase-7-context-map.md`. High-risk planning gate approved Wave 0 after `P7-PLAN-001`–`003`; Wave 1 blocked on measured host-placement architecture gate.
