...
- `P1-FINAL-LIFE-001` — **fixed**. SSE `forward_events` tasks now select on stream receiver closure, process shutdown cancellation, and broadcast work; `main` cancels a process-wide `CancellationToken` when shutdown is requested before Axum drains in-flight responses; concurrent authenticated SSE connections are hard-capped at 64 per process with retryable `503 sse_connection_limit`. Dropped bodies and SIGINT/SIGTERM with an open SSE stream release forwarders and profile locks.
