...
Date: 2026-08-04
Status: Wave 1 production authority; exact `.wit`, generated Rust/TypeScript bindings, source consumers, and retained inspected components are checked in; Slice 2D authority clarifications are fixed in plan under `P7-PLAN-2D-001`–`004` but pending focused API/planning recheck and make no WIT or generated-body shape change
...
The parent additionally binds every dispatch to one runtime-local, nonserialized `PluginDeliveryAuthority`: exact plugin ID, package generation, activation epoch, current host session, invocation ID, payload SHA-256, and closed internal mode `StartingResync | StartingCatchUp | Active`. The domain-separated canonical authority hash is incorporated into the existing invocation `request_hash` and must exact-match at reserve, transition, terminal, verified skip, and resync finalization. `StartingResync` can call only `resync`. `StartingCatchUp` can call only retained `handle-event` and cannot authorize command/action/HTTP/domain/KV/arbitrary-cursor work; it must reach the sampled head before same-epoch activation. These modes are parent runtime authority, not new WIT variants. Process loss abandons exact starting-mode `reserved`/`effect_committing` work and starts a fresh session; it never converts starting work into HTTP ambiguity.
...
Task moved/reordered/bulk/restored, undo/import, plugin, AI, settings, templates, comments, relations, time resources, every other/future revision, and an unsubscribed direct kind are cursor-only skips. A malformed direct event fails closed. Reservation binds the exact retained event hash, revision, and expected source cursor. A bounded skip verifier stops immediately before the first subscribed direct event. The accepted domain effect, KV patch, or HTTP terminal result and exact source cursor commit in one transaction where the delivery mode permits that result. Event envelopes still contain no receipt response or arbitrary guest event JSON.
...
2. A bounded nonpersisted `PluginResyncTranscript` uses a domain-separated rolling hash over the exact fence/session and every canonical request/outcome. It retains only compact page/item/byte counts, last IDs, per-page digests, flush/finalize state, and staged-KV count/bytes/digest.
...
5. Every snapshot outcome acknowledges only its exact page and may add one sorted unique KV segment whose full lifted outcome is ≤256 KiB. The parent buffers but does not commit. Keys remain globally unique and the existing ≤256-key/2-MiB aggregate applies. With `storage`, the guest may list/read old isolated KV.
6. After the exhausted Tag terminal page, flush requests use contiguous indices `0` through at most `9`. Each response before the terminal one is `more` with a nonempty segment, exactly one response is `complete` (possibly at index `0`), and no flush may follow it; index `10`, a duplicate key, or any bound violation fails.
7. Exactly one `finalize` follows `complete` and returns only the existing leave/replace choice. Zero staged entries plus replace means intentional empty replacement. Staged entries are bytewise-key sorted and hashed exactly as `SHA-256("junban.plugin.resync-kv.v1\0" || u32be(count) || each u32be(key-length) || key || u32be(value-length) || value)`.
8. One final transaction revalidates the transcript/session/delivery authority, invocation request hash/state, KV digest/count/bytes/bounds, package generation, activation epoch, current host session, contiguous retained tail after `R`, and expected cursor. It applies the KV choice, CASes the cursor to `(E,R,false)`, and deletes the invocation row together. No global event is emitted.
9. Crash before commit loses the runtime transcript/staging and restarts with a fresh session. Crash after commit is recognized by the cursor and cannot finalize twice. Catch-up applies only retained events `> R`, reaches head, then permits same-epoch activation. Epoch/tail/session/request/CAS failure restarts or suspends rather than skipping.
...
Before Slice 2D coding, its focused API/planning recheck must additionally approve unchanged WIT/generated-body hashes plus the exact runtime tests named in the Wave 2 plan: delivery mode/hash mismatches; every resync page/flush-index/finalize/KV/crash sequence; exact HTTP header and zero/one-callback ambiguity boundaries; all sixteen direct-event converters and every malformed/cursor-only/atomic-cursor case; and service root depth 0 with edge 8 accepted, edge 9 rejected before dispatch, cycles rejected, and activation depth 16 independent. Until then `P7-PLAN-2D-001`–`004` remain fixed-in-plan/recheck-pending and Slice 2D is blocked.
