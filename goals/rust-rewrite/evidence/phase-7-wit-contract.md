...
Focused Slice 2D review at exact `08c42c5b44411fdb4b81545c993a60a80960682d` already fixed/closed `P7-PLAN-2D-003`. Before coding, one narrow recheck must approve unchanged WIT/generated-body hashes plus corrected `P7-PLAN-2D-001`, `002`, and `004` tests: all-hook payload/final request hash goldens, one-field/canonical-body failures, Active+Starting process-loss recovery with only HTTP ambiguity; exact transcript initial/step framing/tags/index/bytes, SET-only/delete rejection and replacement semantics; all sixteen direct converters and concurrent moved/reordered/bulk/restored, multi/cascade complete/delete, Project/Tag/Section delete, subscribed/unsubscribed direct, undo/import/future baseline-ID, irrelevant-event, restart-then-complete cases; atomic cursor; and the retained service-depth matrix. Those three remain fixed-in-plan/narrow-recheck-pending and Slice 2D is blocked.
