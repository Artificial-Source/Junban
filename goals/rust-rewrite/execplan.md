...
- `P7-PLAN-2D-005` — **high/fixed in plan; focused recheck pending**. The WIT query shapes were frozen, but ordinary internal AppService/SQLite query semantics, cursor authentication/private-key ownership, complete SDK reply-byte accounting, and focused tests were underspecified. `evidence/phase-7-capability-matrix.md` now freezes the minimal internal authority without WIT/schema/public/dependency changes. Query/callback coding is blocked only on its focused recheck; resync and supervisor fixes remain independent.
