...
- Strict parent process transport began at `cc23507`; corrections at exact `ddacfa2d65c4b4a373f8165f20033bad1ffbfc2f` passed focused security recheck with **APPROVE**, no material issues and all 18 focused tests. Strict pre/post deadline checks and bounded 10-ms polling now place send, acknowledgement, process exit and reader EOF under one 1,000-ms deadline; every failure kills, waits/reaps and joins, and success requires a timely zero exit plus clean EOF. Trailing, partial-body, extra-body and malformed output are fatal. `P7-PC-SEC-002` and `P7-PC-SEC-003` remain fixed and closed, so the process owner is accepted at exact `ddacfa2` for serialized supervisor consumption. `P7-PC-SEC-001` remains rejected—not fixed—under the explicit same-user/install-directory threat model, and the Phase 8/9 sibling-directory install rule remains unchanged. Wakeable runtime-driver commit `6d0e94dc51cc9dae4ca835a5a3c2bbba71d4f200` then added bounded nonblocking command/event multiplexing. Initial medium finding `P7-WAKE-SEC-001` showed that forced or `Drop` closure could emit a graceful terminal; first fix `2ac281350aab551b1c1ab025bdea07d10baa4f08` remained open after a shutdown-overlap race. Final correction `c9474bce9258dfe6072f2bc702f658d497be1be2` uses monotonic `Open`→`GracefulInProgress`→`GracefulCommitted` CAS authority, permits forced states to win from `Open` or `GracefulInProgress`, keeps admission in one nonblocking critical section, and uses RAII body-byte accounting. Focused security recheck returned **APPROVE** and closed `P7-WAKE-SEC-001`: all 35 focused tests passed in three full debug runs and one full optimized run, and the paused-overlap regression passed 20 debug plus 10 optimized repetitions. The wakeable driver is authorized for supervisor consumption. Final supervisor-packet implementation is in progress but is not accepted or composed.
...
The four high Slice 2D findings are fixed in this plan but remain pending focused API/planning recheck. No Slice 2D implementation starts before that recheck; the final Slice 2C supervisor is independently still in progress, unaccepted, and uncomposed.

`PluginDeliveryAuthority` is bounded runtime-local state and is never serialized. It contains exactly plugin ID, package generation, activation epoch, current host session, invocation ID, payload SHA-256, and closed mode `StartingResync | StartingCatchUp | Active`. The canonical length-framed fields in that order are hashed under `junban.plugin.delivery-authority.v1\0`; the raw digest is incorporated into the existing invocation `request_hash`. The exact same authority/hash must match reserve, every transition, terminalization, verified cursor-only skip, and resync finalization. `StartingResync` is resync-only. `StartingCatchUp` is retained-`HandleEvent`-only and cannot authorize command/action/HTTP/domain/KV/arbitrary-cursor work; exact catch-up reaches head before same-epoch activation. Process loss abandons its exact starting-mode `reserved`/`effect_committing` rows and starts a fresh host session/authority, never HTTP ambiguity.

`PluginResyncTranscript` is also bounded runtime-local, nonpersisted state. A domain-separated rolling hash binds its exact delivery fence/session and every canonical request/outcome. It records a global contiguous page index and compact per-kind counts/bytes/last IDs/digests, flush/finalize state, and staged-KV count/bytes/digest. Task→Project→Tag order is mandatory, including at least one page per kind; kind/`after_id` exact-match, and only exhausted Tag carries the terminal marker. Tasks use their real `revision <= R`; Project/Tag use the existing retained-tail anti-join. Flush requests use contiguous indices `0` through at most `9`: each preterminal `more` is nonempty, exactly one response is `complete` (possibly at index `0`), and nothing follows it; exactly one finalize follows. Sorted globally unique staged KV is hashed as `SHA-256("junban.plugin.resync-kv.v1\0" || u32be(count) || length-framed bytewise-key-sorted entries)`. One transaction revalidates transcript/session/invocation/KV digest and bounds/epoch/current session/contiguous tail/expected cursor, applies the KV choice, CASes `(E,R,false)`, and deletes the invocation. Crash before commit restarts; crash after commit is recognized by cursor. No row/migration is added.
...
The exhaustive event converter directly admits only Task create/update/complete/uncomplete/cancel/reopen/delete plus create/update/delete Project/Tag/Section. Nondelete type/primary/snapshot resource and ID must agree; delete has exact typed primary and no snapshot. Task uses retained snapshot; other WIT revision is the enclosing event revision. Task moved/reordered/bulk/restored, undo/import, plugin/AI/settings/templates/comments/relations/time events, all other/future revisions, and unsubscribed direct kinds are cursor-only. Malformed direct events fail closed. No live reads or affected-ID synthesis exist. Reserve binds exact retained event hash/revision/cursor; bounded skip verification stops before the first subscribed direct event; accepted effect/KV/HTTP terminal result and source cursor commit together. Service depth counts edges separately from graph depth: root 0, eighth allowed, ninth rejected before dispatch, while activation depth stays 16.
...
- AppService-only mutations, trap-before-commit, exact receipt replay, changed replay conflict, and Slice 2D `PluginDeliveryAuthority` field/mode/hash/fence/startup-loss/head-before-active matrix;
- exhaustive sixteen direct-event conversion and malformed/cursor-only/no-live-read/hash-revision-cursor/skip-stop/atomic terminal-cursor matrix; runtime-only resync transcript page/order/after-ID/terminal/flush-index-0-and-9/index-10-rejection/finalize/KV-digest/tail/CAS/crash matrix; root-0/eighth-service-edge-allowed/ninth-rejected plus independent activation-depth-16 matrix;
- exact HTTP request/response header names and all case/order/duplicate/value/OWS/control/count/aggregate/raw-metadata limits; forbidden headers and transport-owned Host/content-length/delivery-id/identity encoding; zero/one callback, second no-send, durable pre-send dispatch, exact same-memory resend, and process-restart unresolved/no-guest-rerun;
...
4. Wave 2 security gate for package trust, selective linker, capability enforcement, resources, IPC, effects, crash containment, and hostile matrix. Before Slice 2D coding, one focused API/planning recheck must approve `P7-PLAN-2D-001`–`004`; fixed-in-plan is not closed/authorized.
...
| `P7-PLAN-2D-001`                 | High     | fixed-in-plan; recheck pending | Invocation `request_hash` did not bind a runtime delivery mode/current session/payload authority consistently through reserve, transition, terminal, skip and resync-finalize paths, leaving startup recovery and activation ordering ambiguous. The correction freezes nonserialized `PluginDeliveryAuthority`, exact hash binding, closed startup/active modes, starting-mode denials, process-loss abandonment/fresh session, and catch-up-head-before-active.                                                                                                                                                                                                                                                                                                                                                                               |
| `P7-PLAN-2D-002`                 | High     | fixed-in-plan; recheck pending | Resync could trust caller summaries without one exhaustive transcript/KV digest and incorrectly described Project/Tag as row-revision filtered. The correction freezes the bounded nonpersisted rolling transcript, mandatory global Task→Project→Tag sequence, exact flush-index-0-through-9/one-complete/finalize/KV digest, one revalidating final transaction/crash rule, real Task revision predicate, retained-tail Project/Tag anti-join, and no schema row.                                                                                                                                                                                                                                                                                                                                                                             |
| `P7-PLAN-2D-003`                 | High     | fixed-in-plan; recheck pending | HTTP header syntax/exposure, single-callback authority, durable pre-send order, and process-loss ambiguity were underspecified. The correction freezes exact allowlists and metadata bounds, forbidden/transport-owned headers, zero/one consume-once callback, same-memory identical resend only, and unresolved process loss with no guest rerun.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `P7-PLAN-2D-004`                 | High     | fixed-in-plan; recheck pending | Event conversion could synthesize from live reads/affected IDs or skip malformed/subscribed direct events, effect/cursor atomicity was not exhaustive, and service depth had an off-by-one ambiguity. The correction freezes all sixteen direct mappings, cursor-only families, exact event/cursor reserve and bounded skip stop, atomic terminal result+cursor, root-0/eighth-edge-allowed/ninth-rejected service depth, and separate activation depth 16.                                                                                                                                                                                                                                                                                                                                                                                     |

The original focused planning recheck approved `P7-PLAN-001`–`003`, after which Wave 0 retained the on-demand child and Wave 1 was accepted. The focused Slice 2C planning verdict at exact reviewed HEAD `442ebdd61e71a19a3e906b33b54bf7857ad555f8` remains the prior **APPROVE** authority: `P7-PLAN-2C-001`–`003`, `P7-PLAN-2C-004`, and `P7-PLAN-2C-004-EVENT-ATOMICITY` remain fixed and closed, and all prior approvals/evidence remain intact. Focused planning recheck at exact `f6183fd9ffdbe5879600cf7c071157e78e615ebb` subsequently approved and closed `P7-PLAN-2C-005`; focused database review approved its Packet B implementation at exact `656e8431956dd3aebf7dc86029e5130beff930cd` with no material issues and authorized supervisor consumption. Process corrections at exact `ddacfa2d65c4b4a373f8165f20033bad1ffbfc2f` likewise passed focused security recheck with **APPROVE**, and `P7-PC-SEC-002` and `P7-PC-SEC-003` remain fixed/closed while `P7-PC-SEC-001` remains rejected under the explicit same-user/install-directory threat model. Wakeable driver commit `6d0e94d` plus final CAS correction `c9474bc` passed focused security recheck with **APPROVE**; medium `P7-WAKE-SEC-001` is fixed/closed and the driver is authorized for supervisor consumption. CI run `31303724227` at `2c8affb` was interrupted only by the Rust job's former 20-minute limit after tests and release-binary compilation; `4fbd100` raises only that timeout to 30 minutes. Exact pre-supervisor driver head `c9474bc` subsequently passed CI `31310661503` and process-memory calibration `31310658785`, proving the codec/source/process/storage/driver integration boundary without expanding the focused reviews. Final supervisor-packet implementation and its current-head CI remain pending and are not accepted or composed; Slice 2C, Wave 2 and `P7-DEP-001` remain open. Slice 2D findings `P7-PLAN-2D-001`–`004` are fixed in the written plan but pending focused API/planning recheck; they are not closed, and Slice 2D coding remains blocked.
