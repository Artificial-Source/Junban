...
This document focuses on the main AI entry points, core abstractions, provider wiring, and tool categories rather than trying to be a brittle file-count inventory.
...
- `createDefaultRegistry()` — creates an `LLMProviderRegistry` and registers the built-in providers
- `createDefaultToolRegistry()` — creates a `ToolRegistry` and registers the built-in AI tool set
