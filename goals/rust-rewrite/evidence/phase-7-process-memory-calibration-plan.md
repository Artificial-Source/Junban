...
During the placement calibration, `.github/workflows/phase7-process-memory-calibration.yml` was manual and listened only to branches named `phase7-process-memory-calibration/**`. Its Ubuntu, macOS, and Windows jobs built the exact release host, ran the fixed warm-up/five-sample campaign, and uploaded one raw JSON artifact without making an acceptance claim. Phase 7 completion reuses that registered manual workflow path for the integrated Wave 5 campaign; the superseded calibration matrix and duplicate workflow entry were removed.
