...
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Contract remains `evidence/phase-7-context-map.md`. Planning, schema, package, WIT and host-placement gates are approved.
  ...
  - [ ] Wave 1 SDK-first acceptance: initial implementation reviews are blocked on `P7-PKG-001`–`003` and `P7-WIT-001`–`006` (bounded JRI1/metadata parsing, real Rust+TypeScript consumers, callback-capable fenced protocol, shared exact WIT/domain types). Run focused rechecks, then the clean parent five-sample SDK matched-release gate; schema v7 must not start earlier. Runtime host, routes, UI, registry artifacts, and reference plugins have not started.
