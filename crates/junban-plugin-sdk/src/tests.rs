...
#[test]
fn jri1_public_construction_is_deterministic_canonical_and_package_derived() {
    let package_bytes = valid_package();
    let package = verify_package(&package_bytes).unwrap();
    let key = root_key();
    let entry = registry_entry_from_verified_package(&package, vec!["tasks".into()]).unwrap();
    assert_eq!(entry, valid_registry_entry(&package));
    ...
    let canonical = index.canonical_bytes().unwrap();
    assert_eq!(RegistryIndex::parse_canonical(&canonical).unwrap(), index);
    assert!(
        RegistryIndex::parse_canonical(
            format!(" {}", String::from_utf8(canonical.clone()).unwrap()).as_bytes()
        )
        .is_err()
    );

    let first = pack_registry(&index, &key).unwrap();
    let second = pack_registry(&index, &key).unwrap();
    assert_eq!(first, second);
    assert_eq!(first, registry_envelope(&index, &key));
    assert_eq!(
        hex(&sha256(&first)),
        "a7b4fa79344094e1202e392e789e6f07dd458d552aa9a9f27056549af82e0ca8"
    );
    assert_eq!(
        parse_and_verify_registry(&first, &key.verifying_key().to_bytes())
            .unwrap()
            .index(),
        &index
    );
    ...
    assert!(matches!(
        pack_registry(&wrong_root, &key),
        Err(SdkError::Registry {
            field: "root_key_id"
        })
    ));
    assert!(registry_entry_from_verified_package(&package, vec!["z".into(), "a".into()]).is_err());
}
...
fn protocol_load_callback_cancellation_and_limits_are_fenced() {
    ...
    assert_eq!(
        ...
        format!(
            "{{\"linear_memory_bytes\":67108864,\"guest_stack_bytes\":2097152,\"table_elements\":10000,\"memories\":1,\"tables\":2,\"instances\":14,\"fuel\":{},\"host_resources\":64,\"hostcall_copy_bytes\":4194304,\"output_bytes\":262144,\"guest_log_message_bytes\":4096,\"guest_log_fields\":16,\"guest_log_invocation_bytes\":32768,\"wasi_stderr_bytes\":32768,\"compile_timeout_ms\":30000,\"command_timeout_ms\":1000,\"event_render_timeout_ms\":250,\"http_timeout_ms\":5000}}",
