...
- SQLite/AppService-owned desired state, epochs, health, retry/backoff, dependency propagation, events and receipts; `Loaded` precedes required resync/catch-up and same-epoch activation CAS; ordinary plugin-local trap/timeout/resource failure retains its attempt epoch and replaces only its Store/instance; child/session-fatal loss uses one operation identity and exactly one AppService mutation/SQLite transaction/global revision/bounded `plugin.health_changed` event/durable receipt to fence every matching selected plugin once at a new epoch before replacement, with one sorted unique max-16 canonical per-plugin result list and matching sorted `AffectedIds.plugin_ids`; a later retry increments once again and retains its retry epoch through activation;
...
- ordinary plugin-local trap/timeout/resource failure is same-attempt, retains that plugin's current epoch and replaces only its Store/instance while siblings remain active; protocol/transport/EOF/exit/worker/session loss and compile/load timeout or failure requiring child reap use one fail-closed atomic graph fence before replacement;
- one PID loads the dependency graph; 16/17 runtime bounds and four/fifth/same-plugin/nested saturation pass; child-fatal mixed `starting`/`active`/not-yet-loaded coverage proves exact before/after epochs, no old-epoch material transition, and exactly one operation/AppService mutation/SQLite transaction/global revision/bounded `plugin.health_changed` event/durable receipt for the graph fence—not N envelopes; the canonical request/result list is sorted, unique, bounded to 16 and complete for expected/new epoch, prior/target health, stable cause, desired-enable outcome and dependency disposition; sorted `AffectedIds.plugin_ids` contains every changed plugin; whole-graph CAS, transitions, affected IDs, event and receipt roll back together on mismatch; exact replay returns the one receipt, a changed request conflicts, and no new event/schema/OpenAPI shape or migration is introduced; retry-epoch activation plus stale-frame rejection, compile/load reap, discovery rejection, sibling survival and Linux/macOS/Windows no-orphan behavior pass;
...
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Contract remains `evidence/phase-7-context-map.md`. Wave 0/1 planning, schema, package, WIT and host-placement gates are approved; the exact-HEAD Slice 2C planning verdict is **APPROVE** with no blockers, and Slice 2C is authorized to implement from planning and security.
  ...
  - [x] Wave 2 Slice 2C Packet C shared-codec foundation: shared symmetric SDK framing codec extraction commit `aa3e591` is integrated and locally validated. It has no separate review or remote current-head gate and is not part of Packet A's exact review scope.
...
- 2026-08-09: Shared symmetric SDK framing codec extraction commit `aa3e591` is integrated and locally validated as a Packet C foundation. It has no separate review or remote current-head gate and is outside Packet A's exact review scope.
- 2026-08-09: Verified component-source port commit `48ac139b0ad6109a1784d0ad2043916b80a16e00` is integrated. Its focused security/integrity review verdict is **APPROVE** with no material issues; static Linux/macOS/Windows path review and 13 focused source/package/normal-open/restore tests passed, and the parent supervisor may consume `AppService::open_plugin_component_sources`. The public hidden constructor is trusted in-repository only and must not be called by the supervisor. No stable finding ID was created. The strict parent process owner is underway separately; Packet C, Slice 2C and Wave 2 remain incomplete and unaccepted, and current C1-head CI plus process-owner/supervisor/integrated evidence remain.
