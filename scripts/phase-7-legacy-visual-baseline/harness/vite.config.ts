...
if (!LEGACY_ROOT) {
  throw new Error("JUNBAN_LEGACY_WORKTREE is required for the Phase 7 visual harness");
}
...
export default defineConfig({
  ...
  cacheDir: path.join(LEGACY_ROOT, ".phase7-vite-cache"),
  publicDir: false,
  ...
  server: {
    host: "127.0.0.1",
    port: Number(process.env.PHASE7_VITE_PORT ?? 5197),
    strictPort: true,
    fs: {
