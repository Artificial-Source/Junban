...
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Contract remains `evidence/phase-7-context-map.md`. Wave 0/1 planning, schema, package, WIT and host-placement gates are approved; Slice 2C remains unauthorized pending the `P7-PLAN-2C-004` focused recheck.
  ...
  - [ ] Wave 2 Slice 2C lifecycle planning recheck: `P7-PLAN-2C-004` is fixed in writing but pending one focused recheck, and implementation remains unauthorized. Ordinary plugin-local trap/timeout/resource failure retains the current attempt epoch and replaces only that plugin's Store/instance. Child/session-fatal loss uses one atomic graph-fence AppService mutation to advance every matching selected `starting` or `active` plugin exactly once, record only new-epoch `child-fatal`/`compile-load`, `child/session-lost`, or `dependency_failed` progression, and complete before replacement; CAS mismatch aborts the graph transition. A later retry advances once again and retains its retry epoch through activation, so `starting` E→E+1 fencing followed by E+1→E+2 retry is intentional rather than double-counting.
...
- Phase 7 Slice 2C planning review — **partially closed; implementation unauthorized**. The focused recheck closed `P7-PLAN-2C-001`–`003`. `P7-PLAN-2C-004` now freezes same-attempt plugin-local failure versus one atomic child/session-fatal graph fence, exact epoch/cause/receipt/event/stale-frame behavior and the separate retry increment; it is fixed in writing but pending one focused recheck before Slice 2C is authorized.
