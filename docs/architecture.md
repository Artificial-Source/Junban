...
| `junban-plugin-host` | Isolated on-demand Wasmtime child; Slice 2A owns strict private framing, one-component compilation, and process cleanup                 |
...
The accepted runtime placement is the on-demand `junban-plugin-host` child. Slice 2A pins `wasmtime` and `wasmtime-wasi` 36.0.13 only in that binary crate, validates the SDK-canonical protocol before bounded allocation, constructs the Engine in the launched child, and compiles one exact hash-bound component without instantiating or linking guest imports. Slice 2A.1 adds the generated typed body boundary, exhaustive generated neutral↔Wasmtime value adapters, and invocation-kind-bound guest outcome frames; compile-time binding/adapter round trips do not instantiate, link, or execute a guest. Selective linking, stores, guest execution, and capability callbacks remain Slice 2B. Default `junban-server` still touches only the SDK's static authority; `--no-default-features` remains the matched feature-off baseline, and neither server build links Wasmtime. The completed Wave 0 probe crate, harness, and Wasmtime advisory exception remain removed.

Real retained Rust 1.93/`wasm32-wasip2` and TypeScript/JavaScript consumers compile the public WIT and are structurally inspected by ordinary SDK tests. Rust uses exact wit-bindgen 0.51.0 and exactly five frozen WASI imports; TypeScript uses build-only jco 1.26.1 plus ComponentizeJS 0.22.0 `--disable all` and has zero WASI imports. Node is never a product runtime. Declarative host-rendered UI replaces arbitrary plugin React execution. Schema v7, package publication, product runtime, routes, registry artifacts, reference plugins, and UI remain outside this SDK task.
