...
**Status:** approved by the user on 2026-07-28 and expanded with an evidence-driven Phase 10 on 2026-07-29. Phases 0 through 6 are complete. Phase 7 capability-limited portable plugins are in progress: Waves 1 and child-runtime Slice 2B are accepted; Slice 2C, Slice 2D, Wave 2, supervisor, resync, and final callback composition are not accepted. At exact clean HEAD `dbf63a58f38df245b0b62337b29c644b8054984c`, focused query planning recheck **APPROVED** and closes `P7-PLAN-2D-005`/`P7-PLAN-2D-005-UUID`, authorizing app/storage coding only while preserving exact UUID octet/nil/non-RFC/version semantics, fixed 122-byte layout, MAC, and error mapping. Ordinary plugin query implementation at exact clean HEAD `dbb3700fa5459369633eb42b53723427244a6683` (implementation equivalent `aa31d9878937e4310752b0999aa222bec3a174f5`) received focused database **APPROVE** with no material issues: focused AppService 3/3, focused storage query 16/16, full app 65/65, full storage 296/296, `fmt`, Clippy warnings denied for app/storage, and diff check. Delivered authority is limited to typed AppService/repository APIs, one-transaction snapshot/keyset pages, fixed authenticated cursor/private HMAC/rotation, and exact canonical SDK success-reply byte accounting; no server/WIT/schema/public/dependency change. Server callback composition may consume these APIs while retaining required pre/post authority and grant checks. Final supervisor security recheck **APPROVED** exact `768671ed07cc55df902d525d757d8fb0f1eb0308`, closing `P7-2C-SEC-004`/`P7-2C-TEST-001` for final supervisor/Slice 2C coding security only. Bounded resync integrated at `1cc4ba9` (reviewed implementation equivalent `90467bf`) received focused database **APPROVE** with no material issues: 62 app tests, 280 storage tests, focused resync tests, Clippy warnings denied, and format/diff; it has no schema/WIT/public/dependency expansion, no global event/revision/receipt, and one final transaction, and server composition may consume its APIs. EOF fixture correction `71856e5` consumes exact parent Hello before deliberate EOF; focused debug/release passed, resolving the second prior Rust race, but no exact-head CI rerun exists and CI green is not claimed. `P7-2D-DB-002`, callback/server composition, integrated CI/evidence, Slice 2C/Slice 2D/Wave 2/Phase acceptance, `P7-DEP-001`, later packets, and Phases 8–10 remain open.
...
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Wave 0/1 gates are approved. Exact `dbf63a58f38df245b0b62337b29c644b8054984c` query planning recheck closed `P7-PLAN-2D-005`/`P7-PLAN-2D-005-UUID`, authorizing app/storage coding only. Exact `768671ed07cc55df902d525d757d8fb0f1eb0308` final supervisor security recheck closed `P7-2C-SEC-004`/`P7-2C-TEST-001` for coding security only. Bounded resync at `1cc4ba9` (reviewed equivalent `90467bf`) is focused database-approved for server consumption, not overall resync acceptance. `P7-2D-DB-002`, callback/server composition, integrated CI/evidence, supervisor/Slice 2C/Slice 2D/Wave 2/Phase acceptance, `P7-DEP-001`, and later packets remain open; no exact-head CI rerun after EOF correction `71856e5` means CI green is not claimed.
  ...
  - [x] Wave 2 plan and dependency-security checkpoint: `RUSTSEC-2026-0222` blocks the historical Wasmtime 45.0.3 spike line. Production keeps Rust 1.93 and selects exact patched 36.0.13 LTS with defaults off, no advisory exception, and replacement hostile/performance evidence; `P7-DEP-001` remains open until that implementation evidence passes. See `evidence/phase-7-wave-2-plan.md`.
  - [x] Wave 2 Slice 2A isolated child/protocol foundation: exact Wasmtime 36.0.13 is confined to `junban-plugin-host`; the SDK-owned strict protocol codec, one-component hash-bound compile authority, state/correlation fencing, bounded failures, protocol-only stdout, clean EOF/shutdown, and scrubbed absolute sibling-process harness pass. Selective linking, stores, guest execution, capability callbacks, hostile containment and replacement optimized evidence remain Slices 2B–2E; `P7-DEP-001` and the Wave 2 security gate stay open. See `evidence/phase-7-wave-2-slice-2a.md`.
  ...
  - [ ] Wave 2 Slice 2C Packet C through Slice 2E: final supervisor security recheck **APPROVED** exact `768671ed07cc55df902d525d757d8fb0f1eb0308`, closing `P7-2C-SEC-004`/`P7-2C-TEST-001` for coding security only. Resync `1cc4ba9` (reviewed equivalent `90467bf`) is focused database-approved but remains unaccepted overall. Corrected zero-swap Slice 2E run `31629776892` at exact `e196313b5a463681254a2401ebc9787f99e97e13` closed `P7-2E-PERF-001` and `P7-2E-EVID-001` while truthfully failing all historical active gates; formal `P7-2E-PERF-002` amendment now requires one passing exact-amended-head campaign. Parent supervision/callback completion, server removal blocker, integrated review/evidence, amended optimized measurements, and `P7-DEP-001` remain.
...
- Supervisor recheck — final supervisor security recheck **APPROVED** exact `768671ed07cc55df902d525d757d8fb0f1eb0308`, closing `P7-2C-SEC-004`/`P7-2C-TEST-001` for coding security only. EOF correction `71856e5` resolved the second prior Rust race, but no exact-head CI rerun exists, so CI green and supervisor/Slice 2C/Wave 2 acceptance remain withheld.
- Phase 7 hostcall-fuel security recheck — **approved after fixes**. The recheck closes `P7-PLAN-RUNTIME-001` and `P7-RUNTIME-SEC-001` and authorizes Slice 2C from that security gate. The replacement-CI/macOS gate is closed at `ce80dea`, the Packet B database gate is approved at exact `e573cfe` and `656e843`, and integrated Packet A/B full CI and process-memory calibration passed at exact `0f2f9b31633470dc75973fb15aef99ade354eafb`; `P7-DEP-001`, final supervisor/compile-load wiring, final-supervisor current-head CI and integrated evidence remain open; exact pre-supervisor head `c9474bc` has passed its narrower remote gate. The process owner at exact `ddacfa2` and wakeable driver at exact `c9474bc` are accepted only for supervisor consumption. Slice 2C and Wave 2 are not accepted.
