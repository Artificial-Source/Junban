...
use junban_app::{
    ...
    PluginInvocationDelivery, PluginInvocationDeliveryCheck, PluginInvocationPublicOutcome,
    ...
    PluginMutationRequestIdentity, PluginOperatorRequestIdentity, PluginPackageAdmission,
    PluginPackageReconciliation, PluginResyncEvent, PluginResyncKvCommit, PluginResyncPage,
    PluginResyncPageRequest, PluginResyncSession, PluginRetainedEventClassification,
    PluginRuntimeState, PluginSetting, PluginSnapshotItem, PluginSnapshotKind, PublisherTrust,
    PublisherTrustStatus, RecordPluginAttemptFailureRequest, ReplacePluginGrantsRequest,
    RepositoryError, ReservePluginInvocationRequest, ReservedPluginInvocation, ResourceRef,
    ResourceSnapshot, ResyncScope, RevokePluginGrantsRequest, SetPluginSettingRequest,
    TransitionPluginInvocationRequest, TrustPublisherRequest, VerifiedBundledPluginAdmission,
    VerifiedPluginCursorSkipRequest, classify_plugin_resync_event, convert_active_plugin_event,
    plugin_committed_event_content_hash, plugin_invalidating_event_request_digest,
...
fn trust_allows_install(
    ...
    match request.source {
        ...
        // Only the nonserializable verified-registry admission command may set
        // this flag. A caller-selected enum remains unable to bypass Restricted Mode.
        PluginInstallSource::BundledRegistry if verified_bundled => Ok(()),
...
pub(crate) fn install_plugin(
    ...
    install_plugin_with_authority(connection, store, operation_id, request, false, now)
}

fn install_plugin_with_authority(
    connection: &mut Connection,
    store: &PluginPackageStore,
    operation_id: OperationId,
    request: InstallPluginRequest,
    ...
    let result = mutate(
        ...
        move |tx, revision| {
            ...
            trust_allows_install(tx, &request, verified_bundled)?;
...
pub(crate) fn install_plugin_admission(
    ...
    install_plugin_admission_with_authority(
        connection,
        store,
        operation_id,
        admission,
...
pub(crate) fn install_bundled_plugin_admission(
    connection: &mut Connection,
    store: &PluginPackageStore,
    operation_id: OperationId,
    admission: VerifiedBundledPluginAdmission,
    replace_existing: bool,
    allow_downgrade: bool,
    now: Timestamp,
) -> Result<PluginMutationOutcome, RepositoryError> {
    let (admission, _registry_index_sha256) = admission.into_parts();
    let package = admission.package().clone();
    install_plugin_admission_with_authority(
        connection,
        store,
        operation_id,
        ...
        InstallPluginRequest {
            package,
            source: PluginInstallSource::BundledRegistry,
            replace_existing,
            allow_downgrade,
        },
        true,
...
fn install_plugin_admission_with_authority(
    connection: &mut Connection,
    store: &PluginPackageStore,
    operation_id: OperationId,
    admission: PluginPackageAdmission,
    ...
    let result = install_plugin_with_authority(
        connection,
        store,
        operation_id,
        InstallPluginRequest {
...
struct StoredGrantMutationRequest<T> {
    op: String,
    request: T,
}

pub(crate) fn replay_plugin_mutation(
    connection: &mut Connection,
    identity: PluginMutationRequestIdentity,
    now: Timestamp,
    ...
    let receipt = connection
        ...
        .optional()
        .map_err(storage_error)?;
    let Some((stored_request, stored_response, expires_at)) = receipt else {
        return Ok(None);
    };
    if expires_at
        .as_deref()
        .map(|value| parse_timestamp(value, "plugin mutation receipt expiry"))
        .transpose()?
    ...
    let matches = match identity {
        PluginMutationRequestIdentity::ReplaceGrants {
            ...
            let parsed: StoredGrantMutationRequest<ReplacePluginGrantsRequest> =
                serde_json::from_str(&stored_request)
                    .map_err(|_| RepositoryError::IdempotencyMismatch)?;
            parsed.op == "replace_grants"
                && canonical_json(&OperatorRequest::ReplaceGrants {
                    request: &parsed.request,
                })? == stored_request
                && parsed.request.plugin_id == plugin_id
                && parsed.request.package_generation == package_generation
                && parsed.request.permissions == permissions
        }
        PluginMutationRequestIdentity::RevokeGrants {
            ...
            let parsed: StoredGrantMutationRequest<RevokePluginGrantsRequest> =
                serde_json::from_str(&stored_request)
                    .map_err(|_| RepositoryError::IdempotencyMismatch)?;
            parsed.op == "revoke_grants"
                && canonical_json(&OperatorRequest::RevokeGrants {
                    request: &parsed.request,
                })? == stored_request
                && parsed.request.plugin_id == plugin_id
                && parsed.request.package_generation == package_generation
        }
        PluginMutationRequestIdentity::SetSetting { request, .. } => {
            canonical_json(&OperatorRequest::SetSetting { request: &request })? == stored_request
        }
        PluginMutationRequestIdentity::DeleteSetting { request, .. } => {
            canonical_json(&OperatorRequest::DeleteSetting { request: &request })? == stored_request
        }
    };
    if !matches {
    ...
    let mutation: CommittedMutation =
        serde_json::from_str(&stored_response).map_err(storage_error)?;
    if mutation.event.operation_id != operation_id
        || serde_json::to_string(&mutation).map_err(storage_error)? != stored_response
    {
        return Err(RepositoryError::Storage(
            "plugin mutation receipt is inconsistent".to_owned(),
        ));
    }
    Ok(Some(mutation))
}
...
pub(crate) fn plugin_invocation_terminal_shape_is_valid(
    ...
    match (response.outcome, response.rejection) {
        (PluginInvocationPublicOutcome::Completed, None) => {
            (response.terminal_kind == PluginInvocationTerminalKind::DomainEffect)
                == response.mutation.is_some()
        }
        (PluginInvocationPublicOutcome::InvalidOutput, Some(_)) => {
            response.terminal_kind == PluginInvocationTerminalKind::DomainEffect
                && response.mutation.is_none()
                && response.cursor.is_none()
        }
        (PluginInvocationPublicOutcome::Completed, Some(_)) | (_, Some(_)) => false,
        (_, None) => {
            matches!(
                response.terminal_kind,
                PluginInvocationTerminalKind::ReadOnly | PluginInvocationTerminalKind::Http
            ) && response.mutation.is_none()
                && response.cursor.is_none()
        }
    }
...
pub(crate) fn validate_plugin_invocation_receipts(
    ...
    for row in rows {
        let (operation_id, request_json, response_json) = row.map_err(storage_error)?;
        let parsed_response = serde_json::from_str::<CommittedPluginInvocation>(&response_json);
        let request_identifies_invocation =
            serde_json::from_str::<serde_json::Value>(&request_json)
                ...
                .as_deref()
                == Some("plugin_invocation_terminal");
        if !request_identifies_invocation && parsed_response.is_err() {
            continue;
        }
        let response = parsed_response.map_err(|_| {
            RepositoryError::Storage(
                "plugin invocation terminal receipt response is invalid".to_owned(),
            )
        })?;
...
fn reserve_plugin_invocation_with_delivery(
    ...
    if operator_origin(request.hook_kind) {
            ...
            read_receipt_response(&transaction, request.operation_id, &receipt_request)?
        {
            ...
            if serde_json::to_string(&committed).map_err(storage_error)? != response
                || committed.cursor.is_some()
                || !plugin_invocation_terminal_shape_is_valid(&committed)
            {
                return Err(RepositoryError::Storage(
                    "plugin invocation terminal receipt is inconsistent".to_owned(),
                ));
...
fn terminalize_invocation_for_test(
    ...
    terminalize_invocation_outcome_for_test(
        connection,
        operation_id,
        plugin_id,
        package_generation,
...
#[cfg(test)]
fn terminalize_invocation_outcome_for_test(
    connection: &mut Connection,
    operation_id: OperationId,
    plugin_id: PluginId,
    package_generation: u64,
...
fn terminalize_plugin_invocation_with(
    ...
    terminalize_plugin_invocation_with_delivery(
        ...
        PluginInvocationCompletionFence {
            ...
            package_generation,
            activation_epoch,
            outcome: PluginInvocationPublicOutcome::Completed,
        },
        None,
        now,
        before_commit,
...
fn terminalize_plugin_invocation_with_delivery(
    ...
    if !plugin_invocation_terminal_shape_is_valid(&committed) {
        return Err(RepositoryError::Conflict);
    }
...
mod tests {
    ...
    use junban_app::{
        ...
        SetPluginSettingRequest, StagedFile, VerifiedBundledPluginAdmission,
        plan_authorized_plugin_invocation_commit,
    };
    use junban_domain::{
    ...
    use junban_plugin_sdk::{
        ...
        RegistryEntry, RegistryIndex, RuntimeProfile, SettingDeclaration, SettingSchema,
        SurfaceDeclaration, SurfaceKind, SurfaceLocation, UnscopedPermission, WitAuthority,
        pack_package, parse_and_verify_registry,
        private_body_types::{
            ...
            SnapshotPage, SnapshotRecords, SurfaceAction, WitResult,
        },
        signer_key_id,
    };
    use uuid::Uuid;
    ...
    fn verified_registry_admission(
        bytes: &[u8],
        authority: &PluginPackageAuthority,
        root_key: &SigningKey,
    ) -> VerifiedBundledPluginAdmission {
        let index = RegistryIndex {
            schema_version: 1,
            junban_version: env!("CARGO_PKG_VERSION").to_owned(),
            generated_at: "2026-08-10T00:00:00Z".to_owned(),
            root_key_id: Sha256Digest::of(&root_key.verifying_key().to_bytes()).to_string(),
            entries: vec![RegistryEntry {
                plugin_id: authority.plugin_id().to_string(),
                version: authority.manifest().version.clone(),
                package_sha256: authority.package_sha256().to_string(),
                package_size: authority.package_size(),
                publisher_key_id: authority.publisher_key_id().to_string(),
                name: authority.manifest().name.clone(),
                description: authority.manifest().description.clone(),
                author: authority.manifest().publisher.name.clone(),
                license: authority.manifest().license.clone(),
                search_tags: Vec::new(),
                runtime_profile: authority.manifest().runtime_profile,
                requested_capabilities: authority
                    .manifest()
                    .permissions
                    .iter()
                    .map(|permission| permission.capability)
                    .collect(),
                filename: junban_plugin_sdk::registry_package_path(
                    authority.package_sha256().as_str(),
                )
                .unwrap(),
            }],
        };
        ...
        message.extend_from_slice(&Sha256::digest(&index_bytes));
        let mut envelope = junban_plugin_sdk::JRI1_MAGIC.to_vec();
        envelope.extend_from_slice(&(index_bytes.len() as u32).to_be_bytes());
        envelope.extend_from_slice(&index_bytes);
        envelope.extend_from_slice(&root_key.sign(&message).to_bytes());
        let registry =
            parse_and_verify_registry(&envelope, &root_key.verifying_key().to_bytes()).unwrap();
        VerifiedBundledPluginAdmission::new(
            PluginPackageAdmission::inspect(stage_bytes(bytes)).unwrap(),
            &registry,
            authority.plugin_id(),
            &authority.manifest().version,
        )
        .unwrap()
    }
    ...
    #[test]
    fn grant_and_setting_receipt_probes_replay_before_current_plugin_authority() {
        let profile = TestProfile::new();
        let mut connection = profile.connection();
        let store = PluginPackageStore::open(&profile.path).unwrap();
        let now = Timestamp::constant(1_800_000_020, 0);
        let plugin = install_fixture(&mut connection, &store, now);

        let permission = plugin.manifest.permissions[0].clone();
        let grant_operation = OperationId::new();
        let grant_request = ReplacePluginGrantsRequest::new(
            plugin.plugin_id.clone(),
            plugin.package_generation,
            &plugin.manifest.permissions,
        ...
        .unwrap();
        let grant_mutation =
            replace_plugin_grants(&mut connection, grant_operation, grant_request, now).unwrap();

        let setting_operation = OperationId::new();
        ...
        let setting_mutation = set_plugin_setting(
            &mut connection,
            setting_operation,
        ...
        let replayed_grant = replay_plugin_mutation(
            &mut connection,
            PluginMutationRequestIdentity::ReplaceGrants {
        ...
        .unwrap();
        assert_eq!(
            serde_json::to_vec(&replayed_grant).unwrap(),
            serde_json::to_vec(&grant_mutation).unwrap()
        );

        let replayed_setting = replay_plugin_mutation(
            &mut connection,
            PluginMutationRequestIdentity::SetSetting {
                operation_id: setting_operation,
        ...
        .unwrap();
        assert_eq!(
            serde_json::to_vec(&replayed_setting).unwrap(),
            serde_json::to_vec(&setting_mutation).unwrap()
        );

        let mut changed_setting = setting_request;
        changed_setting.value = SettingValue::Boolean(false);
        assert_eq!(
            replay_plugin_mutation(
                &mut connection,
                PluginMutationRequestIdentity::SetSetting {
                    operation_id: setting_operation,
                    request: changed_setting,
                },
                now,
            )
            .unwrap_err(),
    ...
    #[test]
    fn bundled_install_requires_nonserializable_verified_registry_authority() {
        let profile = TestProfile::new();
        let mut connection = profile.connection();
        let store = PluginPackageStore::open(&profile.path).unwrap();
        let now = Timestamp::constant(1_800_000_040, 0);
        let (bytes, package, _) = package("bundled-plugin", "1.0.0");
        ...
        let admission = verified_registry_admission(&bytes, &package, &root_key);
        assert!(
        ...
        assert_eq!(
            get_installed_plugin(&connection, package.plugin_id().clone())
                .unwrap()
                .plugin_id,
            *package.plugin_id()
        );
    ...
    #[test]
    fn operator_public_terminal_classifications_replay_exact_receipt_bytes() {
        let profile = TestProfile::new();
        let mut connection = profile.connection();
        let store = PluginPackageStore::open(&profile.path).unwrap();
        let now = Timestamp::constant(1_800_000_183, 1);
        let installed = install_fixture(&mut connection, &store, now);
        ...
        for outcome in [
            PluginInvocationPublicOutcome::Completed,
            PluginInvocationPublicOutcome::Cancelled,
            ...
            let reservation = ReservePluginInvocationRequest {
                ...
                request_sha256: Sha256Digest::of(format!("{outcome:?}").as_bytes()),
                delivery_operation_id: OperationId::new(),
                resync_session: None,
            };
            reserve_invocation_for_test(&mut connection, reservation.clone(), now).unwrap();
            let initial = terminalize_invocation_outcome_for_test(
                &mut connection,
                operation_id,
                plugin.plugin_id.clone(),
                plugin.package_generation,
            ...
            .unwrap();
            assert_eq!(initial.outcome, outcome);
            let initial_bytes = serde_json::to_vec(&initial).unwrap();
            let replay = reserve_invocation_for_test(&mut connection, reservation.clone(), now)
                .unwrap()
                .terminal()
                .unwrap()
                .clone();
            assert!(replay.replayed);
            assert_eq!(serde_json::to_vec(&replay).unwrap(), initial_bytes);
    ...
    fn operator_domain_and_kv_terminal_receipts_replay_typed_results() {
        ...
        let mut changed_domain = domain_reservation;
        changed_domain.request_sha256 = Sha256Digest::of(b"changed-domain-terminal");
        assert_eq!(
            reserve_invocation_for_test(&mut connection, changed_domain, now).unwrap_err(),
            RepositoryError::IdempotencyMismatch
        );

        let rejected_operation = OperationId::new();
        let rejected_reservation = reservation(rejected_operation, b"rejected-domain-terminal");
        reserve_invocation_for_test(&mut connection, rejected_reservation.clone(), now).unwrap();
        transition_invocation_for_test(
            &mut connection,
            TransitionPluginInvocationRequest {
        ...
        .unwrap();
        let rejected = commit_invocation_for_test(
            &mut connection,
            plan(CommitPluginInvocationRequest {
                invocation_operation_id: rejected_operation,
                plugin_id: plugin.plugin_id.clone(),
                package_generation: plugin.package_generation,
                activation_epoch: plugin.activation_epoch,
                outcome: PluginInvocationPublicOutcome::Completed,
                ...
                domain_effect: Some(PluginDomainEffect::CreateTask {
                    task_id,
                    draft: TaskDraft::new(TaskTitle::new("Changed typed terminal").unwrap()),
                }),
                kv_patch: None,
                resync_kv: None,
                cursor: None,
        ...
        let rejected_replay =
            reserve_invocation_for_test(&mut connection, rejected_reservation.clone(), now)
                .unwrap()
                .terminal()
                .unwrap()
                .clone();
        assert_eq!(
            serde_json::to_vec(&rejected_replay).unwrap(),
            rejected_bytes
        );
        let mut changed_rejection = rejected_reservation;
        changed_rejection.request_sha256 = Sha256Digest::of(b"changed-rejected-terminal");
        assert_eq!(
            reserve_invocation_for_test(&mut connection, changed_rejection, now).unwrap_err(),
            RepositoryError::IdempotencyMismatch
        );
    ...
    fn guest_effect_rejection_receipts_accept_only_closed_domain_effect_shapes() {
        let valid_rejection = CommittedPluginInvocation {
            outcome: PluginInvocationPublicOutcome::InvalidOutput,
        ...
        let valid_completed_read_only = CommittedPluginInvocation {
            outcome: PluginInvocationPublicOutcome::Completed,
        ...
        assert!(plugin_invocation_terminal_shape_is_valid(
            &valid_completed_read_only
        ));

        let mut invalid = valid_rejection.clone();
        invalid.outcome = PluginInvocationPublicOutcome::Completed;
    ...
    fn completed_operator_replays_by_stable_identity_across_host_rotation() {
        ...
        .unwrap();
        let replay_after_uninstall =
            replay_completed_plugin_operator(&mut connection, identity, now)
                .unwrap()
                .unwrap();
        assert_eq!(
            serde_json::to_string(&replay_after_uninstall).unwrap(),
            serde_json::to_string(&committed).unwrap()
        );
        crate::plugin_validation::validate_plugin_authority(&connection).unwrap();
        let backup = crate::backup_ops::create_backup(&connection, &profile.path).unwrap();
    ...
    #[test]
    fn completed_surface_action_replays_with_stale_host_session_and_changed_body_conflicts() {
        let profile = TestProfile::new();
        let mut connection = profile.connection();
        let store = PluginPackageStore::open(&profile.path).unwrap();
        let now = Timestamp::constant(1_800_000_188, 21);
        let (bytes, authority, public_key) = package_with_surfaces(
            "action-plugin",
            "1.0.0",
            Vec::new(),
        ...
        .unwrap();
        let installed =
            get_installed_plugin(&connection, PluginId::parse("action-plugin").unwrap()).unwrap();
        let ui_grant = installed
            .manifest
            .permissions
            .iter()
            .find(|permission| permission.capability == Capability::UiView)
            .unwrap()
            .clone();
        let granted = replace_plugin_grants(
            &mut connection,
            OperationId::new(),
            ReplacePluginGrantsRequest::new(
                installed.plugin_id.clone(),
        ...
        .unwrap();
        assert!(granted.event.snapshot.is_some());
        let granted = get_installed_plugin(&connection, installed.plugin_id).unwrap();
        let plugin = activate_plugin(&mut connection, &store, &granted, now);

        let surface_id = PluginId::parse("dashboard").unwrap();
        let action_id = PluginId::parse("refresh").unwrap();
        let entry = PluginManifestEntry::SurfaceAction {
            surface_id: surface_id.clone(),
            action_id: action_id.clone(),
        };
        let persisted_entry_id = junban_app::plugin_manifest_entry_persisted_id(&entry).unwrap();
        let private_body = serde_json::to_vec(&InvocationRequest::handle_surface_action(
        ...
        changed.payload_sha256 = Sha256Digest::of(b"changed action body");
        assert_eq!(
            replay_completed_plugin_operator(&mut connection, changed, now).unwrap_err(),
            RepositoryError::IdempotencyMismatch
        );
    ...
    fn operator_receipt_stable_and_executed_authority_corruption_fails_all_validators() {
        ...
        let mut missing_outcome: serde_json::Value =
            serde_json::from_str(&canonical_response).unwrap();
        missing_outcome.as_object_mut().unwrap().remove("outcome");
        let mut unknown_outcome: serde_json::Value =
            serde_json::from_str(&canonical_response).unwrap();
        unknown_outcome["outcome"] = serde_json::Value::String("unknown".to_owned());
        for malformed_response in [missing_outcome.to_string(), unknown_outcome.to_string()] {
            connection
                .execute(
                    "UPDATE operation_receipts SET response_json = ?2 WHERE operation_id = ?1",
                    params![operation_id.to_string(), malformed_response],
                )
                .unwrap();
            drop(connection);
            assert!(crate::ProfileOwner::open(&profile.path).is_err());
            connection = Connection::open(profile.path.join("junban.sqlite3")).unwrap();
