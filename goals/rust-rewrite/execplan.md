...
**Status:** approved by the user on 2026-07-28 and expanded with an evidence-driven Phase 10 on 2026-07-29. Phases 0 through 6 are complete. Phase 7 capability-limited portable plugins are in progress: Waves 1 and child-runtime Slice 2B are implemented, while process-memory planning/security blockers require a cross-platform calibration campaign, a later hard cap, hostile amplification evidence, and focused recheck before Slice 2C. Phases 8–10 remain.
...
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Contract remains `evidence/phase-7-context-map.md`. Planning, schema, package, WIT and host-placement gates are approved.
  ...
  - [ ] Wave 2 child-runtime security correction: `P7-RUNTIME-SEC-001` found canonical ABI lifting can amplify 64/128-MiB guest strings/lists into native allocations before post-lift callback/output limits. Add a hard cross-platform child-process memory ceiling and amplification/replacement evidence, then pass focused recheck before continuing Slice 2C.
    - [x] Calibration-only preflight implementation: exact 128-MiB Wasmtime reservation with zero guard/growth reservation, retained valid Rust/TypeScript working-set callback barriers, exact-child release sampler, and manual Linux/macOS/Windows warm-up-plus-five-run workflow are committed. `P7-PLAN-RUNTIME-001`, cross-platform runs, cap selection/enforcement, amplification proof, and both finding closures remain pending; no cap or Slice 2C authorization is claimed.
...
- 2026-08-05: `P7-PLAN-RUNTIME-001` blocks selecting a child cap from Wasmtime's default approximately 4-GiB wasm32 reservation. The calibration-only preflight tunes the one-memory Engine to the frozen 128-MiB maximum with zero guard/growth reservation and adds valid held-callback workloads plus exact-child release sampling on Linux/macOS/Windows. Cross-platform runs, a cap at least 125% of every valid platform/profile maximum, canonical-lift amplification triggering, enforcement, and focused security recheck remain mandatory; no cap is frozen and Slice 2C stays blocked.
