...
One serial owner retains the limited Store/instance across successful calls. Each call resets finite profile-specific fuel and an epoch deadline. One condition-variable watchdog advances the Engine epoch exactly once when the active invocation reaches its frozen 1-second command/lifecycle/UI deadline or 250-millisecond event/render/validation/resync deadline, or when an exact cancel/drain wins. The same stop transition cancels a pending callback rendezvous. Cancel, unload, shutdown, EOF, and malformed-input cleanup wait for active runtime ownership to drain; only test-process safety guards poll or sleep. Callback replies must exact-match the active generation/epoch/session/invocation/callback/kind authority, and late/duplicate replies remain fenced.

Successful calls retain their Store. Trap, fuel, epoch interruption, failed limit, invalid/oversized guest output, cancellation, and abort freeze completion, destroy the failed Store before any terminal frame or control acknowledgement, and lazily instantiate a clean Store from the retained compiled Component on the next call. Memory, core table/instance, 2-MiB stack, 64-entry host resource table, IPC/canonical callback copies, 256-KiB output, 32-KiB bounded stderr, and guest-log message/field-list/per-invocation bytes are finite. Stable fenced codes distinguish wall timeout, cancellation, resource exhaustion, guest trap, and authority failure without returning Wasmtime trap strings, guest stderr, callback bodies, or credentials. Retained launched Rust and TypeScript hostile components cover CPU/fuel, memory bulk/grow, core-table and host-resource pressure, stack, output/log/stderr, callback/EOF/kill races, replacement, and forbidden network/filesystem/random/clock/HTTP authorities; CI runs the Rust containment suite and TypeScript execution/failure/replacement path on Linux, macOS, and Windows.

The protocol retains the exact 10-second compile/load timeout authority, but the child deliberately does not claim to enforce it: Wasmtime 36 compilation and initial instantiation are synchronous and cannot be interrupted safely by this child-local invocation watchdog. Slice 2C's parent supervisor owns that 10-second deadline by killing and reaping the child and fencing the whole host session. Slice 2B.2 is complete at the child boundary; parent supervision, product callback/effect authority, integrated Wave 2 security review, and optimized replacement measurements remain Slices 2C–2E, so `P7-DEP-001` and the Wave 2 security gate remain open.
