...
- [x] Phase 7 Wave 0 host-placement spike protocol and preliminary campaign: temporary `tools/phase7-host-placement` probes compare protocol-only / lazy in-process / SDK-parent-plus-child with real Rust + pure TypeScript components, actual import inspection, fair warm/peak/cold selection, trap/CPU/memory/in-flight-child-crash/recovery/cleanup containment, exact byte/hash binding, cgroup self-check and quick evidence. `P7-ARCH-001`–`004` corrections are implemented. Exact Wasmtime 45.0.3; ordinary `junban-server` does not link Wasmtime. Evidence is still **not authoritative** (quick/contended, `accepted=false`). See `evidence/phase-7-host-placement-adr.md` and `evidence/phase-7-host-placement-quick.json`; Wave 1 remains blocked on the clean idle five-sample run + focused architecture recheck.
