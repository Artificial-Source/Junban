...
use junban_app::{
    ...
    PluginGraphFenceRequest, PluginHookKind, PluginInvocationDelivery,
    PluginInvocationPublicOutcome, PluginInvocationState, PluginManifestEntry,
    PluginPackageReconciliation, PluginRuntimeState, RecordPluginAttemptFailureRequest,
    ReservePluginInvocationRequest, ReservedPluginInvocation, TransitionPluginInvocationRequest,
};
...
pub struct PluginRuntimeSnapshot {
    ...
    /// Process-local fence carried by server-confirmed contributions.
    pub host_session_id: Option<OperationId>,
}
...
pub struct PluginInvocationDispatch {
    ...
    /// Required for browser-originated contribution actions. Internal delivery
    /// leaves this unset because it already owns the actor session.
    pub expected_host_session_id: Option<OperationId>,
...
impl PluginRuntimeSupervisor {
    ...
    pub async fn invoke_transient(
        ...
        self.invoke_transient_fenced(plugin_id, None, request).await
    }

    pub async fn invoke_transient_fenced(
        &self,
        plugin_id: PluginId,
        expected_host_session_id: Option<OperationId>,
        request: InvocationRequest,
    ) -> Result<PluginInvocationHandle, PluginRuntimeError> {
        if self.fenced.load(Ordering::Acquire) {
            return Err(PluginRuntimeError::Fenced);
...
impl RuntimeActor {
    ...
    async fn handle_command(&mut self, command: ActorCommand) {
        match command {
            ...
            ActorCommand::InvokeTransient {
                ...
                let result = if expected_host_session_id.is_some()
                    && expected_host_session_id != current_session
                {
    ...
    fn handle_invoke(
        ...
        if dispatch.expected_host_session_id.is_some()
            && dispatch.expected_host_session_id != OperationId::parse(&session_id).ok()
        {
            reject(PluginRuntimeError::AuthorityRejected, accepted);
            return;
        }
    ...
    fn cancel_invocation(&mut self, invocation_id: OperationId) {
            ...
            record.durable.then(|| {
                (
                    ...
                    record
                        ...
                        .clone()
                        .expect("durable record retains delivery"),
                    record.terminal.take(),
                )
            })
        };
        if let Some((operation_id, plugin_id, generation, epoch, delivery, terminal)) = completion {
        ...
        } else if let Some(mut record) = self.invocations.remove(&invocation_id) {
            self.publish_terminal(&mut record, InvocationOutcome::Cancelled);
        ...
        self.spawn_commit(
            ...
            AuthorizedCommitPluginInvocationRequest {
                request: CommitPluginInvocationRequest {
                    ...
                    package_generation: generation,
                    activation_epoch: epoch,
                    outcome: PluginInvocationPublicOutcome::Cancelled,
    ...
    async fn handle_internal(&mut self, event: InternalEvent) {
        match event {
            ...
            InternalEvent::CancellationCompletion {
                ...
                let Some(mut cancelled) = self.cancelled_reservations.remove(&invocation_id) else {
                    self.enter_fenced();
                    self.begin_drain(DrainPurpose::Fenced).await;
                    return;
                };
                match result {
                    Ok(committed)
                        if committed.outcome == PluginInvocationPublicOutcome::Cancelled =>
                    {
                        if let Some(terminal) = cancelled.terminal.take() {
                            let _ = terminal.send(InvocationOutcome::Cancelled);
                        }
                    }
                    Ok(_) | Err(_) => {
                        if let Some(terminal) = cancelled.terminal.take() {
                            let _ = terminal.send(InvocationOutcome::Failed(
        ...
        if let Some(cancelled) = self.cancelled_reservations.get(&invocation_id) {
            ...
            match result {
                ...
                Ok(ReservedPluginInvocation::TerminalReplay(committed)) => {
                    if let Some(mut cancelled) = self.cancelled_reservations.remove(&invocation_id)
                        && let Some(terminal) = cancelled.terminal.take()
                    {
                        let _ = terminal.send(InvocationOutcome::Replayed(committed));
                    }
                }
                Err(_) => {
                    if let Some(mut cancelled) = self.cancelled_reservations.remove(&invocation_id)
                ...
                AuthorizedCommitPluginInvocationRequest {
                    request: CommitPluginInvocationRequest {
                        ...
                        outcome: match cause {
                            ...
                            PluginAttemptFailureCause::ActivationFailed
                            | PluginAttemptFailureCause::InternalError => {
                                unreachable!("non-invocation failures do not terminalize receipts")
                            }
                        },
                        child_operation_id: None,
                        domain_effect: None,
