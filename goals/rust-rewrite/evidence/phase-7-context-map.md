...
- Strict parent process transport commit `cc23507` is integrated but unaccepted. Its completed security review found no material issue in the shared codec. `P7-PC-SEC-001` is rejected as a blocker because same-user control of the Junban binary/filesystem account is already outside Phase 7's threat model; supported packaging must instead enforce and verify the sibling-directory ownership invariant below. Medium `P7-PC-SEC-002` (late-exit acceptance plus busy spin) and `P7-PC-SEC-003` (discarded trailing stdout after `ShutdownComplete`) remain open with focused fixes underway. The process owner and supervisor stay blocked pending those fixes and focused recheck; current C1-head CI is still pending.
...
Packet B's current request and receipt validators require at least one `Failing` entry and permit multiple. After this plan correction receives focused planning approval and before supervisor coding, Packet B requires a small validator/test correction to admit exactly the two frozen zero-or-one shapes and reject multiple `Failing` entries. That correction is required, not implemented by this documentation-only change.
...
| `P7-PLAN-2C-005`                 | High     | fixed  | The corrected authority distinguishes a one-trigger correlated graph fence from triggerless Hello/idle/uncorrelated session loss, covers the complete selected max-16 graph, forbids multiple `Failing` entries and false plugin blame, preserves one-envelope/CAS/replay/progression authority, and keeps an empty graph `Dormant`. Packet B currently requires at least one `Failing` and permits multiple; its small validator/test correction is explicitly pending after planning approval and before supervisor coding. Focused planning recheck is pending, and no implementation is claimed. |

The original focused planning recheck approved `P7-PLAN-001`–`003`, after which Wave 0 retained the on-demand child and Wave 1 was accepted. The focused Slice 2C planning verdict at exact reviewed HEAD `442ebdd61e71a19a3e906b33b54bf7857ad555f8` remains the prior **APPROVE** authority: `P7-PLAN-2C-001`–`003`, `P7-PLAN-2C-004`, and `P7-PLAN-2C-004-EVENT-ATOMICITY` remain fixed and closed, and all prior approvals/evidence remain intact. Newly exposed `P7-PLAN-2C-005` is fixed by this written correction but awaits a focused planning recheck. After that approval and before supervisor coding, the required Packet B validator/test correction must land; this document does not claim it implemented. Slice 2C and Wave 2 remain incomplete and unaccepted, and `P7-DEP-001` plus current C1-head CI remain pending. The process-transport security review is complete: `P7-PC-SEC-001` is rejected as a blocker under the frozen same-user threat model, while medium `P7-PC-SEC-002` and `P7-PC-SEC-003` remain open; focused fixes and recheck are required before accepting the process owner or supervisor.
