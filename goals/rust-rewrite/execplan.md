...
**Status:** approved by the user on 2026-07-28 and expanded with an evidence-driven Phase 10 on 2026-07-29. Phases 0 through 6 are complete. Phase 7 capability-limited portable plugins are in progress: Waves 1 and child-runtime Slice 2B are accepted, but Slice 2C, Slice 2D, Wave 2, the supervisor, resync, and final callback composition are not accepted. Narrow Slice 2D planning recheck at exact `073f00d98dac4b9110ec028da01d0fb71eaa3ae3` returned **APPROVED**, closing `P7-PLAN-2D-001`–`004` and authorizing coding at that boundary. Delivery persistence at `d641d69ac89436eb39624ca3eeca7dd6b90d795d` passed focused database recheck: `P7-2D-DB-001` and `003` are fixed/closed; `P7-2D-DB-002` accepts the persistence portion but remains an open **REMOVAL BLOCKER** until the server migrates and every legacy unwrapped API is removed. Retained-event conversion at `ff76e01abec9fb7377210833a48e409d694b6b1b` passed focused API recheck and closed `P7-API-2D-001`–`004`. HTTPS correction `71aad742cf33ab256a70142858b720146f7dc439` passed focused security recheck and closed `P7-2D-SEC-001`; IPv6 is never usable and transport pins only validated public IPv4. The supervisor recheck closed `P7-2C-SEC-001`, `002`, `003`, and `005`, but `P7-2C-SEC-004` and `P7-2C-TEST-001` remain open, so supervisor/Slice 2C/Wave 2 acceptance is explicitly withheld. High planning finding `P7-PLAN-2D-005` remains unapproved and is fixed in plan by the exact ordinary-query authority in `evidence/phase-7-capability-matrix.md`. Query-authority review of exact `b4905b1d7afdd0ef8f95b6739eb8f5bebe34d1d1` raised subsidiary high `P7-PLAN-2D-005-UUID`, now fixed in plan with focused recheck pending; only query/callback coding is blocked on the corrected combined review, while resync implementation and supervisor fixes remain independent. At exact implementation head `01be6739364f4147e4f25bed416e5eee482a6beb`, process-memory calibration run `31332338475` passed Linux, macOS, and Windows. CI run `31332340357` passed supply chain, frontend/repository, release E2E, all three containment jobs, and Rust format/generated/clippy, but Rust tests failed only the two focused races `plugin_host_process::wrong_hello...` (EOF fixture) and `plugin_runtime::connect_hello...` (`Closed` versus `Ok` send); release build was skipped. Both races remain under focused diagnosis/fix, CI is not green, and no supervisor or phase acceptance is claimed. `P7-DEP-001`, Phases 8–10, resync completion, final callback composition, final integrated review, and current accepted CI remain open.
...
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Contract remains `evidence/phase-7-context-map.md`. Wave 0/1 planning, schema, package, WIT and host-placement gates are approved. Slice 2D planning recheck at exact `073f00d98dac4b9110ec028da01d0fb71eaa3ae3` approved and closed `P7-PLAN-2D-001`–`004`, authorizing coding at that boundary. Delivery persistence, retained-event conversion, and public-IPv4-only HTTPS corrections passed their focused rechecks at exact `d641d69`, `ff76e01`, and `71aad74`; `P7-2D-DB-001`/`003`, `P7-API-2D-001`–`004`, and `P7-2D-SEC-001` are closed, while `P7-2D-DB-002` remains an open server migration/removal blocker. A later high planning gap, `P7-PLAN-2D-005`, remains unapproved and fixed in plan by `evidence/phase-7-capability-matrix.md`; subsidiary high `P7-PLAN-2D-005-UUID` corrects its UUID rule and is fixed in plan with focused recheck pending. Query/callback coding is paused only on the corrected combined review, while resync and supervisor fixes remain independent. The supervisor recheck closed `P7-2C-SEC-001`/`002`/`003`/`005` but left `P7-2C-SEC-004` and `P7-2C-TEST-001` open. Exact-head calibration at `01be673` is green, but CI `31332340357` is not green because two Rust race tests failed and release build was skipped. Supervisor, Slice 2C, Slice 2D, Wave 2, resync, final callback composition, integrated evidence and `P7-DEP-001` remain open.
  ...
  - [ ] `P7-PLAN-2D-005` ordinary-query planning gate: internal AppService/repository/one-page SQLite transaction ownership, exact filters, authenticated five-minute cursor, profile-private MAC reuse, canonical SDK reply-byte ceiling, and focused tests are frozen in `evidence/phase-7-capability-matrix.md`. Subsidiary high `P7-PLAN-2D-005-UUID` corrects both 16-byte UUID fields to preserve existing canonical parse/round-trip authority and adds nil/non-RFC keyset and event-epoch regressions without changing cursor layout or MAC behavior. Both findings are fixed in plan with focused recheck pending; `P7-PLAN-2D-005` remains unapproved. Query/callback coding is blocked only on the corrected combined review; resync and supervisor fixes remain independent.
  - [ ] Wave 2 Slice 2C Packet C through Slice 2E: the supervisor recheck closed `P7-2C-SEC-001`, `002`, `003`, and `005`, but `P7-2C-SEC-004` and `P7-2C-TEST-001` remain open, so the supervisor, Slice 2C, and Wave 2 are not accepted. Resync implementation and final callback composition are in progress and unaccepted. At exact `01be6739364f4147e4f25bed416e5eee482a6beb`, process-memory calibration `31332338475` passed every platform. CI `31332340357` passed all non-Rust jobs, containment, and Rust format/generated/clippy, but two focused Rust race tests failed and release build was skipped; diagnosis/fix remains open and CI green is not claimed. Parent supervision/callback completion, server removal blocker, integrated security review, optimized replacement measurements, and `P7-DEP-001` remain.
...
- `P7-PLAN-2D-005` — **high/fixed in plan; focused recheck pending; unapproved**. The WIT query shapes were frozen, but ordinary internal AppService/SQLite query semantics, cursor authentication/private-key ownership, complete SDK reply-byte accounting, and focused tests were underspecified. `evidence/phase-7-capability-matrix.md` now freezes the minimal internal authority without WIT/schema/public/dependency changes.
- `P7-PLAN-2D-005-UUID` — **high/fixed in plan; focused recheck pending**. Review of exact `b4905b1d7afdd0ef8f95b6739eb8f5bebe34d1d1` found the cursor-v1 plan wrongly rejected non-RFC UUID variants. The corrected authority preserves any UUID accepted by existing canonical parse/round-trip rules in both exact 16-byte fields, requires nil/non-RFC resource-keyset and event-epoch regressions, and leaves the 122-byte layout and tamper/MAC behavior unchanged. Query/callback coding remains blocked on one focused recheck of `P7-PLAN-2D-005` and this subsidiary; resync and supervisor fixes remain independent.
...
- Supervisor recheck — `P7-2C-SEC-001`, `002`, `003`, and `005` are fixed/closed; `P7-2C-SEC-004` and `P7-2C-TEST-001` remain open. Supervisor, Slice 2C, and Wave 2 acceptance is withheld. Exact-head calibration `31332338475` passed, while CI `31332340357` is not green because two Rust race tests failed and release build was skipped.
...
- 2026-08-09: Supervisor recheck closed `P7-2C-SEC-001`/`002`/`003`/`005` but left `P7-2C-SEC-004` and `P7-2C-TEST-001` open; no supervisor/Slice 2C/Wave 2 acceptance follows. At exact `01be673`, cross-platform process-memory calibration passed, but current CI is not green because two focused Rust races failed and the release build was skipped.
- 2026-08-09: Newly discovered high `P7-PLAN-2D-005` found the existing WIT ordinary-query implementation authority incomplete. `evidence/phase-7-capability-matrix.md` fixes the plan by freezing the internal one-transaction AppService/repository query path, exact filters and keyset pages, authenticated five-minute cursor using the existing profile-private key, canonical successful SDK-reply byte limit, non-change boundary, and focused test matrix. Query-authority review of exact `b4905b1d7afdd0ef8f95b6739eb8f5bebe34d1d1` then raised high `P7-PLAN-2D-005-UUID`; the plan now preserves nil and non-RFC UUID values through both exact 16-byte fields and requires focused keyset/event-epoch regressions while retaining the 122-byte layout and unchanged tamper/MAC behavior. Both are fixed in plan with focused recheck pending, `P7-PLAN-2D-005` remains unapproved, query/callback coding stays blocked, and resync/supervisor status is unchanged.
