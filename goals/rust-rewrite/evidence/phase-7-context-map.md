...
Date: 2026-08-04
Status: Wave 0 and Wave 1 accepted; Wave 2 child-runtime security recheck accepted; Slice 2C planning corrections fixed in writing and pending focused planning recheck
...
- `junban-plugin-sdk` owns bounded package/WIT/trust/component/protocol authority without Wasmtime, storage or runtime construction. The isolated `junban-plugin-host` child now owns strict protocol I/O, one Engine and one load attempt, an actual-import/grant-selective linker, one limited mutable Store/instance owner, typed callback transport, serial execution of all nine guest exports, exact invocation watchdog interruption, active drain, failed-Store destruction and clean replacement. That one-load shape is an implemented Slice 2B checkpoint, not the accepted product topology: Slice 2C must upgrade it before parent composition to the bounded multi-runtime child defined below. Parent supervision and capability/effect execution remain Slices 2C–2D.
...
- persisted monotonic `activation_epoch` identifies one runtime admission generation, not every health-state change. Enable, manual retry and a due automatic retry each increment it exactly once while entering `starting`; disable, grant/revoke, package, restore and other authority changes that invalidate admitted work also advance it once. Child/session-fatal failure advances every affected plugin epoch atomically before replacement. `loaded`, same-attempt `starting`→`active`, and same-attempt failure-state changes never increment it again.
...
Authority transitions acquire the plugin reconfiguration mutex, close admission for the exact activation epoch, cancel and drain its invocations, then commit the package/grant/desired-state change and next epoch before changing the child instance. If the commit fails after a complete drain, unchanged old authority resumes admission. Timeout/partial drain remains fail-closed until bounded recovery. Enable or retry marks the one new epoch `starting`, loads the exact package/grants/dependencies, completes required resync/catch-up, then same-epoch CASes `active`. A failed attempt same-epoch CASes to its next bounded `degraded`/`failed`/`suspended` state without reopening stale authority; suspension clears desired enablement. A child crash ends its host session, discards all late replies, and transactionally advances affected activation epochs and graph health before any replacement spawn.
...
Slice 2C bumps the private host protocol from v1 to **v2** before parent composition. The exact protocol name/version are `junban-plugin-host-v2` and `2`. Parent `Hello` and child `Hello` reply both carry the exact compiled `env!("CARGO_PKG_VERSION")` Junban product-version string as `junban_version` in addition to the protocol identity and fresh host session. The parent sends no component bytes until it has exact-matched the complete reply; protocol-name, protocol-version, product-version or session mismatch is child/session-fatal, with no v1 fallback or compatibility negotiation. Canonical frame goldens cover both Hello directions and every mismatch.
...
- a plugin-local trap, invocation timeout or plugin resource failure destroys and replaces only that plugin's Store/instance from its retained Component/Linker. Sibling plugin entries and the child process survive;
- transport loss, process exit, malformed/stale protocol, or compile/load failure is child/session-fatal: close all admission, kill and reap if needed, invalidate the session, and discard every late frame before any replacement;
- last disable, restore/recovery drain, and graceful server shutdown terminate the child and verify no orphan on Linux, macOS and Windows;
...
Enable, manual retry and a due automatic retry each perform one transactional epoch increment into `starting`. A successful child `Loaded` reply is runtime-local evidence, not `active`. Required resync and retained-event catch-up complete first; only then may AppService same-epoch CAS `starting`→`active`. A failed activation attempt records the next persisted progression `degraded`→`failed`→`suspended` across consecutive attempts; the terminal suspension clears `desired_enabled`. Those health changes use the retry attempt's epoch rather than incrementing once per state, and every material transition receives its exact bounded receipt and event while counter-only bookkeeping remains revision-neutral.

Child/session-fatal failure first closes admission and kills/reaps as required, then one AppService transaction invalidates the session effects by advancing every affected plugin epoch and recording the bounded graph-health transitions before a replacement child can spawn. Dependency-first reactivation skips a dependent whose prerequisite did not become active and records its bounded `dependency_failed` transition, receipt and event; it never reports the skipped plugin as loaded or active. A later due retry enters a fresh `starting` epoch under the ordinary rule.
...
- Slice 2C minimum lifecycle/process matrix: one PID loads a dependency graph; 16 runtimes load and a 17th fails closed; four concurrent invocations run while a fifth, a same-plugin second invocation and a saturated nested dependency invoke fail immediately/bounded in both parent and child; a sibling survives a plugin-local trap; child loss fences the complete graph before replacement; compile/load timeout kills and reaps; stale frames cannot cross the session fence; product host discovery rejects PATH/override, relative, missing, wrong-type, non-executable, symlink/reparse and wrong protocol/product-version candidates; exact starting/active/degraded/failed/suspended transitions, receipts, events and `dependency_failed` propagation match SQLite authority; shutdown and failure leave no orphan on Linux, macOS or Windows;
...
- Host crash never changes task data. Runtime failure counters/cursors/KV remain plugin-local and cannot create a recursive global event stream. Admission closes and the child is killed/reaped before one AppService transaction advances every affected activation epoch and records bounded graph-health/dependency transitions, receipts and material events. Replacement starts only afterward, re-verifies exact package-generation/grants/graph, and discards late replies; repeated failure follows `degraded`→`failed`→`suspended` without guessing.
...
- Discover only the strict `current_exe()` sibling and require private protocol v2 plus exact product-version handshake; reject PATH/config/environment overrides and v1 fallback.
...
| `P7-PLAN-2C-004` | High     | fixed  | SQLite/AppService exclusively owns desired state, epochs, health, retry/backoff, propagation, events and receipts. One retry epoch enters `starting`; `Loaded` is not active; resync/catch-up precedes same-epoch active CAS; failures progress boundedly to suspension; child loss atomically fences graph health/epochs before replacement and skipped dependents receive `dependency_failed`.                                                       |

The original focused planning recheck approved `P7-PLAN-001`–`003`, after which Wave 0 retained the on-demand child and Wave 1 was accepted. The written Slice 2C corrections mark `P7-PLAN-2C-001`–`004` fixed, but their focused planning recheck verdict is still pending. Separately, the approved hostcall-fuel security recheck fixes `P7-PLAN-RUNTIME-001` and `P7-RUNTIME-SEC-001` and authorizes Slice 2C from that security gate. That approval does not waive the pending planning recheck, the open `P7-DEP-001`, or the requirement that the current macOS containment CI failure pass before Wave 2 acceptance.
