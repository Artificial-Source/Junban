...
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Contract remains `evidence/phase-7-context-map.md`. Wave 0/1 planning, schema, package, WIT and host-placement gates are approved; the exact-HEAD Slice 2C planning verdict is **APPROVE** with no blockers, and Slice 2C is authorized to implement from planning and security.
  ...
  - [ ] Wave 2 Slice 2C Packet A child: private protocol v2/product-version fencing, one Engine with at most 16 activation-fenced runtimes, independent per-plugin/four-total child admission and per-plugin replacement are integrated at `651cf75`. Packet A is not accepted; hostile-runtime/security review remains pending.
  - [ ] Wave 2 Slice 2C Packet C through Slice 2E: parent supervisor/verified source and compile-time kill/reap, capability/effect/dependency/event authority, integrated security review and optimized replacement measurements remain. Slice 2E may use one non-shipped real-supervisor/storage/AppService/sibling-host harness for separate Rust/TypeScript and one/four/sixteen-runtime replacement evidence only; Wave 3 ordinary `ServerState` composition and Wave 5 product-integrated evidence remain separate. Replacement CI run `31296663305` and process-memory calibration run `31296660062` passed at exact head `ce80dead79c5ec2dadd95bddb8202ee7dbd2bf46`; all three strict exact-head macOS containment executions (`93202756069`, `93205509663`, `93207040552`) passed. Those runs close the old macOS and replacement-CI gates at `ce80dea` but predate Packet A/B integration, so current integrated-head CI green, Slice 2C acceptance and Wave 2 acceptance are not claimed. `P7-DEP-001` remains open.
...
- 2026-08-09: Slice 2C Packet B lifecycle authority is integrated at `ae9cd24`; its focused database recheck approved exact `e573cfe` with no material issues, fixed/closed high findings `P7-2C-B-DB-001` and `P7-2C-B-DB-002`, left schema v7 unchanged, and authorized Packet C to consume those APIs. Packet A protocol-v2/multi-runtime child authority is integrated at `651cf75` but remains unaccepted while hostile-runtime/security review is pending. Slice 2C is not accepted, and `P7-DEP-001` remains open.
