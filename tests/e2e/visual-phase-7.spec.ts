/**
 * Phase 7 immutable visual suite.
 * Compares rewrite rendering against legacy-captured PNGs.
...
const SNAPSHOT_DIR = join(import.meta.dirname, "visual-phase-7.spec.ts-snapshots");
const PLATFORM =
...
test.beforeAll(async () => {
  ...
  for (const scene of manifest.scenes) {
    ...
    const dest = join(SNAPSHOT_DIR, `${scene.id}-visual-phase-7-${PLATFORM}.png`);
...
async function openScene(page: Page, scene: ManifestScene) {
  ...
  const path = `/?visual-fixture=phase-7&scene=${encodeURIComponent(scene.id)}`;
...
for (const scene of manifest.scenes) {
  test(`phase-7 visual: ${scene.id}`, async ({ page }) => {
    await openScene(page, scene);
    await expect(page.getByTestId("phase7-scene-root")).toHaveScreenshot(`${scene.id}.png`, {
      ...SCREENSHOT_OPTS,
    });
  });
}
