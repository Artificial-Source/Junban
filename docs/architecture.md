...
The accepted runtime placement is the on-demand `junban-plugin-host` child. Exact `wasmtime` and `wasmtime-wasi` 36.0.13 remain confined to that binary crate. Slice 2B.1 constructs one Engine and permits one load attempt, re-inspects actual imports against the runtime profile and canonical grants, selectively adds only those Junban interfaces, and individually links only Rust's exact five frozen WASI 0.2.6 interfaces. The Rust baseline has empty environment/arguments, closed stdin, absent stdout, bounded sink stderr, and no WASI random, clock, network, filesystem or broad WASI linkage; TypeScript has zero WASI. One serial runtime thread owns the limited mutable Store/instance, retains it across successful calls, runs all nine generated-adapter export paths, and rendezvouses canonical typed callbacks with the concurrently live protocol reader. Failed guest execution discards the Store; idle unload drops it. Slice 2B.2 still owns wall-deadline/cancellation interruption, hostile exhaustion and clean replacement/recovery. Default `junban-server` still touches only the SDK's static authority; `--no-default-features` remains the matched feature-off baseline, and neither server build links Wasmtime. The completed Wave 0 probe crate, harness, and Wasmtime advisory exception remain removed.
