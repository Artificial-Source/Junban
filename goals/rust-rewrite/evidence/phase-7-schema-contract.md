...
Resync closes that plugin's event admission and atomically samples global event epoch `E` plus head revision `R`; it does not hold a SQLite read transaction while guest code runs. A fresh runtime-local resync session receives count/byte-bounded task/project/tag snapshot pages in canonical kind/ID keyset order, selected as `id > last_id AND row_revision <= R`. Rows changed or deleted after `R` are supplied by the retained typed events `> R`, so offset pagination and a materialized unbounded snapshot are unnecessary. Each snapshot outcome acknowledges only its exact page and may add a noncommitted sorted unique KV segment whose full lifted outcome is ≤256 KiB; the runtime buffers unique keys to the existing ≤256-key/2-MiB aggregate. Under the storage grant, resync may list/read all isolated old KV so unrelated state can be preserved. After the exhaustive data page, up to nine host-driven ordered flush requests allow additional nonempty segments within the same aggregate, then one finalize request chooses leave-KV or replace-with-staged-segments (including intentional empty replacement). One final transaction verifies exact old cursor/session identity, epoch `E`, ordered exhaustive snapshot/flush/finalize acknowledgements, and retained tail after `R`, then atomically performs that KV choice and sets the cursor to `(E, R, false)`. Catch-up applies only retained events `> R` before live admission reopens. Epoch change, tail loss, request/session mismatch, trap/restart or CAS race abandons guest state/staged KV and restarts; repeated inability suspends rather than skipping. Resync denies HTTP, domain effects and dependency service calls.
