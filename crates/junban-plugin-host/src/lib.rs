...
// Two workers keep the largest TypeScript component's compilation peak below
// its frozen cgroup budget while retaining bounded parallel compilation.
const PARALLEL_COMPILATION_WORKERS: usize = 2;
const PARALLEL_COMPILATION_STACK_BYTES: usize = 1024 * 1024;
const OUTBOUND_CHANNEL_CAPACITY: usize = 8;

#[derive(Clone, Copy)]
struct ParallelCompilationPoolError;

static PARALLEL_COMPILATION_POOL: OnceLock<Result<(), ParallelCompilationPoolError>> =
    OnceLock::new();
...
fn initialize_parallel_compilation_pool() -> Result<(), HostError> {
    match PARALLEL_COMPILATION_POOL.get_or_init(|| {
        ThreadPoolBuilder::new()
            .num_threads(PARALLEL_COMPILATION_WORKERS)
            .stack_size(PARALLEL_COMPILATION_STACK_BYTES)
            .thread_name(|index| format!("junban-plugin-compiler-{index}"))
            .build_global()
            .map_err(|_| ParallelCompilationPoolError)
    }) {
        Ok(()) => Ok(()),
        Err(ParallelCompilationPoolError) => Err(HostError::Engine),
    }
}

fn build_engine() -> Result<Engine, HostError> {
    initialize_parallel_compilation_pool()?;
    ...
    Engine::new(&config).map_err(|_| HostError::Engine)
}
...
mod tests {
    ...
    #[test]
    fn parallel_compilation_pool_has_frozen_bounds() {
        assert_eq!(PARALLEL_COMPILATION_WORKERS, 2);
        assert_eq!(PARALLEL_COMPILATION_STACK_BYTES, 1024 * 1024);
        initialize_parallel_compilation_pool().unwrap();
        assert_eq!(rayon::current_num_threads(), PARALLEL_COMPILATION_WORKERS);
    }

    #[test]
    fn parallel_compilation_pool_initialization_is_concurrently_reusable() {
        std::thread::scope(|scope| {
            let handles = (0..8)
                .map(|_| scope.spawn(initialize_parallel_compilation_pool))
                .collect::<Vec<_>>();
    ...
    #[test]
    fn engine_build_succeeds_with_the_bounded_compilation_pool() {
