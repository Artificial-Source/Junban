...
impl VerifiedRegistry {
    #[must_use]
    pub const fn index(&self) -> &RegistryIndex {
        &self.index
    }
...
impl RegistryIndex {
    /// Validate all schema, ordering, identity, presentation, and size fields.
    pub fn validate(&self) -> Result<()> {
        validate_index(self)
    }

    /// Serialize this validated index with the sole fixed-order JRI1 JSON authority.
    pub fn canonical_bytes(&self) -> Result<Vec<u8>> {
        self.validate()?;
        let bytes = serde_json::to_vec(self).map_err(|_| SdkError::CanonicalJson)?;
        if bytes.is_empty() || bytes.len() > REGISTRY_INDEX_BYTES_MAX {
    ...
    /// Parse a bounded index and require exact fixed-order canonical bytes.
    pub fn parse_canonical(bytes: &[u8]) -> Result<Self> {
        ...
        let index = serde_json::from_slice(bytes).map_err(|_| SdkError::CanonicalJson)?;
        Self::validate(&index)?;
        if serde_json::to_vec(&index).map_err(|_| SdkError::CanonicalJson)? != bytes {
...
/// Deterministically serialize and sign one JRI1 registry envelope.
///
/// The signature covers the existing domain-separated hash of the exact raw
/// canonical index bytes. The index root key identifier must match the signing
/// key; the completed envelope is immediately verified before it is returned.
pub fn pack_registry(index: &RegistryIndex, signing_key: &SigningKey) -> Result<Vec<u8>> {
    let index_bytes = index.canonical_bytes()?;
    let public_key = signing_key.verifying_key().to_bytes();
    if decode_hex_32(&index.root_key_id, "root_key_id")? != sha256(&public_key) {
    ...
    if capacity > REGISTRY_ENVELOPE_BYTES_MAX {
        return Err(SdkError::Length { field: "registry" });
    }
    let mut envelope = Vec::with_capacity(capacity);
    envelope.extend_from_slice(JRI1_MAGIC);
    envelope.extend_from_slice(
        &u32::try_from(index_bytes.len())
    ...
    let verified = parse_and_verify_registry(&envelope, &public_key)?;
...
/// Build one registry entry by deriving every package-bound field from an
/// already verified JBP1 package. Only sorted presentation search tags remain
/// caller supplied.
pub fn registry_entry_from_verified_package(
    ...
    let entry = RegistryEntry {
        ...
        author: manifest.publisher.name.clone(),
        ...
        requested_capabilities: manifest
            ...
            .collect(),
        filename: registry_package_path(&package.identities.package_sha256)?,
    };
    validate_entry(&entry)?;
    validate_registry_package_agreement(&entry, package)?;
    Ok(entry)
}
...
pub fn parse_and_verify_registry(
    ...
    let index = RegistryIndex::parse_canonical(index_bytes)?;
...
fn validate_index(index: &RegistryIndex) -> Result<()> {
    ...
    for entry in &index.entries {
        let version = validate_entry(entry)?;
...
fn validate_entry(entry: &RegistryEntry) -> Result<semver::Version> {
