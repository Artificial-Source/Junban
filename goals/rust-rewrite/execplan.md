...
- SQLite/AppService-owned desired state, epochs, health, retry/backoff, dependency propagation, events and receipts; `Loaded` precedes required resync/catch-up and same-epoch activation CAS;
...
- plugin-local trap/timeout/resource failure replaces only that plugin Store while transport/process/protocol/compile-load failure durably fences the graph before one child/session replacement;
- one PID loads the dependency graph; 16/17 runtime bounds, four/fifth/same-plugin/nested saturation, stale frames, compile/load reap, discovery rejection, exact health/receipt/event/dependency transitions, sibling survival and Linux/macOS/Windows no-orphan behavior pass;
...
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Contract remains `evidence/phase-7-context-map.md`. Planning, schema, package, WIT and host-placement gates are approved.
  ...
  - [ ] Wave 2 Slice 2C corrected planning recheck: `P7-PLAN-2C-001`–`004` are fixed in the written authority. Exactly one child must upgrade the implemented one-load checkpoint to one Engine/at most 16 serialized activation-fenced runtimes; parent and child independently enforce one/plugin and four total including nested invokes; product discovery is only the strict `current_exe()` sibling; private protocol v2 exact-matches Junban product version; and SQLite/AppService exclusively owns lifecycle/health/epoch/retry/dependency/event/receipt transitions. Focused planning verdict remains pending before implementation.
...
- Phase 7 Slice 2C planning review — **pending focused recheck after written fixes**. `P7-PLAN-2C-001`–`004` now freeze the one-child/16-runtime topology and dual admission, exact sibling discovery and v2/product-version handshake, the bounded real-owner Slice 2E harness, and exclusive SQLite/AppService lifecycle authority. Fixed status records the correction; it is not an approval verdict.
- Phase 7 hostcall-fuel security recheck — **approved after fixes**. The recheck closes `P7-PLAN-RUNTIME-001` and `P7-RUNTIME-SEC-001` and authorizes Slice 2C from that security gate. `P7-DEP-001`, current macOS containment CI run `31036645108`, corrected planning recheck, parent compile/load reap, and integrated evidence remain open.
