...
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Contract remains `evidence/phase-7-context-map.md`. Wave 0/1 planning, schema, package, WIT and host-placement gates are approved. All Slice 2C planning findings through `P7-PLAN-2C-005` are approved/fixed/closed, and the Packet B zero-or-exactly-one follow-up is approved for supervisor consumption. The process owner at exact `ddacfa2` and wakeable runtime driver at exact `c9474bc` are accepted for supervisor consumption. Exact pre-supervisor head `c9474bc` has passed CI and process-memory calibration. Final supervisor-packet implementation, final-supervisor current-head CI, Slice 2C/Wave 2 acceptance and `P7-DEP-001` remain open. Slice 2D findings `P7-PLAN-2D-001`–`004` are fixed in plan but pending focused API/planning recheck; no Slice 2D coding is authorized.
  ...
  - [x] Wave 2 Slice 2D authority-only planning correction: high findings `P7-PLAN-2D-001`–`004` are fixed in the written plan. The correction freezes runtime-local nonserialized `PluginDeliveryAuthority` with domain-separated binding into existing `request_hash` and exact starting/active mode lifecycle; a bounded nonpersisted rolling `PluginResyncTranscript`, mandatory Task→Project→Tag pages, exact flush-index-0-through-9/one-complete/finalize/KV digest and one final revalidating KV/cursor/invocation transaction using real Task revision and Project/Tag retained-tail predicates; exact HTTPS request/response header allowlists and metadata bounds, forbidden/transport-owned headers, zero/one durable pre-send callback, identical same-memory resend only and unresolved process loss; exhaustive sixteen-kind direct event conversion, cursor-only skips, no live synthesis, atomic event result+cursor, and root-0/eighth-edge-allowed/ninth-rejected service depth separate from activation depth 16. No WIT/WIT SHA/generated body, JBP1/JRI1/package hash, OpenAPI/DTO, schema SQL/version/table shape, or dependency shape changes.
  - [ ] Wave 2 Slice 2D focused API/planning recheck gate: one focused reviewer must approve `P7-PLAN-2D-001`–`004` and their exact named delivery, resync, HTTP, event/cursor, and service-depth tests. Until the verdict is recorded, all four findings remain fixed-in-plan/recheck-pending rather than closed, and Slice 2D coding remains blocked. The final Slice 2C supervisor packet remains independently in progress, unaccepted and uncomposed.
  - [ ] Wave 2 Slice 2C Packet C through Slice 2E: Packet C, Slice 2C and Wave 2 remain unaccepted and incomplete. Final supervisor-packet implementation is now in progress but is not accepted or composed. Parent supervision and compile/load graph-fence wiring, the pending Slice 2D API/planning recheck and later capability/effect/dependency/HTTP/event implementation, final-supervisor current-head CI, integrated security review and optimized replacement measurements remain. The exact 1,000-ms parent control deadline remains frozen at docs commit `18fbb87`. Slice 2E may use one non-shipped real-supervisor/storage/AppService/sibling-host harness for separate Rust/TypeScript and one/four/sixteen-runtime replacement evidence only; Wave 3 ordinary `ServerState` composition and Wave 5 product-integrated evidence remain separate. Replacement CI run `31296663305` and process-memory calibration run `31296660062` passed at exact head `ce80dead79c5ec2dadd95bddb8202ee7dbd2bf46`; all three strict exact-head macOS containment executions (`93202756069`, `93205509663`, `93207040552`) passed. Integrated Packet A/B CI and process-memory calibration passed later at accepted exact `0f2f9b31633470dc75973fb15aef99ade354eafb`. CI run `31303724227` at exact `2c8affb028af27372437be0f8827f0f375029e16` passed every non-Rust job; its Rust job completed workspace tests and release-binary compilation before GitHub cancelled it at the former 20-minute job limit prior to step completion. This is a CI time-budget interruption, not a product or test failure. Commit `4fbd1001a3496d8012f8e45586d478b033d38a68` raises only the Rust job timeout to a bounded 30 minutes. Exact pre-supervisor head `c9474bc` later passed its remote gate; no final-supervisor current-head CI, Slice 2C acceptance or Wave 2 acceptance is claimed. `P7-DEP-001` remains open.
...
- `P7-PLAN-2D-001` — **high/fixed in plan; focused recheck pending**. Runtime-local nonserialized `PluginDeliveryAuthority` now binds plugin/package/epoch/current-session/invocation/payload/mode by domain-separated digest into existing `request_hash` at reserve/transition/terminal/verified-skip/resync-finalize. Starting modes are narrowly restricted, starting process loss abandons exact rows for a fresh session without HTTP ambiguity, and catch-up reaches head before activation. Focused hash/mode/recovery/order tests and reviewer approval remain required; Slice 2D coding is blocked.
- `P7-PLAN-2D-002` — **high/fixed in plan; focused recheck pending**. Bounded nonpersisted `PluginResyncTranscript` now proves exact fence/session, global Task→Project→Tag pages, after IDs, terminal marker, contiguous flush indices 0 through 9 ending in one complete and followed by one finalize, sorted unique staged-KV digest, and one transcript/session/invocation/KV/epoch/tail/cursor final transaction. Tasks use `revision <= R`; Project/Tag retain their existing tail anti-join; no schema row/migration is added. Focused transcript/crash tests and reviewer approval remain required.
- `P7-PLAN-2D-003` — **high/fixed in plan; focused recheck pending**. HTTPS now has exact request/response header allowlists, lowercase-token/value/count/aggregate rules, forbidden and transport-owned headers, invalid-versus-omitted response metadata, one consume-once callback with durable pre-send `DispatchingHttp`, identical in-memory ambiguity resend only, and unresolved process loss with no guest rerun. Focused boundary/no-send/restart tests and reviewer approval remain required.
- `P7-PLAN-2D-004` — **high/fixed in plan; focused recheck pending**. Event conversion now directly admits only seven Task and create/update/delete Project/Tag/Section kinds with exact retained primary/snapshot/delete/revision rules; all named other/unsubscribed kinds are cursor-only, malformed direct fails closed, and no live read/affected synthesis exists. Exact event hash/revision/cursor reservation, skip-stop, atomic effect/KV/HTTP terminal+cursor, root-0/eighth-service-edge-allowed/ninth-rejected depth, and independent activation depth 16 are frozen. Exhaustive converter/atomicity/depth tests and reviewer approval remain required.
...
- 2026-08-09: Slice 2D authority-only correction records high `P7-PLAN-2D-001`–`004` as fixed in plan and pending focused API/planning recheck. It freezes exact runtime delivery authority/startup recovery, bounded resync transcript/final transaction, HTTPS metadata/cardinality/ambiguity, exhaustive event conversion/cursor atomicity, service depth, no-shape-change boundaries, and required tests. Slice 2D coding remains blocked until recheck; final Slice 2C supervisor work remains independently in progress and unaccepted.
