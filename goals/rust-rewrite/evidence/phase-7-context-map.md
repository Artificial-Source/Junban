...
Focused Slice 2D API/planning review at exact reviewed HEAD `08c42c5b44411fdb4b81545c993a60a80960682d` returned **REVISE** overall: HTTP finding `P7-PLAN-2D-003` is fixed and closed, while `P7-PLAN-2D-001`, `002`, and `004` required the corrections below. Those three are fixed in plan but pending one narrow focused recheck. No Slice 2D implementation starts before that recheck; the final Slice 2C supervisor is independently still in progress, unaccepted, and uncomposed.
...
4. Wave 2 security gate for package trust, selective linker, capability enforcement, resources, IPC, effects, crash containment, and hostile matrix. Focused Slice 2D review at exact `08c42c5b44411fdb4b81545c993a60a80960682d` fixed/closed `P7-PLAN-2D-003` and returned **REVISE** overall; before coding, one narrow recheck must approve fixed-in-plan `P7-PLAN-2D-001`, `002`, and `004`.
...
| `P7-PLAN-2D-001`                 | High     | fixed-in-plan; narrow recheck pending | Review at exact `08c42c5b44411fdb4b81545c993a60a80960682d` found the prior delivery digest did not close hook payload/final persisted request hashing and abandoned only Starting non-HTTP rows. The revision freezes exact per-hook payload digests, canonical-body equality, final v2 hook-tag/persisted-entry-ID/authority framing, all-stage matching, and process/session-loss abandonment of every Starting or Active non-HTTP row while only HTTP dispatch becomes/stays ambiguous.                                                                                                                                                                                                                                                                                                                                                      |
| `P7-PLAN-2D-002`                 | High     | fixed-in-plan; narrow recheck pending | Review at exact `08c42c5b44411fdb4b81545c993a60a80960682d` found the transcript fold and replacement semantics incomplete. The revision freezes the exact initial digest and per-step domain/`u32be` index/tag/request/outcome framing with canonical equality, compact semantic counters, SET-only globally sorted/unique replacement candidates, exact resulting-map digest, omission deletion, leave/replace/zero-replace behavior, and the existing one final transaction/no-row boundary.                                                                                                                                                                                                                                                                                                                                                  |
| `P7-PLAN-2D-003`                 | High     | fixed and closed                      | Exact HTTP allowlists/bounds, forbidden/transport-owned headers, consume-once callback, durable pre-send transition, identical same-memory resend, and unresolved process loss were accepted by the focused review at exact `08c42c5b44411fdb4b81545c993a60a80960682d`; this finding is fixed and closed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `P7-PLAN-2D-004`                 | High     | fixed-in-plan; narrow recheck pending | Review at exact `08c42c5b44411fdb4b81545c993a60a80960682d` found cursor-only treatment could skip baseline rows omitted by concurrent resync queries. The revision freezes exact affected-ID/subscription-based `Represented`/`Irrelevant`/`Invalidating` classification, restart on every invalidating tail/catch-up event, direct-event fail-closed rules, atomic event result/cursor, and the already-correct service-depth authority.                                                                                                                                                                                                                                                                                                                                                                                                       |

The original focused planning recheck approved `P7-PLAN-001`–`003`, after which Wave 0 retained the on-demand child and Wave 1 was accepted. The focused Slice 2C planning verdict at exact reviewed HEAD `442ebdd61e71a19a3e906b33b54bf7857ad555f8` remains the prior **APPROVE** authority: `P7-PLAN-2C-001`–`003`, `P7-PLAN-2C-004`, and `P7-PLAN-2C-004-EVENT-ATOMICITY` remain fixed and closed, and all prior approvals/evidence remain intact. Focused planning recheck at exact `f6183fd9ffdbe5879600cf7c071157e78e615ebb` subsequently approved and closed `P7-PLAN-2C-005`; focused database review approved its Packet B implementation at exact `656e8431956dd3aebf7dc86029e5130beff930cd` with no material issues and authorized supervisor consumption. Process corrections at exact `ddacfa2d65c4b4a373f8165f20033bad1ffbfc2f` likewise passed focused security recheck with **APPROVE**, and `P7-PC-SEC-002` and `P7-PC-SEC-003` remain fixed/closed while `P7-PC-SEC-001` remains rejected under the explicit same-user/install-directory threat model. Wakeable driver commit `6d0e94d` plus final CAS correction `c9474bc` passed focused security recheck with **APPROVE**; medium `P7-WAKE-SEC-001` is fixed/closed and the driver is authorized for supervisor consumption. CI run `31303724227` at `2c8affb` was interrupted only by the Rust job's former 20-minute limit after tests and release-binary compilation; `4fbd100` raises only that timeout to 30 minutes. Exact pre-supervisor driver head `c9474bc` subsequently passed CI `31310661503` and process-memory calibration `31310658785`, proving the codec/source/process/storage/driver integration boundary without expanding the focused reviews. Final supervisor-packet implementation and its current-head CI remain pending and are not accepted or composed; Slice 2C, Wave 2 and `P7-DEP-001` remain open. Focused Slice 2D review at exact `08c42c5b44411fdb4b81545c993a60a80960682d` returned **REVISE** overall: it fixed/closed `P7-PLAN-2D-003`, while the now-written corrections for `P7-PLAN-2D-001`, `002`, and `004` remain pending one narrow recheck. Slice 2D coding remains blocked.
