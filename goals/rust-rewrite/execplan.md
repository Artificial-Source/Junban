...
- Phase 7 process-transport security review — initial transport `cc23507` had no material codec issue. `P7-PC-SEC-001` remains **high/rejected**, not fixed: same-user control of the Junban binary/filesystem account is outside the frozen threat model, so supported packaging instead must verify the server/host sibling directory is writable only by the installation owner or administrator/elevated authority; a less-privileged writer is an unsupported insecure installation with fail/stop guidance, not an in-scope attacker, and descriptor-bound execution is not claimed. Corrections at exact `ddacfa2d65c4b4a373f8165f20033bad1ffbfc2f` passed focused security recheck with **APPROVE**, no material issues and all 18 tests. Strict pre/post deadline checks and bounded 10-ms polling place send/acknowledgement/exit/EOF under one 1,000-ms deadline; every failure kills, waits/reaps and joins; success requires timely zero exit plus clean EOF; and trailing, partial-body, body-bearing and malformed output are fatal. `P7-PC-SEC-002` and `P7-PC-SEC-003` are **medium/fixed and closed**, and the process owner is accepted for serialized supervisor consumption. The wakeable runtime-driver adaptation is underway because this synchronous API cannot itself multiplex, but it is not implemented or reviewed; supervisor acceptance and current-head CI remain pending.
...
- 2026-08-09: Initial process transport commit `cc23507` completed focused security review with no material codec issue. `P7-PC-SEC-001` remains rejected under the existing same-user binary/filesystem threat-model exclusion; supported Phase 8/9 packaging must verify owner/mode/ACL on the server/host sibling directory, and a less-privileged-writable installation is unsupported/insecure with fail/stop guidance. No descriptor-bound execution is claimed. Corrections at exact `ddacfa2d65c4b4a373f8165f20033bad1ffbfc2f` passed focused security recheck with **APPROVE**, no material issues and 18 tests; strict pre/post checks, bounded 10-ms polling and one 1,000-ms send/ack/exit/EOF deadline now require timely zero exit plus clean EOF, make all trailing/partial/body/malformed output fatal, and kill/wait/reap/join every failure. `P7-PC-SEC-002`/`003` are fixed and closed, and the process owner is accepted for serialized supervisor consumption. The wakeable runtime-driver adaptation is underway because the synchronous API cannot itself multiplex, but it is not implemented or reviewed; supervisor acceptance and current-head CI remain pending.
