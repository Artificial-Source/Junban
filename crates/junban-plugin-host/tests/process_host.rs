...
fn timeout_epoch_and_trap_replacement_are_isolated_per_plugin() {
    ...
    assert!(saw_timeout);
    // The parent watchdog may have sent this exact cancel before it observed
    // the timeout terminal. Once that terminal is queued, the late cancel is
    // idempotent and must not publish a second stale-authority terminal.
    host.send(
        &ParentFrame::Cancel {
            fence: spin.clone(),
        },
