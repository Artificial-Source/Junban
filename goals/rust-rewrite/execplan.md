...
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Contract remains `evidence/phase-7-context-map.md`. Planning, schema, package, WIT and host-placement gates are approved.
  ...
  - [ ] Wave 2 child-runtime security correction: `P7-RUNTIME-SEC-001` found canonical ABI lifting can amplify 64/128-MiB guest strings/lists into native allocations before post-lift callback/output limits. Add a hard cross-platform child-process memory ceiling and amplification/replacement evidence, then pass focused recheck before continuing Slice 2C.
    - [x] Calibration-only preflight and exact run `31008905408`: 128-MiB Wasmtime reservation with zero guard/growth reservation, retained valid Rust/TypeScript working-set callback barriers, and exact-child release sampling passed one warm-up plus five runs per profile on Linux/macOS/Windows. Linux and Windows have candidate separation; macOS's roughly 415-GiB valid VSIZE makes its mechanical 519-GiB `RLIMIT_AS` minimum ineffective. `P7-PLAN-RUNTIME-001`, cap selection/enforcement, amplification proof and both finding closures remain pending; no cap or Slice 2C authorization is claimed.
...
- 2026-08-05: Exact clean-commit run `31008905408` passed one warm-up plus five optimized runs per Rust/TypeScript profile on Linux, macOS and Windows. Linux virtual-address and Windows private-commit ranges separate valid workloads from a candidate cap, but macOS reports roughly 415 GiB valid VSIZE, making its mechanical 519-GiB `RLIMIT_AS` minimum ineffective. A corrected macOS native-allocation/process authority, hostile canonical-lift triggering, enforcement and focused security recheck remain mandatory; no cap is frozen and Slice 2C stays blocked.
