...
**Status:** approved by the user on 2026-07-28 and expanded with an evidence-driven Phase 10 on 2026-07-29. Phases 0 through 6 are complete. Phase 7 capability-limited portable plugins are in progress: Waves 1 and child-runtime Slice 2B are accepted; Slice 2C Packet B lifecycle authority commit `ae9cd24` is integrated, and its focused database recheck approved exact `e573cfe` with no material issues, fixed/closed `P7-2C-B-DB-001` and `P7-2C-B-DB-002`, left schema v7 unchanged, and authorized Packet C to consume those APIs. The completed single hostile-runtime/security review accepted Packet A protocol-v2/multi-runtime child exact commit `651cf7530c951302be0e135143360706a42a9eac` with no material security issues. Integrated Packet A/B head `0f2f9b31633470dc75973fb15aef99ade354eafb` passed full CI and process-memory calibration. Packet C foundation commits `aa3e591` (shared SDK framing codec) and `48ac139` (verified component-source port) are integrated; the completed process-transport review found no material codec issue but no remote current-head gate is claimed, while the source port has focused security/integrity **APPROVE** with no material issues. Strict parent process transport is present at `cc23507`; its completed security review rejected `P7-PC-SEC-001` under the frozen same-user threat model and left medium/open `P7-PC-SEC-002`/`003` with focused fixes underway, so process owner/supervisor acceptance remains blocked. Separately, `P7-PLAN-2C-005` is fixed by the written triggerless graph-fence correction but awaits focused planning recheck and a subsequent Packet B validator/test correction before supervisor coding. Current C1-head CI and integrated evidence remain pending. `P7-DEP-001` remains open, Wave 2 and Slice 2C are not accepted, and Phases 8–10 remain.
...
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Contract remains `evidence/phase-7-context-map.md`. Wave 0/1 planning, schema, package, WIT and host-placement gates are approved. The prior exact-HEAD Slice 2C planning verdict remains **APPROVE** for its reviewed authority, while new correction `P7-PLAN-2C-005` awaits focused planning recheck and the subsequent Packet B validator/test correction before supervisor coding.
  ...
  - [x] Wave 2 Slice 2C process-transport security review: current transport is present at `cc23507`, and the completed focused review found no material codec issue. `P7-PC-SEC-001` is rejected as a blocker because same-user Junban binary/filesystem control is outside the frozen threat model; supported Phase 8/9 packaging must verify that the server/host sibling directory is writable only by the installation owner or administrator/elevated authority, while a less-privileged writer is an unsupported insecure installation with fail/stop guidance. No descriptor-bound execution is claimed. Medium `P7-PC-SEC-002` (late-exit acceptance plus busy spin) and `P7-PC-SEC-003` (trailing stdout discarded after `ShutdownComplete`) remain open.
  - [ ] Wave 2 Slice 2C process-owner correction: fix `P7-PC-SEC-002` and `P7-PC-SEC-003` with focused regressions and pass focused security recheck before accepting the process owner or beginning supervisor acceptance.
  - [ ] Wave 2 Slice 2C Packet C through Slice 2E: Packet C, Slice 2C and Wave 2 remain unaccepted and incomplete. Process transport at `cc23507` is blocked on medium/open `P7-PC-SEC-002`/`003`; completion of the separate `P7-PLAN-2C-005` planning recheck and Packet B validator/test correction, parent supervision and compile/load kill/reap, capability/effect/dependency/event authority, current C1-head CI, integrated security review and optimized replacement measurements remains. The exact 1,000-ms parent control deadline remains frozen at docs commit `18fbb87`. Slice 2E may use one non-shipped real-supervisor/storage/AppService/sibling-host harness for separate Rust/TypeScript and one/four/sixteen-runtime replacement evidence only; Wave 3 ordinary `ServerState` composition and Wave 5 product-integrated evidence remain separate. Replacement CI run `31296663305` and process-memory calibration run `31296660062` passed at exact head `ce80dead79c5ec2dadd95bddb8202ee7dbd2bf46`; all three strict exact-head macOS containment executions (`93202756069`, `93205509663`, `93207040552`) passed. Integrated Packet A/B CI and process-memory calibration passed later at exact `0f2f9b31633470dc75973fb15aef99ade354eafb`, but those runs do not cover the current Packet C1 head through `48ac139`. Current-head CI, Slice 2C acceptance and Wave 2 acceptance are not claimed. `P7-DEP-001` remains open.
...
- Phase 7 verified component-source security/integrity review — **APPROVE; no material issues** at exact commit `48ac139b0ad6109a1784d0ad2043916b80a16e00`. Static Linux/macOS/Windows path review and 13 focused source/package/normal-open/restore tests passed, so the parent supervisor may consume `AppService::open_plugin_component_sources`. The public hidden constructor remains trusted in-repository only and must not be called by the supervisor. No stable finding ID was created because no issue was found. This does not accept the strict process owner, supervisor, Slice 2C or Wave 2.
- Phase 7 process-transport security review — **blocked pending fixes** at current transport `cc23507`. The shared codec had no material issue. `P7-PC-SEC-001` is **high/rejected**, not fixed: same-user control of the Junban binary/filesystem account is outside the frozen threat model, so supported packaging instead must verify the server/host sibling directory is writable only by the installation owner or administrator/elevated authority; a less-privileged writer is an unsupported insecure installation with fail/stop guidance, not an in-scope attacker, and descriptor-bound execution is not claimed. `P7-PC-SEC-002` and `P7-PC-SEC-003` are **medium/open** for late-exit acceptance plus busy spin and discarded trailing stdout after `ShutdownComplete`. Focused fixes and recheck are required before process-owner or supervisor acceptance; current C1-head CI remains pending.
...
- 2026-08-09: `P7-PLAN-2C-005` freezes triggerless graph fencing without changing contracts or claiming implementation: complete selected max-16 fences have zero or exactly one `Failing`; only exact plugin correlation authorizes a trigger, while Hello/idle/uncorrelated loss assigns every selection `LoadedSibling` + `SessionLost`. The focused planning recheck is pending. Packet B's validator/test correction from at-least-one/multiple-allowed to zero-or-one is required only after planning approval and before supervisor coding.
- 2026-08-09: Process transport commit `cc23507` completed focused security review. The codec had no material issue. `P7-PC-SEC-001` is rejected as a blocker under the existing same-user binary/filesystem threat-model exclusion; supported Phase 8/9 packaging must verify owner/mode/ACL on the server/host sibling directory, and a less-privileged-writable installation is unsupported/insecure with fail/stop guidance. No descriptor-bound execution is claimed. Medium/open `P7-PC-SEC-002` and `P7-PC-SEC-003` require focused fixes and recheck, so the process owner/supervisor remain unaccepted; current C1-head CI is still pending.
