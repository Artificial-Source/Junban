...
**Status:** approved by the user on 2026-07-28 and expanded with an evidence-driven Phase 10 on 2026-07-29. Phases 0 through 6 are complete. Phase 7 capability-limited portable plugins are in progress: Waves 1 and child-runtime Slice 2B are implemented; the approved hostcall-fuel recheck fixes `P7-PLAN-RUNTIME-001`/`P7-RUNTIME-SEC-001`, and the focused Slice 2C planning verdict at exact reviewed HEAD `442ebdd61e71a19a3e906b33b54bf7857ad555f8` is **APPROVE** with no blockers. `P7-PLAN-2C-001`–`003` remain closed; `P7-PLAN-2C-004` and `P7-PLAN-2C-004-EVENT-ATOMICITY` are fixed and closed; Slice 2C is authorized to implement from planning and security. `P7-DEP-001` remains open, Wave 2 is not accepted, and a clean replacement full CI run at current head `ce80dea` is pending. Phases 8–10 remain.
...
- [ ] Phase 7 portable-plugin implementation (Waves 1–5). Contract remains `evidence/phase-7-context-map.md`. Wave 0/1 planning, schema, package, WIT and host-placement gates are approved; the exact-HEAD Slice 2C planning verdict is **APPROVE** with no blockers, and Slice 2C is authorized to implement from planning and security.
  ...
  - [ ] Wave 2 Slices 2C–2E: parent supervisor/verified source and compile-time kill/reap, capability/effect/dependency/event authority, integrated security review and optimized replacement measurements remain. Slice 2E may use one non-shipped real-supervisor/storage/AppService/sibling-host harness for separate Rust/TypeScript and one/four/sixteen-runtime replacement evidence only; Wave 3 ordinary `ServerState` composition and Wave 5 product-integrated evidence remain separate. Cross-platform plugin-host containment passed on exact code head `0220c8c67bd1da94f90fecc11aeabf87e784f777` in CI run `31294773893` jobs `93197967769`, `93197967800`, and `93197967814`; that run's only other failures were unrelated storage test isolation and newly disclosed tooling advisories, fixed at `ce80dead79c5ec2dadd95bddb8202ee7dbd2bf46`. A clean replacement full CI run is pending; Wave 2 and `P7-DEP-001` remain open.
...
- Phase 7 hostcall-fuel security recheck — **approved after fixes**. The recheck closes `P7-PLAN-RUNTIME-001` and `P7-RUNTIME-SEC-001` and authorizes Slice 2C from that security gate. `P7-DEP-001`, parent compile/load reap, clean replacement full CI at the current head, and integrated evidence remain open; Wave 2 is not accepted.
...
- 2026-08-09: On exact code head `0220c8c67bd1da94f90fecc11aeabf87e784f777`, Phase 7 process-memory calibration run `31294772199` passed, and CI run `31294773893` jobs `93197967769`, `93197967800`, and `93197967814` passed Ubuntu, macOS, and Windows plugin-host containment, proving the competing `Timeout`/`ResourceLimit` assertion correction. That CI run failed only unrelated storage test isolation and newly disclosed tooling advisories, both fixed at `ce80dead79c5ec2dadd95bddb8202ee7dbd2bf46`. A clean replacement full CI run remains pending; current-head full-CI green and Wave 2 acceptance are not claimed.
